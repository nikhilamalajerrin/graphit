...
GraphIt is an open-source AI graph processing pipeline. We take security
...
pull requests, or discussions.** Doing so could expose users of GraphIt to
risk before a fix is available.

Instead, report security vulnerabilities by emailing the GraphIt security
team directly:
...
- The affected component(s) and version(s) of GraphIt
...
- Core GraphIt Python packages (`graphit`, `graphit-base`, etc.)
- The GraphIt REST/graph gateway and processing pipeline components
...
- Third-party services or infrastructure not maintained by GraphIt
...
GraphIt follows the principle of
