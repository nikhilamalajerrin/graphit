"""
Unit tests for graphit.decoding.mistral_ocr.processor
"""
...
from graphit.decoding.mistral_ocr.processor import Processor
from graphit.schema import Document, TextDocument, Metadata, Triples
...
class TestMistralOcrProcessor(IsolatedAsyncioTestCase):
    ...
    @patch('graphit.base.librarian_client.Consumer')
    @patch('graphit.base.librarian_client.Producer')
    @patch('graphit.decoding.mistral_ocr.processor.Mistral')
    @patch('graphit.base.async_processor.AsyncProcessor', MockAsyncProcessor)
    ...
    async def test_ocr_single_chunk(
        ...
        with patch('graphit.decoding.mistral_ocr.processor.PdfReader', return_value=mock_pdf_reader):
            with patch('graphit.decoding.mistral_ocr.processor.PdfWriter') as mock_pdf_writer_class:
    ...
    async def test_chunks_function(self, mock_mistral_class):
        ...
        from graphit.decoding.mistral_ocr.processor import chunks
    ...
    async def test_replace_images_in_markdown(self, mock_mistral_class):
        ...
        from graphit.decoding.mistral_ocr.processor import replace_images_in_markdown
    ...
    @patch('graphit.base.flow_processor.FlowProcessor.add_args')
    ...
    @patch('graphit.decoding.mistral_ocr.processor.Processor.launch')
    def test_run(self, mock_launch):
        """Test run function"""
        from graphit.decoding.mistral_ocr.processor import run
        run()
