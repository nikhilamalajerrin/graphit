...
from graphit.api.socket_client import SocketClient
from graphit.api.types import AgentThought, AgentObservation, AgentAnswer
...
class TestParseChunkMessageId:
    ...
    def test_thought_message_id(self, client):
        resp = {
            ...
            "message_id": "urn:graphit:agent:sess/i1/thought",
        }
        chunk = client._parse_chunk(resp)
        assert isinstance(chunk, AgentThought)
        assert chunk.message_id == "urn:graphit:agent:sess/i1/thought"
    ...
    def test_observation_message_id(self, client):
        resp = {
            ...
            "content": "result",
            "end_of_message": True,
            "message_id": "urn:graphit:agent:sess/i1/observation",
        }
        chunk = client._parse_chunk(resp)
        assert isinstance(chunk, AgentObservation)
        assert chunk.message_id == "urn:graphit:agent:sess/i1/observation"
    ...
    def test_answer_message_id(self, client):
        resp = {
            ...
            "end_of_message": False,
            "end_of_dialog": False,
            "message_id": "urn:graphit:agent:sess/final",
        }
        chunk = client._parse_chunk(resp)
        assert isinstance(chunk, AgentAnswer)
        assert chunk.message_id == "urn:graphit:agent:sess/final"
