...
from graphit.base.graph_rag_client import GraphRagClient
...
def sample_focus_triples():
    ...
    return [
        make_triple(
            "urn:graphit:focus:abc",
            "http://www.w3.org/1999/02/22-rdf-syntax-ns#type",
            "https://graphit.ai/ns/Focus",
        ),
        make_triple(
            "urn:graphit:focus:abc",
            "http://www.w3.org/ns/prov#wasDerivedFrom",
            "urn:graphit:exploration:abc",
        ),
        make_triple(
            "urn:graphit:focus:abc",
            "https://graphit.ai/ns/selectedEdge",
            "urn:graphit:edge-sel:abc:0",
        ),
    ]
...
class TestGraphRagServiceExplainTriples:
    ...
    @patch('graphit.retrieval.graph_rag.rag.GraphRag')
    @pytest.mark.asyncio
    async def test_explain_messages_include_triples(self, mock_graph_rag_class):
        ...
        """
        from graphit.retrieval.graph_rag.rag import Processor
        ...
        async def mock_query(**kwargs):
            ...
            if explain_callback:
                await explain_callback(
                    question_triples, "urn:graphit:question:abc"
                )
                await explain_callback(
                    focus_triples, "urn:graphit:focus:abc"
                )
            return "The answer."
        ...
        assert explain_msgs[0].explain_id == "urn:graphit:question:abc"
        assert explain_msgs[0].explain_triples == question_triples

        # Second explain message should carry focus triples
        assert explain_msgs[1].explain_id == "urn:graphit:focus:abc"
...
class TestGraphRagClientExplainForwarding:
    ...
    async def test_explain_callback_receives_triples(self):
        ...
        responses = [
            GraphRagResponse(
                ...
                explain_id="urn:graphit:focus:abc",
        ...
        assert received_calls[0]["explain_id"] == "urn:graphit:focus:abc"
    ...
    async def test_explain_callback_receives_empty_triples(self):
        ...
        responses = [
            GraphRagResponse(
                message_type="explain",
                explain_id="urn:graphit:question:abc",
                explain_graph="urn:graph:retrieval",
                explain_triples=[],
            ),
            GraphRagResponse(
    ...
    async def test_multiple_explain_events_all_forward_triples(self):
        ...
        assert received_calls[0]["explain_id"] == "urn:graphit:question:abc"
        assert received_calls[0]["explain_triples"] == q_triples
        assert received_calls[1]["explain_id"] == "urn:graphit:focus:abc"
