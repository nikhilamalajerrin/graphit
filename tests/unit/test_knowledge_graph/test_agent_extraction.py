...
from graphit.extract.kg.agent.extract import Processor as AgentKgExtractor
from graphit.schema import Chunk, Triple, Triples, Metadata, Term, Error, IRI, LITERAL
from graphit.schema import EntityContext, EntityContexts
from graphit.rdf import GRAPHIT_ENTITIES, DEFINITION, RDF_LABEL
from graphit.template.prompt_manager import PromptManager
...
class TestAgentKgExtractor:
    ...
    def agent_extractor(self):
        ...
        from graphit.extract.kg.agent.extract import Processor
    ...
    def test_to_uri_conversion(self, agent_extractor):
        ...
        expected = f"{GRAPHIT_ENTITIES}Machine%20Learning"
        assert uri == expected

        # Test entity with special characters
        uri = agent_extractor.to_uri("Entity with & special chars!")
        expected = f"{GRAPHIT_ENTITIES}Entity%20with%20%26%20special%20chars%21"
        assert uri == expected

        # Test empty string
        uri = agent_extractor.to_uri("")
        expected = f"{GRAPHIT_ENTITIES}"
        assert uri == expected
...
{"type": "definition", "entity": "Trunca'''
    ...
    def test_process_extraction_data_definitions(self, agent_extractor, sample_metadata):
        ...
        assert label_triple.s.iri == f"{GRAPHIT_ENTITIES}Machine%20Learning"
        ...
        assert def_triple.s.iri == f"{GRAPHIT_ENTITIES}Machine%20Learning"
        assert def_triple.o.value == "A subset of AI that enables learning from data."

        # Check entity context
        assert len(entity_contexts) == 1
        assert entity_contexts[0].entity.iri == f"{GRAPHIT_ENTITIES}Machine%20Learning"
    ...
    def test_process_extraction_data_relationships(self, agent_extractor, sample_metadata):
        ...
        subject_uri = f"{GRAPHIT_ENTITIES}Machine%20Learning"
        predicate_uri = f"{GRAPHIT_ENTITIES}is_subset_of"
        ...
        object_uri = f"{GRAPHIT_ENTITIES}Artificial%20Intelligence"
    ...
    def test_process_extraction_data_combined(self, agent_extractor, sample_metadata, sample_extraction_data):
        ...
        assert f"{GRAPHIT_ENTITIES}Machine%20Learning" in entity_uris
        assert f"{GRAPHIT_ENTITIES}Neural%20Networks" in entity_uris
