...
from graphit.exceptions import TooManyRequests, LlmError


class TestAzureServerless429(IsolatedAsyncioTestCase):
    """Azure serverless endpoint: HTTP 429 → TooManyRequests"""

    @patch('graphit.model.text_completion.azure.llm.requests')
    @patch('graphit.base.async_processor.AsyncProcessor.__init__', return_value=None)
    @patch('graphit.base.llm_service.LlmService.__init__', return_value=None)
    async def test_http_429_raises_too_many_requests(self, _llm, _async, mock_requests):
        from graphit.model.text_completion.azure.llm import Processor
        proc = Processor(
...
class TestAzureOpenAIRateLimit(IsolatedAsyncioTestCase):
    ...
    @patch('graphit.model.text_completion.azure_openai.llm.AzureOpenAI')
    @patch('graphit.base.async_processor.AsyncProcessor.__init__', return_value=None)
    @patch('graphit.base.llm_service.LlmService.__init__', return_value=None)
    async def test_rate_limit_error_raises_too_many_requests(self, _llm, _async, mock_cls):
        from openai import RateLimitError
        from graphit.model.text_completion.azure_openai.llm import Processor
...
class TestOpenAIRateLimit(IsolatedAsyncioTestCase):
    ...
    @patch('graphit.model.text_completion.openai.llm.OpenAI')
    @patch('graphit.base.async_processor.AsyncProcessor.__init__', return_value=None)
    @patch('graphit.base.llm_service.LlmService.__init__', return_value=None)
    async def test_rate_limit_error_raises_too_many_requests(self, _llm, _async, mock_cls):
        from openai import RateLimitError
        from graphit.model.text_completion.openai.llm import Processor
        mock_client = MagicMock()
        mock_cls.return_value = mock_client
        proc = Processor(
...
class TestClaudeRateLimit(IsolatedAsyncioTestCase):
    ...
    @patch('graphit.model.text_completion.claude.llm.anthropic')
    @patch('graphit.base.async_processor.AsyncProcessor.__init__', return_value=None)
    @patch('graphit.base.llm_service.LlmService.__init__', return_value=None)
    async def test_rate_limit_error_raises_too_many_requests(self, _llm, _async, mock_anthropic):
        from graphit.model.text_completion.claude.llm import Processor
...
class TestMistralRateLimit(IsolatedAsyncioTestCase):
    ...
    @patch('graphit.model.text_completion.mistral.llm.Mistral')
    @patch('graphit.model.text_completion.mistral.llm.models')
    @patch('graphit.base.async_processor.AsyncProcessor.__init__', return_value=None)
    @patch('graphit.base.llm_service.LlmService.__init__', return_value=None)
    async def test_429_raises_too_many_requests(self, _llm, _async, mock_models, mock_cls):
        from graphit.model.text_completion.mistral.llm import Processor
        mock_client = MagicMock()
        mock_cls.return_value = mock_client
        proc = Processor(
            api_key="k", concurrency=1, taskgroup=AsyncMock(), id="t",
...
class TestCohereRateLimit(IsolatedAsyncioTestCase):
    ...
    @patch('graphit.model.text_completion.cohere.llm.cohere')
    @patch('graphit.base.async_processor.AsyncProcessor.__init__', return_value=None)
    @patch('graphit.base.llm_service.LlmService.__init__', return_value=None)
    async def test_rate_limit_error_raises_too_many_requests(self, _llm, _async, mock_cohere):
        from graphit.model.text_completion.cohere.llm import Processor
        import graphit.model.text_completion.cohere.llm as cohere_llm
...
class TestVllmRateLimit(IsolatedAsyncioTestCase):
    ...
    @patch('graphit.model.text_completion.vllm.llm.aiohttp.ClientSession')
    @patch('graphit.base.async_processor.AsyncProcessor.__init__', return_value=None)
    @patch('graphit.base.llm_service.LlmService.__init__', return_value=None)
    async def test_429_raises_too_many_requests(self, _llm, _async, mock_session):
        from graphit.model.text_completion.vllm.llm import Processor
...
class TestClientSideRateLimitTranslation:
    ...
    def test_error_type_mapping(self):
        ...
        from graphit.schema import Error
