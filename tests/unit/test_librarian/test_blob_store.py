...
from graphit.librarian.blob_store import BlobStore
...
def _make_blob_store():
    ...
    with patch('graphit.librarian.blob_store.Minio', return_value=mock_minio):
        # Prevent ensure_bucket from making network calls during init
        with patch('graphit.librarian.blob_store.BlobStore.ensure_bucket'):
            store = BlobStore(
