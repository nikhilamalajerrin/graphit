...
import pytest
from graphit.schema import (
    GraphRagResponse, DocumentRagResponse, AgentResponse,
    Term, Triple, IRI, LITERAL, Error,
)
from graphit.messaging.translators.retrieval import (
    GraphRagResponseTranslator,
    DocumentRagResponseTranslator,
)
from graphit.messaging.translators.agent import (
    AgentResponseTranslator,
)
from graphit.api.types import ProvenanceEvent
...
def sample_triples():
    ...
    return [
        make_triple(
            "urn:graphit:question:abc123",
            "http://www.w3.org/1999/02/22-rdf-syntax-ns#type",
            "https://graphit.ai/ns/GraphRagQuestion",
            o_type=IRI,
        ),
        make_triple(
            "urn:graphit:question:abc123",
            "https://graphit.ai/ns/query",
...
class TestGraphRagExplainTriples:
    ...
    def test_explain_triples_encoded(self):
        ...
        response = GraphRagResponse(
            ...
            explain_id="urn:graphit:question:abc123",
        ...
        assert t["s"]["t"] == "i"
        assert t["s"]["i"] == "urn:graphit:question:abc123"
        assert t["p"]["t"] == "i"
    ...
    def test_explain_id_and_graph_included(self):
        ...
        result = translator.encode(response)
        assert result["explain_id"] == "urn:graphit:question:abc123"
...
class TestDocumentRagExplainTriples:
    ...
    def test_explain_triples_encoded(self):
        ...
        response = DocumentRagResponse(
            response=None,
            message_type="explain",
            explain_id="urn:graphit:docrag:abc123",
            explain_graph="urn:graph:retrieval",
            explain_triples=sample_triples(),
        )
...
class TestAgentExplainTriples:
    ...
    def test_explain_triples_encoded(self):
        ...
        response = AgentResponse(
            message_type="explain",
            content="",
            explain_id="urn:graphit:agent:session:abc123",
            explain_graph="urn:graph:retrieval",
            explain_triples=sample_triples(),
        )
        ...
        assert t["p"]["i"] == "https://graphit.ai/ns/query"
        assert t["o"]["t"] == "l"
...
class TestBuildProvenanceEvent:
    ...
    def _make_client(self):
        ...
        from graphit.api.socket_client import WebSocketClient
    ...
    def test_entity_parsed_from_wire_triples(self):
        ...
        from graphit.api.explainability import ExplainEntity

        wire_triples = [
            {
                "s": {"t": "i", "i": "urn:graphit:question:abc123"},
                "p": {"t": "i", "i": "http://www.w3.org/1999/02/22-rdf-syntax-ns#type"},
                "o": {"t": "i", "i": "https://graphit.ai/ns/GraphRagQuestion"},
            },
            {
                "s": {"t": "i", "i": "urn:graphit:question:abc123"},
                "p": {"t": "i", "i": "https://graphit.ai/ns/query"},
        ...
        entity = ExplainEntity.from_triples(
            "urn:graphit:question:abc123", parsed
        )
