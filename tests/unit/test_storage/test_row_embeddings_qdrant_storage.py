...
"""
Unit tests for graphit.storage.row_embeddings.qdrant.write
...
class TestQdrantRowEmbeddingsStorage(IsolatedAsyncioTestCase):
    ...
    @patch('graphit.storage.row_embeddings.qdrant.write.QdrantClient')
    async def test_processor_initialization_basic(self, mock_qdrant_client):
        """Test basic Qdrant processor initialization"""
        from graphit.storage.row_embeddings.qdrant.write import Processor
    ...
    @patch('graphit.storage.row_embeddings.qdrant.write.QdrantClient')
    @patch('graphit.storage.row_embeddings.qdrant.write.uuid')
    async def test_on_embeddings_basic(self, mock_uuid, mock_qdrant_client):
        """Test processing basic row embeddings message"""
        from graphit.storage.row_embeddings.qdrant.write import Processor
        from graphit.schema import RowEmbeddings, RowIndexEmbedding, Metadata
    ...
    async def test_on_embeddings_single_vector(self, mock_uuid, mock_qdrant_client):
        ...
        from graphit.storage.row_embeddings.qdrant.write import Processor
        from graphit.schema import RowEmbeddings, RowIndexEmbedding
