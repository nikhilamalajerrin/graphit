...
from graphit.storage.triples.cassandra.write import Processor
from graphit.schema import Triple, LITERAL, IRI
from graphit.direct.cassandra_kg import DEFAULT_GRAPH
...
class TestCassandraStorageProcessor:
    ...
    @pytest.mark.asyncio
    @patch('graphit.storage.triples.cassandra.write.EntityCentricKnowledgeGraph')
    ...
    async def test_table_reuse_when_same(self, mock_kg_class):
        """Test that GraphIt is not recreated when table hasn't changed"""
        taskgroup_mock = MagicMock()
        mock_tg_instance = MagicMock()
        mock_tg_instance.async_insert = AsyncMock()
        mock_kg_class.return_value = mock_tg_instance
        ...
        # First call should create GraphIt
        await processor.store_triples('user1', mock_message)
        assert mock_kg_class.call_count == 1

        # Second call with same table should reuse GraphIt
        await processor.store_triples('user1', mock_message)
    ...
    async def test_exception_handling_on_connection_failure(self, mock_kg_class):
        """Test exception handling during GraphIt creation"""
        taskgroup_mock = MagicMock()
    ...
    def test_add_args_method(self):
        ...
        with patch('graphit.storage.triples.cassandra.write.TriplesStoreService.add_args') as mock_parent_add_args:
    ...
    def test_add_args_with_custom_values(self):
        ...
        with patch('graphit.storage.triples.cassandra.write.TriplesStoreService.add_args'):
            Processor.add_args(parser)
    ...
    @patch('graphit.storage.triples.cassandra.write.Processor.launch')
    def test_run_function(self, mock_launch):
        """Test the run function calls Processor.launch with correct parameters"""
        from graphit.storage.triples.cassandra.write import run, default_ident
