"""
Unit tests for graphit.storage.doc_embeddings.qdrant.write
...
from graphit.storage.doc_embeddings.qdrant.write import Processor


class TestQdrantDocEmbeddingsStorage(IsolatedAsyncioTestCase):
    """Test Qdrant document embeddings storage functionality"""

    @patch('graphit.storage.doc_embeddings.qdrant.write.QdrantClient')
    ...
    @patch('graphit.storage.doc_embeddings.qdrant.write.QdrantClient')
    @patch('graphit.storage.doc_embeddings.qdrant.write.uuid')
    ...
    async def test_add_args_calls_parent(self, mock_qdrant_client):
        ...
        # Act
        with patch('graphit.base.DocumentEmbeddingsStoreService.add_args') as mock_parent_add_args:
    ...
    async def test_chunk_id_with_special_characters(self, mock_uuid, mock_qdrant_client):
        ...
        mock_chunk = MagicMock()
        mock_chunk.chunk_id = 'https://graphit.ai/doc/my-document/p1/c3'
        mock_chunk.vector = [0.1, 0.2]
        ...
        upsert_call_args = mock_qdrant_instance.upsert.call_args
        point = upsert_call_args[1]['points'][0]
        assert point.payload['chunk_id'] == 'https://graphit.ai/doc/my-document/p1/c3'
