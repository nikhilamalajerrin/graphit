...
from graphit.schema.core.topic import queue
from graphit.base.pubsub import get_pubsub, add_pubsub_args
from graphit.base.pulsar_backend import PulsarBackend
...
class TestQueueDefinitions:
    ...
    def test_config_request(self):
        from graphit.schema.services.config import config_request_queue
        assert config_request_queue == 'request:tg:config'

    def test_config_response(self):
        from graphit.schema.services.config import config_response_queue
        assert config_response_queue == 'response:tg:config'

    def test_config_push(self):
        from graphit.schema.services.config import config_push_queue
        assert config_push_queue == 'notify:tg:config'

    def test_librarian_request(self):
        from graphit.schema.services.library import librarian_request_queue
        assert librarian_request_queue == 'request:tg:librarian'

    def test_knowledge_request(self):
        from graphit.schema.knowledge.knowledge import knowledge_request_queue
