...
from graphit.schema import Triple, Term, IRI, LITERAL

from graphit.provenance.agent import (
...
from graphit.provenance.namespaces import (
    RDF_TYPE, RDFS_LABEL,
...
class TestAgentSessionTriples:
    ...
    SESSION_URI = "urn:graphit:agent:test-session"
    ...
    def test_session_parent_uri(self):
        ...
        parent = "urn:graphit:agent:parent/decompose"
        triples = agent_session_triples(
            self.SESSION_URI, "Q", "2024-01-01T00:00:00Z",
            parent_uri=parent,
        )
...
class TestAgentIterationTriples:
    ...
    ITER_URI = "urn:graphit:agent:test-session/i1"
    SESSION_URI = "urn:graphit:agent:test-session"
    PREV_URI = "urn:graphit:agent:test-session/i0"
    ...
    def test_iteration_thought_sub_entity(self):
        ...
        thought_uri = "urn:graphit:agent:test-session/i1/thought"
    ...
    def test_iteration_chaining(self):
        ...
        iter1_uri = "urn:graphit:agent:sess/i1"
        iter2_uri = "urn:graphit:agent:sess/i2"
...
class TestAgentObservationTriples:
    ...
    OBS_URI = "urn:graphit:agent:test-session/i1/observation"
    ITER_URI = "urn:graphit:agent:test-session/i1"
...
class TestAgentFinalTriples:
    ...
    FINAL_URI = "urn:graphit:agent:test-session/final"
    PREV_URI = "urn:graphit:agent:test-session/i3"
    SESSION_URI = "urn:graphit:agent:test-session"
    ...
    def test_final_document_reference(self):
        triples = agent_final_triples(
            self.FINAL_URI, previous_uri=self.PREV_URI,
            document_id="urn:graphit:agent:sess/answer",
        )
        doc = find_triple(triples, TG_DOCUMENT, self.FINAL_URI)
        assert doc is not None
        assert doc.o.type == IRI
        assert doc.o.iri == "urn:graphit:agent:sess/answer"
...
class TestAgentSynthesisTriples:
    ...
    SYNTH_URI = "urn:graphit:agent:test-session/synthesis"
    FINDING_0 = "urn:graphit:agent:test-session/finding/0"
    FINDING_1 = "urn:graphit:agent:test-session/finding/1"
    FINDING_2 = "urn:graphit:agent:test-session/finding/2"
