...
from graphit.provenance.uris import (
    GRAPHIT_BASE,
    _encode_id,
    document_uri,
    page_uri,
...
class TestActivityAndSubgraphUris:
    ...
    def test_activity_uri_with_id(self):
        ...
        assert result == f"{GRAPHIT_BASE}/activity/my-activity-id"

    def test_activity_uri_auto_generates_uuid(self):
        result = activity_uri()
        assert result.startswith(f"{GRAPHIT_BASE}/activity/")
    ...
    def test_subgraph_uri_with_id(self):
        ...
        assert result == f"{GRAPHIT_BASE}/subgraph/sg-123"

    def test_subgraph_uri_auto_generates_uuid(self):
        result = subgraph_uri()
        assert result.startswith(f"{GRAPHIT_BASE}/subgraph/")
    ...
    def test_agent_uri_format(self):
        ...
        assert result == f"{GRAPHIT_BASE}/agent/pdf-extractor"
...
class TestGraphRagQueryUris:
    ...
    def test_question_uri_with_session_id(self):
        ...
        assert result == f"urn:graphit:question:{self.FIXED_UUID}"

    def test_question_uri_auto_generates(self):
        result = question_uri()
        assert result.startswith("urn:graphit:question:")
        uuid_part = result.split("urn:graphit:question:")[1]
        assert len(uuid_part) > 0
    ...
    def test_exploration_uri_format(self):
        ...
        assert result == f"urn:graphit:prov:exploration:{self.FIXED_UUID}"

    def test_focus_uri_format(self):
        result = focus_uri(self.FIXED_UUID)
        assert result == f"urn:graphit:prov:focus:{self.FIXED_UUID}"

    def test_synthesis_uri_format(self):
        result = synthesis_uri(self.FIXED_UUID)
        assert result == f"urn:graphit:prov:synthesis:{self.FIXED_UUID}"

    def test_edge_selection_uri_format(self):
        result = edge_selection_uri(self.FIXED_UUID, 3)
        assert result == f"urn:graphit:prov:edge:{self.FIXED_UUID}:3"
...
class TestAgentProvenanceUris:
    ...
    def test_agent_session_uri_with_id(self):
        ...
        assert result == f"urn:graphit:agent:{self.FIXED_UUID}"

    def test_agent_session_uri_auto_generates(self):
        result = agent_session_uri()
        assert result.startswith("urn:graphit:agent:")
    ...
    def test_agent_iteration_uri_format(self):
        ...
        assert result == f"urn:graphit:agent:{self.FIXED_UUID}/i1"
    ...
    def test_agent_final_uri_format(self):
        ...
        assert result == f"urn:graphit:agent:{self.FIXED_UUID}/final"
...
class TestDocRagProvenanceUris:
    ...
    def test_docrag_question_uri_with_id(self):
        ...
        assert result == f"urn:graphit:docrag:{self.FIXED_UUID}"

    def test_docrag_question_uri_auto_generates(self):
        result = docrag_question_uri()
        assert result.startswith("urn:graphit:docrag:")
    ...
    def test_docrag_exploration_uri_format(self):
        ...
        assert result == f"urn:graphit:docrag:{self.FIXED_UUID}/exploration"

    def test_docrag_synthesis_uri_format(self):
        result = docrag_synthesis_uri(self.FIXED_UUID)
        assert result == f"urn:graphit:docrag:{self.FIXED_UUID}/synthesis"
