...
def _make_processor(tools=None):
    ...
    def mock_session_uri(sid):
        return f"urn:graphit:agent:session:{sid}"
...
class TestAgentReactDagStructure:
    ...
    def _make_service(self):
        from graphit.agent.orchestrator.service import Processor
        from graphit.agent.orchestrator.react_pattern import ReactPattern
        from graphit.agent.orchestrator.plan_pattern import PlanThenExecutePattern
        from graphit.agent.orchestrator.supervisor_pattern import SupervisorPattern
    ...
    async def test_dag_chain(self):
        from graphit.agent.react.types import Action, Final
        ...
        with patch(
            "graphit.agent.orchestrator.react_pattern.AgentManager"
        ) as MockAM:
            mock_am = AsyncMock()
