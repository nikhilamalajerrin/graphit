# Contract Tests for GraphIt

This directory contains contract tests that verify service interface contracts, message schemas, and API compatibility across the GraphIt microservices architecture.
...
Tests the contracts for all Pulsar message schemas used in GraphIt service communication.
...
### Run with Coverage
```bash
pytest tests/contract/ -m contract --cov=graphit.schema --cov-report=html
```
...
The contract tests maintain a registry of all GraphIt schemas:
...
This ensures that GraphIt services can evolve independently while maintaining stable, compatible interfaces for all service communication.