...
from graphit.storage.rows.cassandra.write import Processor
from graphit.schema import ExtractedObject, Metadata, RowSchema, Field
...
class TestRowsCassandraIntegration:
    ...
    def patch_async_execute(self):
        ...
        async def _fake(session, query, params=None):
            ...
            return []
        with patch(
            'graphit.storage.rows.cassandra.write.async_execute',
            new=_fake,
        ):
            yield
    ...
    async def test_end_to_end_object_storage(self, processor_with_mocks):
        ...
        with patch('graphit.storage.rows.cassandra.write.Cluster', return_value=mock_cluster):
    ...
    async def test_authentication_handling(self, processor_with_mocks):
        ...
        with patch('graphit.storage.rows.cassandra.write.Cluster') as mock_cluster_class:
            with patch('graphit.storage.rows.cassandra.write.PlainTextAuthProvider') as mock_auth:
