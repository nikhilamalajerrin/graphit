...
from graphit.storage.triples.cassandra.write import Processor as TriplesWriter
from graphit.storage.rows.cassandra.write import Processor as RowsWriter
from graphit.query.triples.cassandra.service import Processor as TriplesQuery
from graphit.storage.knowledge.store import Processor as KgStore
...
class TestEndToEndConfigurationFlow:
    ...
    @pytest.mark.asyncio
    @patch('graphit.direct.cassandra_kg.Cluster')
    async def test_triples_writer_env_to_connection(self, mock_cluster):
        """Test complete flow from environment variables to GraphIt connection."""
        env_vars = {
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            ...
            # Create a mock message to trigger GraphIt creation
            mock_message = MagicMock()
            mock_message.metadata.collection = 'test_collection'
            mock_message.triples = []

            # Mock collection_exists to return True
            with patch('graphit.direct.cassandra_kg.KnowledgeGraph.collection_exists', return_value=True):
                # This should create GraphIt with environment config
    ...
    @patch('graphit.storage.rows.cassandra.write.Cluster')
    @patch('graphit.storage.rows.cassandra.write.PlainTextAuthProvider')
    ...
    @pytest.mark.asyncio
    @patch('graphit.storage.knowledge.store.KnowledgeTableStore')
...
class TestConfigurationPriorityEndToEnd:
    ...
    async def test_no_config_defaults_end_to_end(self, mock_kg_class):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            # Mock query to trigger GraphIt creation
...
class TestMultipleHostsHandling:
    ...
    async def test_single_host_converted_to_list(self, mock_cluster):
        """Test that single host is converted to list for GraphIt."""
        mock_cluster_instance = MagicMock()
        mock_session = MagicMock()
        mock_cluster_instance.connect.return_value = mock_session
        mock_cluster.return_value = mock_cluster_instance
    ...
    def test_whitespace_handling_in_host_list(self):
        ...
        from graphit.base.cassandra_config import resolve_cassandra_config
