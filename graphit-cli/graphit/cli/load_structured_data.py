"""
Load structured data into GraphIt using a descriptor configuration.
...
4. Import processed data into GraphIt
...
def load_structured_data(
    ...
    Args:
        ...
        parse_only: Parse data but don't import to GraphIt
        load: Load data to GraphIt using existing descriptor
        ...
        flow: GraphIt flow name to use for prompts
    ...
    if auto:
        ...
        try:
            ...
            if dry_run:
                ...
                print(f"💡 Run without --dry-run to import data to GraphIt.")
                return None
            else:
                logger.info("Step 4: Importing data to GraphIt...")
                print("🚀 Importing data to GraphIt...")
                ...
                # Send to GraphIt using shared function
                imported_count = _send_to_graphit(output_objects, api_url, flow, batch_size, token=token, workspace=workspace)
        ...
        if response:
            ...
            print("Available schemas can be viewed using: gr-config-list schema")
        ...
        logger.info(f"Loading {input_file} to GraphIt using descriptor {descriptor_file}...")
        ...
        # Send to GraphIt
        print(f"🚀 Importing {len(output_records)} records to GraphIt...")
        imported_count = _send_to_graphit(output_records, api_url, flow, batch_size, token=token, workspace=workspace)
...
def _format_extracted_objects(processed_records, descriptor, collection):
    """Convert to GraphIt ExtractedObject format"""
    output_records = []
    schema_name = descriptor.get('output', {}).get('schema_name', 'default')
...
def _process_data_pipeline(input_file, descriptor_file, collection, sample_size=None):
    ...
    # Format output for GraphIt ExtractedObject structure
    output_records = _format_extracted_objects(processed_records, descriptor, collection)
    
    return output_records, descriptor


def _send_to_graphit(rows, api_url, flow, batch_size=1000, token=None, workspace="default"):
    """Send ExtractedObject records to GraphIt using Python API"""
    from graphit.api import Api

    try:
        total_records = len(rows)
        logger.info(f"Importing {total_records} records to GraphIt...")
        ...
        logger.info(f"Successfully imported {total_records} records to GraphIt")
        ...
        logger.error(f"Failed to import data to GraphIt: {e}")
...
def _auto_discover_schema(api_url, input_file, sample_chars, flow, logger, return_raw_response=False, token=None, workspace="default"):
    ...
    try:
        ...
        if not schema_keys:
            logger.error("No schemas available in GraphIt configuration")
            return None
        ...
        logger.info("Calling GraphIt schema-selection prompt...")
...
Examples:
  ...
  # Step 4: Import data to GraphIt using descriptor
...
Use Cases:
  ...
  --discover-schema     : Diagnose which GraphIt schemas might match your data
...
""",
    ...
    parser.add_argument(
        '-u', '--api-url',
        default=default_url,
        help=f'GraphIt API URL (default: {default_url})'
    )
    
    parser.add_argument(
        '-f', '--flow',
        default='default',
        help='GraphIt flow name to use for prompts and import (default: default)'
    )
    ...
    mode_group.add_argument(
        '--discover-schema',
        action='store_true',
        help='Analyze data sample and discover matching GraphIt schemas'
    )
    mode_group.add_argument(
        '--generate-descriptor',
        action='store_true', 
    ...
    mode_group.add_argument(
        '--parse-only',
        action='store_true',
        help='Parse data using descriptor but don\'t import to GraphIt'
    )
    mode_group.add_argument(
        '--load',
        action='store_true',
        help='Load data to GraphIt using existing descriptor'
    )
    mode_group.add_argument(
        '--auto',
        action='store_true',
