"""
Multi-queue Pulsar message dumper for debugging GraphIt message flows.
...
Uses GraphIt's Subscriber abstraction for future-proof pub/sub compatibility.
"""
...
from graphit.base.subscriber import Subscriber
from graphit.base.pubsub import get_pubsub, add_pubsub_args
...
Examples:
  ...
  gr-dump-queues flow:tg:agent-request:default \\
  ...
  gr-dump-queues persistent://tg/flow/agent-request:default

IMPORTANT:
  This tool subscribes to queues without a schema (schema-less mode). To avoid
  schema conflicts, ensure that GraphIt services and flows are already started
  ...
  Best practice: Start services → Set up flows → Run gr-dump-queues
        """,
