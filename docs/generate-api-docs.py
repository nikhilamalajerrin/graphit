#!/usr/bin/env python3
"""
Generate clean markdown documentation for graphit.api

This script introspects the graphit.api package and generates markdown
...
sys.path.insert(0, '../graphit-base')
...
def document_class(name, cls):
    ...
    # Import statement
    md.append(f"```python")
    md.append(f"from graphit.api import {name}")
    md.append(f"```\n")
...
def generate_toc(items):
    ...
    md = []
    md.append("# GraphIt Python API Reference\n")

    # Add introduction
    md.append("## Installation\n")
    md.append("```bash")
    md.append("pip install graphit")
    md.append("```\n")

    md.append("## Quick Start\n")
    md.append("All classes and types are imported from the `graphit.api` package:\n")
    md.append("```python")
    md.append("from graphit.api import Api, Triple, ConfigKey")
    md.append("")
...
def main():
    ...
    # Import the package
    try:
        api_module = importlib.import_module('graphit.api')
    except ImportError as e:
        print(f"Error importing graphit.api: {e}", file=sys.stderr)
        sys.exit(1)

    # Get exported names
    if not hasattr(api_module, '__all__'):
        print("Error: graphit.api has no __all__", file=sys.stderr)
        sys.exit(1)
