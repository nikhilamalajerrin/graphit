---
layout: default
title: "GraphIt Python API 参考"
parent: "Chinese (Beta)"
---

# GraphIt Python API 参考
...
主要的 GraphIt API 客户端，用于同步和异步操作。

此类提供对所有 GraphIt 服务的访问，包括流程管理、
...
`url`: GraphIt API 的基本 URL (默认值: "http://localhost:8088/"")
...
从 GraphIt 服务获取 Prometheus 格式的指标，用于监控和可观察性。
...
`question`: 自然语言问题
`user`: Cassandra keyspace 标识符（默认为："graphit"）
...
用于向GraphIt API发送经过身份验证的请求的内部方法。
...
提供了对基于 WebSocket 的 GraphIt 服务的同步接口，
...
`url`：GraphIt API 的基本 URL（HTTP/HTTPS 将转换为 WS/WSS）
...
`user`: 用户/键空间标识符。
`collection`: 集合标识符。
`api`: 用于图书管理员访问的 GraphIt Api 实例（可选）。
...
`user`: 用户/密钥空间标识符
`collection`: 集合标识符
`api`: 用于 librarian 访问的 GraphIt Api 实例 (可选)
`max_content`: 综合的最大内容长度
