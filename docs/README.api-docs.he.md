...
תיעוד ה-API בפייתון נוצר מתוך מחרוזות תיאור (docstrings) באמצעות סקריפט פייתון מותאם, אשר סוקר את החבילה `graphit.api`.

### תנאים מוקדמים

חבילת `graphit` חייבת להיות ניתנת לייבוא. אם אתם עובדים בסביבת פיתוח:
...
התיעוד שנוצר מציג את ה-API הציבורי בדיוק כפי שהמשתמשים מייבאים אותו מ-`graphit.api`, מבלי לחשוף את המבנה הפנימי של המודול.
