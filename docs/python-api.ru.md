---
layout: default
title: "Справочник API GraphIt на Python"
parent: "Russian (Beta)"
---

# Справочник API GraphIt на Python
...
Все классы и типы импортируются из пакета `graphit.api`:
...
Основной клиент API GraphIt для синхронных и асинхронных операций.

Этот класс предоставляет доступ ко всем сервисам GraphIt, включая управление потоками,
...
Инициализируйте клиент API GraphIt.

**Аргументы:**

`url`: Базовый URL для API GraphIt (по умолчанию: "http://localhost:8088/"")
...
Потоки являются основными единицами выполнения в GraphIt, предоставляя доступ к
...
Получает метрики в формате Prometheus из сервиса GraphIt
...
Этот класс предоставляет доступ ко всем сервисам GraphIt, включая:
...
`user`: Идентификатор пользователя (по умолчанию: "graphit")
...
`user`: Идентификатор пользователя/пространства (по умолчанию: "graphit")
...
`user`: Идентификатор пользователя/пространства ключей (по умолчанию: "graphit")
`collection`: Идентификатор коллекции (по умолчанию: "default")
...
`user`: Идентификатор пространства ключей Cassandra (по умолчанию: "graphit")
...
`url`: Базовый URL для API GraphIt.
...
Внутренний метод для выполнения аутентифицированных запросов к API GraphIt.
...
Предоставляет синхронный интерфейс для сервисов GraphIt, основанных на WebSocket,
...
`url`: Базовый URL для API GraphIt (HTTP/HTTPS будет преобразован в WS/WSS)
...
`url`: Базовый URL для API GraphIt (HTTP/HTTPS будут преобразованы в WS/WSS)
`timeout`: Время ожидания WebSocket в секундах
`token`: Необязательный токен для аутентификации
...
`user`: Идентификатор пользователя/пространства ключей.
`collection`: Идентификатор коллекции.
`api`: Экземпляр GraphIt Api для доступа к библиотекарю (необязательно).
...
`api`: Экземпляр GraphIt Api для доступа к библиотеке.
...
`user`: Идентификатор пользователя/пространства имен
`collection`: Идентификатор коллекции
`api`: Экземпляр GraphIt Api для доступа к библиотеке (необязательно)
`max_content`: Максимальная длина содержимого для синтеза
...
Базовый класс для всех ошибок сервиса GraphIt.
