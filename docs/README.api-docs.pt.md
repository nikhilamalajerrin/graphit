...
A documentação da API Python é gerada a partir de docstrings usando um script Python personalizado que inspeciona o pacote `graphit.api`.

### Pré-requisitos

O pacote graphit deve ser importável. Se você estiver trabalhando em um ambiente de desenvolvimento:
...
A documentação gerada mostra a API pública exatamente como os usuários a importam de `graphit.api`, sem expor a estrutura interna do módulo.
