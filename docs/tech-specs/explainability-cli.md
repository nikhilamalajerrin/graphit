...
This specification describes CLI tools for debugging and exploring explainability data in GraphIt. These tools enable users to trace how answers were derived and debug the provenance chain from edges back to source documents.

Three CLI tools:

1. **`gr-show-document-hierarchy`** - Show document → page → chunk → edge hierarchy
2. **`gr-list-explain-traces`** - List all GraphRAG sessions with questions
3. **`gr-show-explain-trace`** - Show full explainability trace for a session
...
GraphIt has two provenance systems:
...
### Tool 1: gr-show-document-hierarchy
...
| `-u/--api-url` | Gateway URL (default: `$GRAPHIT_URL`) |
| `-t/--token` | Auth token (default: `$GRAPHIT_TOKEN`) |
| `-U/--user` | User ID (default: `graphit`) |
...
| File | Purpose |
|------|---------|
| `graphit-cli/graphit/cli/show_document_hierarchy.py` | Tool 1 |
| `graphit-cli/graphit/cli/list_explain_traces.py` | Tool 2 |
| `graphit-cli/graphit/cli/show_explain_trace.py` | Tool 3 |

## Files to Modify

| File | Change |
|------|--------|
| `graphit-cli/setup.py` | Add console_scripts entries |
...
- Token authentication supported via `--token` or `$GRAPHIT_TOKEN`
...
- Existing CLI example: `graphit-cli/graphit/cli/invoke_graph_rag.py`
