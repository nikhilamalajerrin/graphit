...
This specification describes enhanced command-line configuration capabilities for GraphIt, enabling users to manage individual configuration items through granular CLI commands. The integration supports four primary use cases:
...
- **Consistent CLI Pattern**: Follow established GraphIt CLI conventions and patterns
...
GraphIt currently provides configuration management through the Config API and a single CLI command `gr-show-config` that displays the entire configuration. While this works for viewing configuration, it lacks granular management capabilities.
...
This specification addresses these gaps by adding four new CLI commands that provide granular configuration management. By exposing individual Config API operations through CLI commands, GraphIt can:
...
1. **gr-list-config-items**
   ...
   Module: `graphit.cli.list_config_items`
...
2. **gr-get-config-item**
   ...
   Module: `graphit.cli.get_config_item`
...
3. **gr-put-config-item**
   ...
   Module: `graphit.cli.put_config_item`
...
4. **gr-delete-config-item**
   ...
   Module: `graphit.cli.delete_config_item`

### Data Models

#### ConfigKey and ConfigValue

The commands utilize existing data structures from `graphit.api.types`:
...
All commands follow the established GraphIt CLI pattern:
- Use `argparse` for command-line argument parsing
- Import and use `graphit.api.Api` for backend communication
...
**Text Format (Default)**:
- `gr-list-config-items`: One key per line, plain text
- `gr-get-config-item`: Raw string value, no quotes or encoding

**JSON Format**:
- `gr-list-config-items`: Array of strings `["key1", "key2", "key3"]`
- `gr-get-config-item`: JSON-encoded string value `"actual string value"`

#### Input Handling

**gr-put-config-item** supports two mutually exclusive input methods:
...
- Existing `gr-show-config` command remains unchanged
- New commands can be added incrementally
- No breaking changes to existing configuration workflows

## Packaging and Distribution

These commands will be added to the existing `graphit-cli` package:

**Package Location**: `graphit-cli/`
**Module Files**:
...
**Entry Points**: Added to `graphit-cli/pyproject.toml` in `[project.scripts]` section:
```toml
...
1. **Create CLI Modules**: Implement the four CLI command modules in `graphit-cli/graphit/cli/`
2. **Update pyproject.toml**: Add new command entry points to `graphit-cli/pyproject.toml`
3. **Documentation**: Create CLI documentation for each command in `docs/cli/`
4. **Testing**: Implement comprehensive test coverage
5. **Integration**: Ensure commands work with existing GraphIt infrastructure
6. **Package Build**: Verify commands are properly installed with `pip install graphit-cli`
...
- Existing Config API: `graphit/api/config.py`
- CLI patterns: `graphit-cli/graphit/cli/show_config.py`
- Data types: `graphit/api/types.py`
