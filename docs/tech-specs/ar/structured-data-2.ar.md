...
تتناول هذه المواصفات المشكلات والفجوات التي تم تحديدها أثناء التنفيذ الأولي لدمج البيانات المهيكلة في GraphIt، كما هو موضح في `structured-data.md`.
...
**المرحلة 1: حساب التضمين** (`graphit-flow/graphit/embeddings/row_embeddings/`) - تستهلك `ExtractedObject`، وتحسب التضمينات عبر خدمة التضمينات، وتُخرج `RowEmbeddings`.
**المرحلة 2: تخزين التضمين** (`graphit-flow/graphit/storage/row_embeddings/qdrant/`) - تستهلك `RowEmbeddings`، وتكتب المتجهات إلى Qdrant.

كاتب صف Cassandra هو مستهلك متوازي منفصل:

**كاتب صف Cassandra** (`graphit-flow/graphit/storage/rows/cassandra`) - يستهلك `ExtractedObject`، ويكتب الصفوف إلى Cassandra.
...
|-------|--------|-------------|
| المرحلة 1 | `graphit-flow/graphit/embeddings/row_embeddings/` | `row-embeddings` |
| المرحلة 2 | `graphit-flow/graphit/storage/row_embeddings/qdrant/` | `row-embeddings-write-qdrant` |
...
الوحدة: `graphit-flow/graphit/query/row_embeddings/qdrant`
...
| المكون | الموقع |
|-----------|----------|
| الموزع | `graphit-flow/graphit/gateway/dispatch/row_embeddings_query.py` |
...
الأمر: `gr-invoke-row-embeddings`
...
|-----------|----------|-------|-------------|
| خدمة الاستعلام | `graphit-flow/graphit/query/objects/cassandra/service.py` | ~740 | كتلة واحدة: توليد مخطط GraphQL، تحليل المرشحات، استعلامات Cassandra، معالجة الطلبات |
| الكاتب | `graphit-flow/graphit/storage/objects/cassandra/write.py` | ~540 | إنشاء جدول لكل مخطط، فهارس ثانوية، إدراج/حذف |
...
**الكاتب** (`graphit-flow/graphit/storage/rows/cassandra/`):
...
**خدمة الاستعلام** (`graphit-flow/graphit/query/rows/cassandra/`):
...
|--------|---------|
| `graphit-flow/graphit/query/graphql/` | أدوات GraphQL مشتركة |
| `graphit-flow/graphit/query/row_embeddings/qdrant/` | واجهة برمجة تطبيقات استعلام تضمينات الصفوف |
| `graphit-flow/graphit/embeddings/row_embeddings/` | حساب تضمينات الصفوف (المرحلة 1) |
| `graphit-flow/graphit/storage/row_embeddings/qdrant/` | تخزين تضمينات الصفوف (المرحلة 2) |
