...
تصف هذه المواصفة أدوات سطر الأوامر لتصحيح الأخطاء واستكشاف بيانات الشرح في GraphIt. تسمح هذه الأدوات للمستخدمين بتتبع كيفية استخلاص الإجابات وتصحيح سلسلة الأصل من الحواف إلى المستندات المصدر.

ثلاث أدوات سطر أوامر:

1. **`gr-show-document-hierarchy`** - عرض تسلسل المستند → الصفحة → الجزء → الحافة
2. **`gr-list-explain-traces`** - سرد جميع جلسات GraphRAG مع الأسئلة
3. **`gr-show-explain-trace`** - عرض مسار الشرح الكامل لجلسة
...
يحتوي GraphIt على نظامين لتعقب:
...
### الأداة 1: gr-show-document-hierarchy
...
**مثال على الإخراج**:
```
المستند: urn:graphit:doc:abc123
  العنوان: "Sample PDF"
  النوع: application/pdf

  └── الصفحة 1: urn:graphit:doc:abc123/p1
      ├── الجزء 0: urn:graphit:doc:abc123/p1/c0
          المحتوى: "The quick brown fox..." [مقتطف]
      └── الجزء 1: urn:graphit:doc:abc123/p1/c1
          المحتوى: "Machine learning is..." [مقتطف]
```

### الأداة 2: gr-list-explain-traces
...
### الأداة 3: gr-show-explain-trace
...
**مثال على الإخراج**:
```
=== جلسة GraphRAG: urn:graphit:question:abc123 ===
...
- مثال سطر الأوامر: `graphit-cli/graphit/cli/invoke_graph_rag.py`
