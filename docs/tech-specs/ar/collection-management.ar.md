...
تصف هذه المواصفات إمكانات إدارة المجموعات لـ GraphIt، والتي تتطلب إنشاء مجموعات بشكل صريح وتوفر تحكمًا مباشرًا في دورة حياة المجموعة. يجب إنشاء المجموعات بشكل صريح قبل استخدامها، مما يضمن المزامنة الصحيحة بين بيانات وصفية أمين المكتبة وجميع الواجهات الخلفية للتخزين. تدعم هذه الميزة أربع حالات استخدام رئيسية:
...
في السابق، كانت المجموعات في GraphIt يتم إنشاؤها ضمنيًا أثناء عمليات تحميل البيانات، مما أدى إلى مشاكل في المزامنة حيث يمكن أن توجد المجموعات في الواجهات الخلفية للتخزين بدون بيانات وصفية مقابلة في أمين المكتبة. وقد أدى ذلك إلى تحديات إدارية وإمكانية وجود بيانات معزولة.

يعالج نموذج إنشاء المجموعة الصريح هذه المشكلات عن طريق:
طلب إنشاء المجموعات قبل استخدامها عبر `gr-set-collection`
...
تحدد هذه المواصفات نموذج إدارة المجموعة الصريح. من خلال طلب إنشاء المجموعة بشكل صريح، تضمن GraphIt:
...
سيتم تنفيذ نظام إدارة المجموعة داخل البنية التحتية الحالية لـ GraphIt:
...
1. **إنشاء المجموعة** (مساران):
   ...
   **المسار أ: الإنشاء الذي يبدأه المستخدم** عبر `gr-set-collection`:
...
4. **تحديثات البيانات الوصفية**: يمكن للمستخدمين تحديث بيانات وصفية للمجموعة بعد الإنشاء:
   قم بتحديث الاسم والوصف والعلامات عبر `gr-set-collection`
...
5. **الحذف الصريح**: يقوم المستخدمون بحذف المجموعات عبر `gr-delete-collection`:
...
العمليات المطلوبة:
**إنشاء مجموعة**: عملية المستخدم عبر `gr-set-collection` أو تلقائيًا عند إرسال مستند
...
توجد المجموعات عبر واجهات خلفية تخزين متعددة في GraphIt:
...
1. **إعادة هيكلة فئة GraphIt** (`graphit/direct/cassandra.py`):
...
2. **تحديثات كاتب التخزين** (`graphit/storage/triples/cassandra/write.py`):
   الحفاظ على اتصال GraphIt واحد لكل مستخدم بدلاً من لكل (مستخدم، مجموعة)
   تمرير المجموعة إلى عمليات الإدراج
   تحسين استخدام الموارد مع عدد أقل من الاتصالات

3. **تحديثات خدمة الاستعلام** (`graphit/query/triples/cassandra/service.py`):
   اتصال GraphIt واحد لكل مستخدم
...
تتطلب عمليات إدارة المجموعة تفويضًا مناسبًا لمنع الوصول غير المصرح به أو حذف المجموعات. سيتم مواءمة التحكم في الوصول مع نماذج الأمان الحالية لـ GraphIt.
...
1. **خدمة إدارة المجموعة Librarian** (`graphit-flow/graphit/librarian/collection_manager.py`)
...
2. **مخطط البيانات الوصفية للمجموعة** (`graphit-base/graphit/schema/services/collection.py`)
...
3. **مخطط إدارة التخزين** (`graphit-base/graphit/schema/services/storage.py`)
...
4. **مخطط Cassandra المكون من 4 جداول** (`graphit-flow/graphit/direct/cassandra_kg.py`)
