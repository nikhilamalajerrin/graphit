...
تصف هذه المواصفات دمج GraphIt مع تدفقات البيانات المهيكلة، مما يمكّن النظام من العمل مع البيانات التي يمكن تمثيلها كصفوف في الجداول أو كائنات في مخازن الكائنات. يدعم هذا التكامل أربع حالات استخدام رئيسية:
...
**واجهة وصول موحدة للبيانات**: توفير واجهة واحدة للوصول إلى كل من البيانات المهيكلة وغير المهيكلة داخل GraphIt.
**تكامل سلس**: تمكين التوافق السلس بين تمثيل المعرفة القائم على الرسم البياني في GraphIt وتنسيقات البيانات المهيكلة التقليدية.
...
عدم وجود جسر بين استعلامات تشبه SQL واستعلامات الرسم البياني في GraphIt.

تعالج هذه المواصفات هذه الفجوات من خلال تقديم طبقة بيانات مهيكلة تكمل إمكانات GraphIt الحالية. من خلال دعم البيانات المهيكلة أصلاً، يمكن لـ GraphIt:
...
1. **خدمة NLP-to-Structured-Query**
   ...
   يتكامل مع إمكانات NLP الحالية في GraphIt.
   
   الوحدة: graphit-flow/graphit/query/nlp_query/cassandra
...
3. **وحدة استخراج الكائنات** ✅ **[مكتمل]**
   ...
   يسجل معالج تكوين (مثال: graphit-flow/graphit/prompt/template/service.py) لتلقي بيانات التكوين وفك تشفير معلومات المخطط.
   يتلقى الكائنات ويفك تشفيرها إلى كائنات ExtractedObject للتسليم على قائمة الانتظار Pulsar.
   ملاحظة: يوجد رمز موجود في `graphit-flow/graphit/extract/object/row/`. كان هذا محاولة سابقة وسيتطلب إعادة هيكلة كبيرة لأنه لا يتوافق مع واجهات برمجة التطبيقات الحالية. استخدمه إذا كان مفيدًا، وابدأ من البداية إذا لم يكن كذلك.
   يتطلب واجهة سطر أوامر: `kg-extract-objects`

   الوحدة: graphit-flow/graphit/extract/kg/objects/
...
4. **وحدة كاتب المتجر المهيكل** ✅ **[مكتمل]**
   ...
   **ملاحظات التنفيذ**:
   الكود الحالي الموجود في `graphit-flow/graphit/storage/objects/cassandra/` قديم ولا يتوافق مع واجهات برمجة التطبيقات (APIs) الحالية.
   يجب الرجوع إلى `graphit-flow/graphit/storage/triples/cassandra` كمثال على معالج تخزين يعمل.
   يجب تقييم الكود الحالي بحثًا عن أي مكونات قابلة لإعادة الاستخدام قبل اتخاذ قرار بإعادة هيكلة أو إعادة كتابة.

   الوحدة: graphit-flow/graphit/storage/objects/cassandra
...
5. **خدمة الاستعلامات المهيكلة** ✅ **[مكتمل]**
   ...
   الوحدة: graphit-flow/graphit/query/objects/cassandra
...
7. **خدمة استيعاب البيانات المهيكلة**
   ...
   الوحدة: graphit-flow/graphit/decoding/structured
...
8. **خدمة تضمين الكائنات (Object Embedding Service)**
   ...
   الوحدة: graphit-flow/graphit/embeddings/object_embeddings/qdrant

### نماذج البيانات

#### آلية تخزين المخططات

يتم تخزين المخططات في نظام تكوين GraphIt باستخدام الهيكل التالي:
...
تكامل متسق مع إدارة التكوين الحالية لـ GraphIt
...
كل شيء موجود في حزم graphit-flow باستثناء عناصر المخطط
الموجودة في graphit-base.
