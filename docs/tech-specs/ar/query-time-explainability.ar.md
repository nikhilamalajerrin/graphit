...
تستخدم جميع عناوين URI مساحة الاسم `urn:graphit:` مع معرفات UUID:

| الكيان | نمط URI |
|--------|-------------|
| الجلسة | `urn:graphit:session:{uuid}` |
| الاسترداد | `urn:graphit:prov:retrieval:{uuid}` |
| التحديد | `urn:graphit:prov:selection:{uuid}` |
| الإجابة | `urn:graphit:prov:answer:{uuid}` |
| تحديد الحواف | `urn:graphit:prov:edge:{uuid}:{index}` |
...
معرفة في `graphit-base/graphit/provenance/namespaces.py`:
...
| الملف | الغرض |
|------|---------|
| `graphit-base/graphit/provenance/uris.py` | مولدات URI |
| `graphit-base/graphit/provenance/namespaces.py` | ثوابت مساحة الاسم RDF |
| `graphit-base/graphit/provenance/triples.py` | أدوات بناء الثلاثيات |
| `graphit-base/graphit/schema/services/retrieval.py` | مخطط GraphRagResponse |
| `graphit-flow/graphit/retrieval/graph_rag/graph_rag.py` | GraphRAG الأساسي مع الحفاظ على URI |
| `graphit-flow/graphit/retrieval/graph_rag/rag.py` | خدمة مع تكامل أمين المكتبة |
| `graphit-flow/graphit/query/triples/cassandra/service.py` | دعم الاستعلام عن الثلاثيات المقتبسة |
| `graphit-cli/graphit/cli/invoke_graph_rag.py` | واجهة سطر أوامر مع عرض التفسير |
