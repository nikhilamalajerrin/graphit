...
تصف هذه المواصفات خدمة جديدة قابلة للاستدعاء لتشخيص وتحليل البيانات المهيكلة داخل GraphIt. تستخرج الخدمة الوظائف من أداة سطر الأوامر `gr-load-structured-data` الحالية وتعرضها كخدمة طلب/استجابة، مما يتيح الوصول البرمجي إلى قدرات اكتشاف أنواع البيانات وإنشاء الوصف.

تدعم الخدمة ثلاث عمليات رئيسية:

1. **اكتشاف نوع البيانات**: تحليل عينة بيانات لتحديد تنسيقها (CSV أو JSON أو XML)
2. **إنشاء الوصف**: إنشاء وصف بيانات مهيكلة لـ GraphIt لعينة بيانات معينة ونوع
...
**إنشاء أوصاف دقيقة**: إنتاج أوصاف بيانات مهيكلة تعكس بدقة بيانات المصدر إلى مخططات GraphIt
...
حاليًا، يوفر الأمر `gr-load-structured-data` وظائف شاملة لتحليل البيانات المهيكلة وإنشاء الأوصاف. ومع ذلك، فإن هذه الوظائف مرتبطة ارتباطًا وثيقًا بواجهة سطر الأوامر، مما يحد من إمكانية إعادة استخدامها.
...
تعالج هذه المواصفات هذه الفجوات من خلال إنشاء خدمة مخصصة لتشخيص البيانات المهيكلة. من خلال تعريض هذه القدرات كخدمة، يمكن لـ GraphIt:
...
1. **معالج خدمة التشخيص**
   ...
   الوحدة: `graphit-flow/graphit/diagnosis/structured_data/service.py`
...
2. **كاشف نوع البيانات**
   ...
   الوحدة: `graphit-flow/graphit/diagnosis/structured_data/type_detector.py`

3. **مولد الوصف**
   يستخدم خدمة المطالبة لإنشاء الأوصاف
   يستدعي المطالبات الخاصة بالتنسيق (diagnose-csv أو diagnose-json أو diagnose-xml)
   يربط حقول البيانات بحقول مخطط GraphIt من خلال استجابات المطالبة

   الوحدة: `graphit-flow/graphit/diagnosis/structured_data/descriptor_generator.py`
...
ستتبع الخدمة اتفاقيات خدمة GraphIt:
...
ستتكامل الخدمة مع واجهات برمجة تطبيقات GraphIt الحالية:

المكونات المعدلة:
`gr-load-structured-data` CLI - تم إعادة هيكلتها لاستخدام الخدمة الجديدة لعمليات التشخيص
...
التحكم في الوصول من خلال مصادقة GraphIt الحالية.
...
`gr-load-structured-data` التنفيذ: `graphit-cli/graphit/cli/load_structured_data.py`
