...
قم بتمكين تكوينات أدوات MCP لتحديد رموز حامل اختيارية للمصادقة مع خوادم MCP المحمية. يتيح ذلك لـ GraphIt استدعاء أدوات MCP المستضافة على الخوادم التي تتطلب المصادقة بشكل آمن، دون تعديل وكيل الأداة أو واجهات استدعاء الأداة.
...
5. **مصادقة على مستوى الخدمة:** تقوم المصادقة على خدمة GraphIt، وليس على المستخدمين الفرديين.
6. **سياق أمني مشترك:** تستخدم جميع استدعاءات أداة MCP نفس الاعتماد.

### مدى ملاءمة حالة الاستخدام

**✅ حالات الاستخدام المناسبة:**
عمليات نشر GraphIt ذات المستأجر الواحد.
مصادقة من خدمة إلى خدمة (GraphIt → خادم MCP).
بيئات التطوير والاختبار.
أدوات MCP الداخلية التي يتم الوصول إليها بواسطة نظام GraphIt.
...
عملية نشر GraphIt لمنظمة واحدة حيث يستخدم جميع الموظفين نفس أداة MCP الداخلية (على سبيل المثال، البحث في قاعدة بيانات الشركة). يتطلب خادم MCP مصادقة لمنع الوصول الخارجي، ولكن جميع المستخدمين الداخليين لديهم نفس مستوى الوصول.

**مثال على سيناريو غير مناسب:**
منصة SaaS متعددة المستأجرين لـ GraphIt حيث يحتاج كل من المستأجر A والمستأجر B إلى الوصول إلى خوادم MCP الخاصة بهم المعزولة ببيانات اعتماد منفصلة. لا يدعم هذا الميزة إدارة الرموز الخاصة بالمستأجر.

### المكونات ذات الصلة
**graphit-flow/graphit/agent/mcp_tool/service.py**: خدمة استدعاء أداة MCP.
**graphit-cli/graphit/cli/set_mcp_tool.py**: أداة سطر أوامر لإنشاء/تحديث تكوينات MCP.
**graphit-cli/graphit/cli/show_mcp_tools.py**: أداة سطر أوامر لعرض تكوينات MCP.
...
3. **دعم سطر الأوامر:** يجب أن تقبل `gr-set-mcp-tool` معلمة `--auth-token` اختيارية.
4. **عرض الرمز:** يجب أن تشير `gr-show-mcp-tools` إلى متى تم تكوين رمز مميز (تم إخفاءه لأسباب أمنية).
...
1. **تخزين التكوين:** يقوم المستخدم بتشغيل `gr-set-mcp-tool --id my-tool --tool-url http://server/api --auth-token xyz123`
...
**الملف:** `graphit-flow/graphit/agent/mcp_tool/service.py`
...
**الملف**: `graphit-cli/graphit/cli/set_mcp_tool.py`
...
`-u, --api-url` (اختياري): عنوان URL لواجهة برمجة تطبيقات GraphIt
...
**الملف**: `graphit-cli/graphit/cli/show_mcp_tools.py`
...
#### المكون 4: التوثيق
**الملف**: `docs/cli/gr-set-mcp-tool.md`
...
[ ] توثيق المعامل `--auth-token` في `gr-set-mcp-tool.md`
...
| `gr-set-mcp-tool.md` | تم التعديل | متنوع | توثيق المعامل الجديد |
...
**لا يوجد سياق مستخدم**: لا يمكن لخادم MCP التمييز بين مستخدمي GraphIt المختلفين
...
يخدم توزيعة GraphIt الخاصة بك مؤسسات متعددة (متعددة المستأجرين)
...
نشر مثيلات GraphIt منفصلة لكل عميل
...
لا يمكن لخادم MCP التمييز بين مستخدمي GraphIt المختلفين
...
### وثائق المستخدم
[ ] قم بتحديث `gr-set-mcp-tool.md` باستخدام معلمة `--auth-token`
...
[خدمة أداة MCP الحالية](../graphit-flow/graphit/agent/mcp_tool/service.py)
