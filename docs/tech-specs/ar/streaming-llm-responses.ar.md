...
تصف هذه المواصفات تنفيذ دعم البث لاستجابات نماذج اللغة الكبيرة في GraphIt. يتيح البث التسليم في الوقت الفعلي للرموز (tokens) أثناء إنتاجها بواسطة نموذج اللغة الكبيرة، بدلاً من الانتظار حتى اكتمال توليد الاستجابة.
...
المخطط الحالي (`graphit-base/graphit/schema/services/llm.py`):
...
##### مخطط LLM (`graphit-base/graphit/schema/services/llm.py`)
...
##### مخطط المطالبة (`graphit-base/graphit/schema/services/prompt.py`).
...
**تحديثات لـ LlmClient** (`graphit-base/graphit/clients/llm_client.py`):
...
**تحديثات PromptClient** (`graphit-base/graphit/base/prompt_client.py`):

نمط مشابه مع المعامل `streaming` وإصدار مولد غير متزامن.

#### تغييرات في أداة سطر الأوامر

**gr-invoke-llm** (`graphit-cli/graphit/cli/invoke_llm.py`):

```
gr-invoke-llm [system] [prompt] [--no-streaming] [-u URL] [-f flow-id]
```
...
**gr-invoke-prompt** (`graphit-cli/graphit/cli/invoke_prompt.py`):

```
gr-invoke-prompt [template-id] [var=value...] [--no-streaming] [-u URL] [-f flow-id]
```

نفس النمط مثل `gr-invoke-llm`.

#### تغييرات في الفئة الأساسية لخدمة نماذج اللغة الكبيرة (LLM).

**LlmService** (`graphit-base/graphit/base/llm_service.py`):
...
**الوحدة:** `graphit-vertexai/graphit/model/text_completion/vertexai/llm.py`
...
|----------|---------|-------------------|
| OpenAI | graphit-flow | كامل (واجهة برمجة تطبيقات تدفق أصلية) |
| Claude/Anthropic | graphit-flow | كامل (واجهة برمجة تطبيقات تدفق أصلية) |
| Ollama | graphit-flow | كامل (واجهة برمجة تطبيقات تدفق أصلية) |
| Cohere | graphit-flow | كامل (واجهة برمجة تطبيقات تدفق أصلية) |
| Mistral | graphit-flow | كامل (واجهة برمجة تطبيقات تدفق أصلية) |
| Azure OpenAI | graphit-flow | كامل (واجهة برمجة تطبيقات تدفق أصلية) |
| Google AI Studio | graphit-flow | كامل (واجهة برمجة تطبيقات تدفق أصلية) |
| VertexAI | graphit-vertexai | كامل (المرحلة الثانية) |
| Bedrock | graphit-bedrock | كامل (واجهة برمجة تطبيقات تدفق أصلية) |
| LM Studio | graphit-flow | كامل (متوافق مع OpenAI) |
| LlamaFile | graphit-flow | كامل (متوافق مع OpenAI) |
| vLLM | graphit-flow | كامل (متوافق مع OpenAI) |
| TGI | graphit-flow | قيد التحديد |
| Azure | graphit-flow | قيد التحديد |
...
مخطط نموذج اللغة الكبير الحالي: `graphit-base/graphit/schema/services/llm.py`
مخطط المطالبة الحالي: `graphit-base/graphit/schema/services/prompt.py`
مخطط الوكيل الحالي: `graphit-base/graphit/schema/services/agent.py`
الخدمة الأساسية لنموذج اللغة الكبير: `graphit-base/graphit/base/llm_service.py`
مزود VertexAI: `graphit-vertexai/graphit/model/text_completion/vertexai/llm.py`
واجهة برمجة التطبيقات (API) للواجهة: `graphit-base/graphit/api/`
أدوات سطر الأوامر (CLI): `graphit-cli/graphit/cli/`
