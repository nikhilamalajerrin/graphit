...
سيوفر خدمة استعلام GraphQL واجهة مرنة وآمنة من حيث النوع للاستعلام عن البيانات المنظمة المخزنة في Cassandra. ستتكيف ديناميكيًا مع تغييرات المخطط، وتدعم الاستعلامات المعقدة بما في ذلك العلاقات بين الكائنات، وتتكامل بسلاسة مع بنية GraphIt الحالية القائمة على الرسائل.
...
**التكامل مع الطلبات والاستجابات**: الحفاظ على التوافق مع نمط الطلب/الاستجابة القائم على Pulsar في GraphIt.
**معالجة الأخطاء**: توفير تقارير أخطاء شاملة لمطابقة المخططات وأخطاء الاستعلامات ومشكلات التحقق من صحة البيانات.

## الخلفية

تقوم عملية تخزين البيانات المنظمة (graphit-flow/graphit/storage/objects/cassandra/) بكتابة الكائنات إلى جداول Cassandra بناءً على تعريفات المخطط المخزنة في نظام تكوين GraphIt. تستخدم هذه الجداول هيكل مفتاح تقسيم مركب مع مفاتيح أولية محددة في المجموعة ومحددة في المخطط، مما يتيح استعلامات فعالة داخل المجموعات.
...
توليد مخططات GraphQL ديناميكيًا من تكوين GraphIt.
...
سيتم تنفيذ خدمة استعلام GraphQL كمعالج GraphIt جديد باتباع الأنماط الراسخة:

**الموقع**: `graphit-flow/graphit/query/objects/cassandra/`
...
2. **مولد المخططات الديناميكي**:
   يحول تعريفات GraphIt RowSchema إلى أنواع GraphQL.
...
=======
This specification describes the implementation of a GraphQL query interface for GraphIt's structured data storage in Apache Cassandra. Building upon the structured data capabilities outlined in the structured-data.md specification, this document details how GraphQL queries will be executed against Cassandra tables containing extracted and ingested structured objects.

The GraphQL query service will provide a flexible, type-safe interface for querying structured data stored in Cassandra. It will dynamically adapt to schema changes, support complex queries including relationships between objects, and integrate seamlessly with GraphIt's existing message-based architecture.
...
**Request/Response Integration**: Maintain compatibility with GraphIt's Pulsar-based request/response pattern
**Error Handling**: Provide comprehensive error reporting for schema mismatches, query errors, and data validation issues

## Background

The structured data storage implementation (graphit-flow/graphit/storage/objects/cassandra/) writes objects to Cassandra tables based on schema definitions stored in GraphIt's configuration system. These tables use a composite partition key structure with collection and schema-defined primary keys, enabling efficient queries within collections.
...
Dynamically generating GraphQL schemas from GraphIt configuration
...
The GraphQL query service will be implemented as a new GraphIt flow processor following established patterns:

**Module Location**: `graphit-flow/graphit/query/objects/cassandra/`
...
2. **Dynamic Schema Generator**
   Converts GraphIt RowSchema definitions to GraphQL types
...
**GraphIt Base**: لـ FlowProcessor وتعريفات المخططات.
...
وثائق معالج التدفق GraphIt: وثائق داخلية
