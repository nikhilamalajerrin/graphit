...
The canonical vocabulary for all predicates and types is published as an OWL ontology at `specs/ontology/graphit.ttl`.

## Entity Types

All services use PROV-O as the base ontology with GraphIt-specific subtypes.
...
|---------|-------------|-----------|
| `tg:GraphRagQuestion` | `urn:graphit:question:{uuid}` | Knowledge graph RAG |
| `tg:DocRagQuestion` | `urn:graphit:docrag:{uuid}` | Document/chunk RAG |
| `tg:AgentQuestion` | `urn:graphit:agent:session:{uuid}` | Agent orchestrator |
...
gr-invoke-agent -q "What is quantum computing?" -x

# Run with token usage
gr-invoke-agent -q "What is quantum computing?" --show-usage

# GraphRAG with explainability
gr-invoke-graph-rag -q "Tell me about AI" -x

# Document RAG with explainability
gr-invoke-document-rag -q "Summarize the findings" -x
```
