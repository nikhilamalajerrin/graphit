...
This specification describes the collection management capabilities for GraphIt, requiring explicit collection creation and providing direct control over the collection lifecycle. Collections must be explicitly created before use, ensuring proper synchronization between the librarian metadata and all storage backends. The feature supports four primary use cases:
...
Previously, collections in GraphIt were implicitly created during data loading operations, leading to synchronization issues where collections could exist in storage backends without corresponding metadata in the librarian. This created management challenges and potential orphaned data.

The explicit collection creation model addresses these issues by:
- Requiring collections to be created before use via `gr-set-collection`
...
This specification defines the explicit collection management model. By requiring explicit collection creation, GraphIt ensures:
...
The collection management system will be implemented within existing GraphIt infrastructure:
...
1. **Collection Creation** (Two Paths):
   ...
   **Path A: User-Initiated Creation** via `gr-set-collection`:
...
4. **Metadata Updates**: Users can update collection metadata after creation:
   - Update name, description, and tags via `gr-set-collection`
   - Updates apply to librarian metadata only
   - Storage backends maintain collection but metadata updates don't propagate

5. **Explicit Deletion**: Users delete collections via `gr-delete-collection`:
...
Operations required:
- **Create Collection**: User operation via `gr-set-collection` OR automatic on document submission
...
Collections exist across multiple storage backends in GraphIt:
...
- **Create/Update Collection**: Create new collection or update existing metadata via `gr-set-collection`
...
The implementation will follow existing GraphIt patterns for service integration and CLI command structure.
...
1. **GraphIt Class Refactor** (`graphit/direct/cassandra.py`):
...
2. **Storage Writer Updates** (`graphit/storage/triples/cassandra/write.py`):
   - Maintain single GraphIt connection per user instead of per (user, collection)
   - Pass collection to insert operations
   - Improved resource utilization with fewer connections

3. **Query Service Updates** (`graphit/query/triples/cassandra/service.py`):
   - Single GraphIt connection per user
...
Collection management operations require appropriate authorization to prevent unauthorized access or deletion of collections. Access control will align with existing GraphIt security models.
...
1. **Librarian Collection Management Service** (`graphit-flow/graphit/librarian/collection_manager.py`)
...
2. **Collection Metadata Schema** (`graphit-base/graphit/schema/services/collection.py`)
...
3. **Storage Management Schema** (`graphit-base/graphit/schema/services/storage.py`)
...
4. **Cassandra 4-Table Schema** (`graphit-flow/graphit/direct/cassandra_kg.py`)
