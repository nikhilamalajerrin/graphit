...
Unlike other storage backends in GraphIt:
...
The property-based filtering approach provides the best balance of security, performance, and maintainability for adding user/collection isolation to Neo4j. It aligns with GraphIt's existing multi-tenancy patterns while leveraging Neo4j's strengths in graph querying and indexing.

This solution ensures GraphIt's Neo4j backend meets the same security standards as other storage backends, preventing data isolation vulnerabilities while maintaining the flexibility and power of graph queries.
