...
Эта спецификация описывает расширенные возможности конфигурации через командную строку для GraphIt, позволяя пользователям управлять отдельными элементами конфигурации с помощью команд CLI с гранулярным уровнем доступа. Интеграция поддерживает четыре основных сценария использования:
...
- **Единый шаблон CLI**: Соблюдение установленных норм и шаблонов CLI для GraphIt
...
GraphIt в настоящее время предоставляет управление конфигурацией через API для конфигурации и один командный интерфейс `gr-show-config`, который отображает всю конфигурацию. Хотя это работает для просмотра конфигурации, оно не обеспечивает возможности детального управления.
...
Эта спецификация решает эти проблемы, добавляя четыре новые команды CLI, которые обеспечивают детальное управление конфигурацией. Предоставляя отдельные операции API через команды CLI, GraphIt может:
...
1. **gr-list-config-items**
   ...
   Модуль: `graphit.cli.list_config_items`
...
2. **gr-get-config-item**
   ...
   Модуль: `graphit.cli.get_config_item`
...
3. **gr-put-config-item**
   ...
   Модуль: `graphit.cli.put_config_item`
...
4. **gr-delete-config-item**
   ...
   Модуль: `graphit.cli.delete_config_item`

### Модели данных

#### ConfigKey и ConfigValue

Команды используют существующие структуры данных из `graphit.api.types`:
...
Все команды следуют установленной схеме CLI для GraphIt:
- Использование `argparse` для разбора командной строки
- Импорт и использование `graphit.api.Api` для взаимодействия с backend
...
- Существующий API для конфигурации: `graphit/api/config.py`
- Шаблоны CLI: `graphit-cli/graphit/cli/show_config.py`
- Типы данных: `graphit/api/types.py`
