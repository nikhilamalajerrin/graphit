...
больших документов в GraphIt. Текущая архитектура рассматривает загрузку документов
...
Основные файлы:
Точка входа REST/WebSocket: `graphit-flow/graphit/gateway/service.py`
Основной модуль Librarian: `graphit-flow/graphit/librarian/librarian.py`
Хранилище объектов: `graphit-flow/graphit/librarian/blob_store.py`
Таблицы Cassandra: `graphit-flow/graphit/tables/library.py`
Схема API: `graphit-base/graphit/schema/services/library.py`
...
**Файл:** `graphit-flow/graphit/librarian/blob_store.py`
...
Python SDK (`graphit-base/graphit/api/library.py`) должен обрабатывать
...
**`gr-add-library-document`** продолжает работать без изменений:
...
`gr-load-pdf` - устарело, используйте `gr-add-library-document`
`gr-load-text` - устарело, используйте `gr-add-library-document`
