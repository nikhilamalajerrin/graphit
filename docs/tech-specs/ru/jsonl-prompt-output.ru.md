...
Эта спецификация описывает реализацию формата вывода JSONL (JSON Lines) для ответов на запросы в GraphIt. JSONL обеспечивает устойчивость к усечению при извлечении структурированных данных из ответов LLM, решая критические проблемы, связанные с повреждением выходных массивов JSON, когда ответы LLM достигают лимита токенов.
...
Текущая реализация в `graphit-flow/graphit/template/prompt_manager.py`:
...
Текущая реализация: `graphit-flow/graphit/template/prompt_manager.py`
