...
Данное техническое описание посвящено неконсистентности именования и шаблонов конфигурации параметров подключения к Cassandra в кодовой базе GraphIt. В настоящее время существуют две различные схемы именования параметров (`cassandra_*` против `graph_*`), что приводит к путанице и усложняет обслуживание.
...
`graphit-flow/graphit/storage/rows/cassandra/write.py`
`graphit-flow/graphit/query/triples/cassandra/service.py`

### Модули, использующие параметры `cassandra_*` (которые необходимо обновить с использованием резервного варианта из среды):
`graphit-flow/graphit/tables/config.py`
`graphit-flow/graphit/tables/knowledge.py`
`graphit-flow/graphit/tables/library.py`
`graphit-flow/graphit/storage/knowledge/store.py`
`graphit-flow/graphit/cores/knowledge.py`
`graphit-flow/graphit/librarian/librarian.py`
