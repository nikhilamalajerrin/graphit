...
На основе спецификации `STRUCTURED_DATA.md`, этот документ предлагает необходимые дополнения и изменения схемы Pulsar для поддержки возможностей работы со структурированными данными в GraphIt.
...
- `graphit-flow/graphit/decoding/structured` - Использует StructuredDataSubmission
- `graphit-flow/graphit/query/nlp_query/cassandra` - Использует схемы запросов NLP
- `graphit-flow/graphit/query/objects/cassandra` - Использует схемы структурированных запросов
- `graphit-flow/graphit/extract/object/row/` - Потребляет Chunk, производит ExtractedObject
- `graphit-flow/graphit/storage/objects/cassandra` - Использует схему Rows
- `graphit-flow/graphit/embeddings/object_embeddings/qdrant` - Использует схемы встраивания объектов
