...
Позволяет конфигурациям инструментов MCP указывать необязательные токены Bearer для аутентификации с защищенными серверами MCP. Это позволяет GraphIt безопасно вызывать инструменты MCP, размещенные на серверах, требующих аутентификации, без изменения интерфейсов агента или вызова инструментов.
...
5. **Аутентификация на уровне сервиса:** Аутентифицируется сервис GraphIt, а не отдельные пользователи.
...
Развертывания GraphIt для одного арендатора.
Аутентификация от сервиса к сервису (GraphIt → MCP Server).
Среды разработки и тестирования.
Внутренние инструменты MCP, доступные системе GraphIt.
...
Развертывание GraphIt для одной организации, в котором все сотрудники используют один и тот же внутренний инструмент MCP (например, поиск информации в корпоративной базе данных). Сервер MCP требует аутентификации для предотвращения внешнего доступа, но все внутренние пользователи имеют одинаковый уровень доступа.

**Пример неподходящего сценария:**
Многопользовательская SaaS-платформа GraphIt, в которой арендатор A и арендатор B должны получать доступ к своим собственным изолированным серверам MCP с отдельными учетными данными. Эта функция НЕ поддерживает управление токенами на уровне арендатора.

### Связанные компоненты
**graphit-flow/graphit/agent/mcp_tool/service.py**: Сервис вызова инструмента MCP.
**graphit-cli/graphit/cli/set_mcp_tool.py**: Инструмент командной строки для создания/обновления конфигураций MCP.
**graphit-cli/graphit/cli/show_mcp_tools.py**: Инструмент командной строки для отображения конфигураций MCP.
...
3. **Поддержка CLI:** `gr-set-mcp-tool` ДОЛЖЕН принимать необязательный параметр `--auth-token`.
4. **Отображение токена:** `gr-show-mcp-tools` ДОЛЖЕН указывать, когда настроена аутентификация (маскируется для безопасности).
...
1. **Хранение конфигурации**: Пользователь запускает `gr-set-mcp-tool --id my-tool --tool-url http://server/api --auth-token xyz123`.
...
**Файл**: `graphit-flow/graphit/agent/mcp_tool/service.py`.
...
**Файл**: `graphit-cli/graphit/cli/set_mcp_tool.py`
...
`-u, --api-url` (необязательный): URL-адрес API GraphIt.
...
**Файл**: `graphit-cli/graphit/cli/show_mcp_tools.py`
...
**Файл**: `docs/cli/gr-set-mcp-tool.md`
...
[ ] Документировать параметр `--auth-token` в `gr-set-mcp-tool.md`
...
| `gr-set-mcp-tool.md` | Изменено | Различные | Документирован новый параметр |
...
**Отсутствие контекста пользователя**: Сервер MCP не может различать разных пользователей GraphIt
...
Ваша развертка GraphIt обслуживает несколько организаций (многопользовательская среда)
...
Развернуть отдельные экземпляры GraphIt для каждого арендатора
...
Сервер MCP не может различать разных пользователей GraphIt
...
[ ] Обновить `gr-set-mcp-tool.md` с параметром `--auth-token`
...
[Текущий сервис инструмента MCP](../graphit-flow/graphit/agent/mcp_tool/service.py)
