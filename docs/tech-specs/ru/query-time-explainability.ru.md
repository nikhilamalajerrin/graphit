...
Все URI используют пространство имен `urn:graphit:` с UUID:

| Сущность | Шаблон URI |
|--------|-------------|
| Сессия | `urn:graphit:session:{uuid}` |
| Получение | `urn:graphit:prov:retrieval:{uuid}` |
| Выбор | `urn:graphit:prov:selection:{uuid}` |
| Ответ | `urn:graphit:prov:answer:{uuid}` |
| Выбор ребра | `urn:graphit:prov:edge:{uuid}:{index}` |
...
Определены в `graphit-base/graphit/provenance/namespaces.py`:
...
| Файл | Назначение |
|------|---------|
| `graphit-base/graphit/provenance/uris.py` | Генераторы URI |
| `graphit-base/graphit/provenance/namespaces.py` | Константы пространства имен RDF |
| `graphit-base/graphit/provenance/triples.py` | Конструкторы троек |
| `graphit-base/graphit/schema/services/retrieval.py` | Схема GraphRagResponse |
| `graphit-flow/graphit/retrieval/graph_rag/graph_rag.py` | Основной GraphRAG с сохранением URI |
| `graphit-flow/graphit/retrieval/graph_rag/rag.py` | Сервис с интеграцией с библиотекарем |
| `graphit-flow/graphit/query/triples/cassandra/service.py` | Поддержка запросов троек в кавычках |
| `graphit-cli/graphit/cli/invoke_graph_rag.py` | CLI с отображением объяснений |
