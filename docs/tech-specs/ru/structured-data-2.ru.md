...
Эта спецификация рассматривает проблемы и недостатки, выявленные в процессе первоначальной реализации интеграции структурированных данных GraphIt, как описано в `structured-data.md`.
...
**Этап 1: Вычисление векторных представлений** (`graphit-flow/graphit/embeddings/row_embeddings/`) - Использует `ExtractedObject`, вычисляет векторные представления через сервис векторных представлений, выдает `RowEmbeddings`.
**Этап 2: Хранение векторных представлений** (`graphit-flow/graphit/storage/row_embeddings/qdrant/`) - Использует `RowEmbeddings`, записывает векторы в Qdrant.

Модуль записи строк в Cassandra является отдельным параллельным компонентом:

**Модуль записи строк в Cassandra** (`graphit-flow/graphit/storage/rows/cassandra`) - Использует `ExtractedObject`, записывает строки в Cassandra.
...
|-------|--------|-------------|
| Этап 1 | `graphit-flow/graphit/embeddings/row_embeddings/` | `row-embeddings` |
| Этап 2 | `graphit-flow/graphit/storage/row_embeddings/qdrant/` | `row-embeddings-write-qdrant` |
...
Модуль: `graphit-flow/graphit/query/row_embeddings/qdrant`
...
|-----------|----------|
| Диспетчер | `graphit-flow/graphit/gateway/dispatch/row_embeddings_query.py` |
...
Команда: `gr-invoke-row-embeddings`
...
|-----------|----------|-------|-------------|
| Сервис запросов | `graphit-flow/graphit/query/objects/cassandra/service.py` | ~740 | Монолит: генерация схемы GraphQL, разбор фильтров, запросы Cassandra, обработка запросов |
| Записывающий модуль | `graphit-flow/graphit/storage/objects/cassandra/write.py` | ~540 | Создание таблиц для каждой схемы, вторичные индексы, вставка/удаление |
...
**Записывающий модуль** (`graphit-flow/graphit/storage/rows/cassandra/`):
...
**Сервис запросов** (`graphit-flow/graphit/query/rows/cassandra/`):
...
| Модуль | Назначение |
|--------|---------|
| `graphit-flow/graphit/query/graphql/` | Общие утилиты GraphQL |
| `graphit-flow/graphit/query/row_embeddings/qdrant/` | API запросов для векторных представлений строк |
| `graphit-flow/graphit/embeddings/row_embeddings/` | Вычисление векторных представлений строк (Этап 1) |
| `graphit-flow/graphit/storage/row_embeddings/qdrant/` | Хранение векторных представлений строк (Этап 2) |
