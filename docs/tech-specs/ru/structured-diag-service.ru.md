...
Эта спецификация описывает новый сервис, который можно вызывать для диагностики и анализа структурированных данных в GraphIt. Сервис извлекает функциональность из существующего инструмента командной строки `gr-load-structured-data` и предоставляет ее в виде сервиса запросов/ответов, обеспечивая программный доступ к возможностям определения типа данных и генерации описаний.
...
2. **Генерация описания**: Генерация структурированного описания данных GraphIt для заданного образца данных и типа.
...
**Генерация точных описаний**: Создание структурированных описаний данных, которые точно сопоставляют исходные данные со схемами GraphIt.
...
В настоящее время команда `gr-load-structured-data` предоставляет комплексную функциональность для анализа структурированных данных и генерации описаний. Однако эта функциональность тесно связана с интерфейсом командной строки, что ограничивает ее повторное использование.
...
Эта спецификация решает эти проблемы, создавая специальный сервис для диагностики структурированных данных. Предоставляя эти возможности в виде сервиса, GraphIt может:
...
1. **Процессор сервиса диагностики**
   ...
   Модуль: `graphit-flow/graphit/diagnosis/structured_data/service.py`
...
2. **Детектор типа данных**
   ...
   Модуль: `graphit-flow/graphit/diagnosis/structured_data/type_detector.py`
...
3. **Генератор описаний**
   ...
   Отображает поля данных на поля схемы GraphIt с помощью ответов запросов.

   Модуль: `graphit-flow/graphit/diagnosis/structured_data/descriptor_generator.py`
...
Сервис будет соответствовать соглашениям сервиса GraphIt:
...
Сервис будет интегрирован с существующими API GraphIt:

Измененные компоненты:
`gr-load-structured-data` CLI - Переработан для использования нового сервиса для операций диагностики
...
Контроль доступа через существующую аутентификацию GraphIt
...
`gr-load-structured-data` реализация: `graphit-cli/graphit/cli/load_structured_data.py`
