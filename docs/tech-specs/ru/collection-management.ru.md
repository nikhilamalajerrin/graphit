...
Эта спецификация описывает возможности управления коллекциями для GraphIt, требуя явного создания коллекций и обеспечивая прямой контроль над жизненным циклом коллекций. Коллекции должны быть явно созданы перед использованием, что обеспечивает надлежащую синхронизацию между метаданными библиотекаря и всеми хранилищами. Эта функция поддерживает четыре основных сценария использования:
...
Ранее коллекции в GraphIt создавались неявно во время операций загрузки данных, что приводило к проблемам синхронизации, когда коллекции могли существовать в хранилищах без соответствующих метаданных в библиотекаре. Это создавало проблемы управления и потенциальные "сиротские" данные.

Модель явного создания коллекций решает эти проблемы следующим образом:
Требует создания коллекций перед использованием через `gr-set-collection`.
...
Эта спецификация определяет модель явного управления коллекциями. Требуя явного создания коллекций, GraphIt обеспечивает:
...
Система управления коллекциями будет реализована в существующей инфраструктуре GraphIt:
...
1. **Создание коллекции** (Два пути):
   ...
   **Путь A: Создание, инициированное пользователем** через `gr-set-collection`:
...
4. **Обновление метаданных**: Пользователи могут обновлять метаданные коллекции после ее создания:
   Обновление имени, описания и тегов через `gr-set-collection`
...
5. **Явное удаление**: Пользователи удаляют коллекции через `gr-delete-collection`:
...
Требуемые операции:
**Создание коллекции**: Операция пользователя через `gr-set-collection` ИЛИ автоматическое создание при отправке документа
...
Коллекции существуют в нескольких хранилищах данных в GraphIt:
...
**Создание/Обновление коллекции**: Создание новой коллекции или обновление существующих метаданных через `gr-set-collection`.
...
Реализация будет следовать существующим шаблонам GraphIt для интеграции служб и структуры командной строки.
...
1. **Рефакторинг класса GraphIt** (`graphit/direct/cassandra.py`):
...
2. **Обновления модуля записи данных** (`graphit/storage/triples/cassandra/write.py`):
   Поддерживать одно соединение GraphIt на пользователя вместо одного соединения на (пользователь, коллекция)
   Передавать коллекцию в операции вставки
   Улучшенное использование ресурсов за счет меньшего количества соединений

3. **Обновления сервиса запросов** (`graphit/query/triples/cassandra/service.py`):
   Одно соединение GraphIt на пользователя
...
Управление коллекциями требует соответствующей авторизации для предотвращения несанкционированного доступа или удаления коллекций. Контроль доступа будет соответствовать существующим моделям безопасности GraphIt.
...
1. **Сервис управления коллекциями Librarian** (`graphit-flow/graphit/librarian/collection_manager.py`)
...
2. **Схема метаданных коллекции** (`graphit-base/graphit/schema/services/collection.py`)
...
3. **Схема управления хранилищем** (`graphit-base/graphit/schema/services/storage.py`)
...
4. **Схема Cassandra для 4 таблиц** (`graphit-flow/graphit/direct/cassandra_kg.py`)
