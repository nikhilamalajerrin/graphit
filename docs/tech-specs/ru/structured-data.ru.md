...
Эта спецификация описывает интеграцию GraphIt с потоками структурированных данных, что позволяет системе работать с данными, которые могут быть представлены в виде строк в таблицах или объектов в хранилищах объектов. Интеграция поддерживает четыре основных сценария использования:
...
**Унифицированный доступ к данным**: Предоставление единого интерфейса для доступа как к структурированным, так и к неструктурированным данным в GraphIt.
**Бесшовная интеграция**: Обеспечение плавного взаимодействия между графовым представлением знаний GraphIt и традиционными форматами структурированных данных.
...
Отсутствие моста между запросами, похожими на SQL, и графовыми запросами GraphIt.

Эта спецификация решает эти проблемы, вводя слой структурированных данных, который дополняет существующие возможности GraphIt. Поддерживая структурированные данные нативно, GraphIt может:
...
1. **Сервис преобразования естественного языка в структурированные запросы**
   ...
   Интегрируется с существующими возможностями NLP GraphIt.
   
   Модуль: graphit-flow/graphit/query/nlp_query/cassandra
...
3. **Модуль извлечения объектов** ✅ **[ПОЛНО]**
   ...
   Регистрирует обработчик конфигурации (например: graphit-flow/graphit/prompt/template/service.py) для получения данных конфигурации и декодирования информации о схеме.
   Получает объекты и декодирует их в объекты ExtractedObject для передачи в очередь Pulsar.
   ВАЖНО: Существует существующий код по адресу `graphit-flow/graphit/extract/object/row/`. Это была предыдущая попытка, и его потребуется серьезно переработать, поскольку он не соответствует текущим API. Используйте его, если это полезно, начните с нуля, если нет.
   Требуется интерфейс командной строки: `kg-extract-objects`

   Модуль: graphit-flow/graphit/extract/kg/objects/
...
4. **Модуль записи структурированных данных** ✅ **[ПОЛНО]**
   ...
   **Примечания к реализации**:
   Существующий код, расположенный по адресу `graphit-flow/graphit/storage/objects/cassandra/`, устарел и не соответствует текущим API.
   Следует использовать `graphit-flow/graphit/storage/triples/cassandra` в качестве примера работающего процессора хранилища.
   ...
   Модуль: graphit-flow/graphit/storage/objects/cassandra
...
5. **Сервис структурированных запросов** ✅ **[ЗАВЕРШЕНО]**
   ...
   Модуль: graphit-flow/graphit/query/objects/cassandra
...
7. **Сервис приема структурированных данных**:
   ...
   Модуль: graphit-flow/graphit/decoding/structured
...
8. **Сервис встраивания объектов**:
   ...
   Модуль: graphit-flow/graphit/embeddings/object_embeddings/qdrant

### Модели данных

#### Механизм хранения схем

Схемы хранятся в системе конфигурации GraphIt со следующей структурой:
...
Последовательная интеграция с существующим управлением конфигурацией GraphIt
...
Все находится в пакетах graphit-flow, за исключением элементов схемы,
находящихся в graphit-base.
