...
- **Консистентный UX потоковой передачи**: Обеспечить одинаковый опыт потоковой передачи для всех сервисов GraphIt.
...
- Несогласованный опыт по сравнению с другими сервисами GraphIt.

Эта спецификация решает эти проблемы, добавляя поддержку потоковой передачи для GraphRAG и DocumentRAG.  Благодаря потоковой передаче по частям, GraphIt может:
...
1. **PromptClient Streaming** (Уже реализовано)
   ...
   Модуль: `graphit-base/graphit/base/prompt_client.py`
...
2. **Сервис GraphRAG** (Требуется передача параметра `streaming`)
   ...
   - `graphit-flow/graphit/retrieval/graph_rag/rag.py` (Обработчик)
   - `graphit-base/graphit/schema/graph_rag.py` (Схема запроса)
...
3. **Сервис DocumentRAG** (Требуется передача параметра `streaming`)
   ...
   - `graphit-flow/graphit/retrieval/document_rag/rag.py` (Обработчик)
   - `graphit-base/graphit/schema/document_rag.py` (Схема запроса)
...
- Потоковая передача LLM: `graphit-flow/graphit/agent/react/agent_manager.py`
- Потоковая передача PromptClient: `graphit-base/graphit/base/prompt_client.py`
