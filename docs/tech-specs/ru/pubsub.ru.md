...
Этот документ содержит описание всех соединений между кодовой базой GraphIt и инфраструктурой pub/sub. В настоящее время система жестко запрограммирована на использование Apache Pulsar. Этот анализ определяет все точки интеграции для будущей рефакторизации в сторону конфигурируемой абстракции pub/sub.

## Текущее состояние: точки интеграции Pulsar

### 1. Прямое использование клиента Pulsar

**Местоположение:** `graphit-flow/graphit/gateway/service.py`
...
**Местоположение:** `graphit-base/graphit/base/async_processor.py`
...
**Местоположение:** `graphit-base/graphit/base/consumer.py`
...
**Спецификационный файл:** `graphit-base/graphit/base/consumer_spec.py`
**Строка 22:** Ссылка на `processor.pulsar_client`

### 4. Абстракция производителя

**Местоположение:** `graphit-base/graphit/base/producer.py`
...
**Спецификационный файл:** `graphit-base/graphit/base/producer_spec.py`
**Строка 18:** Ссылка на `processor.pulsar_client`

### 5. Абстракция издателя

**Местоположение:** `graphit-base/graphit/base/publisher.py`
...
**Местоположение:** `graphit-base/graphit/base/subscriber.py`
...
**Файл спецификации:** `graphit-base/graphit/base/subscriber_spec.py`
**Строка 19:** Ссылается на `processor.pulsar_client`

### 7. Система схем (Heart of Darkness)

**Расположение:** `graphit-base/graphit/schema/`
...
Создать **систему схем GraphIt**, которая является агностической к pub/sub (используя dataclasses или Pydantic), а затем генерировать схемы Pulsar/Kafka/и т.д. из нее. Это требует переписывания каждого файла схемы и может привести к несовместимым изменениям.
...
1. **Создайте версии классов данных** для всех схем в `graphit/schema/`
