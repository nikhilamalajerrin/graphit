...
מפרט זה מתאר כלי שורת פקודה (CLI) לניפוי באגים ובדיקת נתוני הסברתיות ב-GraphIt. כלים אלה מאפשרים למשתמשים לעקוב אחר אופן קבלת התשובות ולבצע ניפוי באגים בשרשרת המקור, החל מצמתים ועד למסמכים מקוריים.

שלושה כלים של שורת פקודה:

1. **`gr-show-document-hierarchy`** - הצגת היררכיה של מסמך → עמוד → מקטע → צומת
2. **`gr-list-explain-traces`** - הצגת רשימה של כל הסשנים של GraphRAG עם שאלות
3. **`gr-show-explain-trace`** - הצגת מעקב הסברתיות מלא עבור סשן
...
ל-GraphIt יש שתי מערכות מעקב מקור:
...
### כלי 1: gr-show-document-hierarchy
...
| `-u/--api-url` | כתובת URL של ה-Gateway (ברירת מחדל: `$GRAPHIT_URL`) |
| `-t/--token` | טוקן אימות (ברירת מחדל: `$GRAPHIT_TOKEN`) |
| `-U/--user` | מזהה משתמש (ברירת מחדל: `graphit`) |
...
| קובץ | מטרה |
|------|---------|
| `graphit-cli/graphit/cli/show_document_hierarchy.py` | כלי 1 |
| `graphit-cli/graphit/cli/list_explain_traces.py` | כלי 2 |
| `graphit-cli/graphit/cli/show_explain_trace.py` | כלי 3 |

## קבצים לשינוי

| קובץ | שינוי |
|------|--------|
| `graphit-cli/setup.py` | הוספת רשומות ל-console_scripts |
...
אימות טוקן נתמך באמצעות `--token` או `$GRAPHIT_TOKEN`
...
דוגמה קיימת של שורת פקודה: `graphit-cli/graphit/cli/invoke_graph_rag.py`
