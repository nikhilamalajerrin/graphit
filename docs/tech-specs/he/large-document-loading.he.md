...
מסמכים גדולים ל-GraphIt. הארכיטקטורה הנוכחית מתייחסת להעלאת מסמכים
...
קבצים מרכזיים:
נקודת כניסה REST/WebSocket: `graphit-flow/graphit/gateway/service.py`
ליבה של Librarian: `graphit-flow/graphit/librarian/librarian.py`
אחסון Blob: `graphit-flow/graphit/librarian/blob_store.py`
טבלאות Cassandra: `graphit-flow/graphit/tables/library.py`
סכימת API: `graphit-base/graphit/schema/services/library.py`
...
**קובץ:** `graphit-flow/graphit/librarian/blob_store.py`
...
ערכת הפיתוח תוכנה (`graphit-base/graphit/api/library.py`) עבור Python צריכה לטפל
...
**`gr-add-library-document`** ממשיך לעבוד ללא שינוי:
...
`gr-load-pdf` - מיושן, השתמשו ב-`gr-add-library-document`
`gr-load-text` - מיושן, השתמשו ב-`gr-add-library-document`
