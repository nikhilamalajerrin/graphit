...
ב-GraphIt. סטרימינג מאפשר העברה בזמן אמת של טוקנים שנוצרו
...
סכימה נוכחית (`graphit-base/graphit/schema/services/llm.py`):
...
##### סכימת LLM (`graphit-base/graphit/schema/services/llm.py`)
...
##### סכימת הפרומפט (`graphit-base/graphit/schema/services/prompt.py`)
...
**עדכונים ל-LlmClient** (`graphit-base/graphit/clients/llm_client.py`):
...
**עדכונים עבור PromptClient** (`graphit-base/graphit/base/prompt_client.py`):
...
**מודול:** `graphit-vertexai/graphit/model/text_completion/vertexai/llm.py`
...
|----------|---------|-------------------|
| OpenAI | graphit-flow | מלא (ממשק סטרימינג מקורי) |
| Claude/Anthropic | graphit-flow | מלא (ממשק סטרימינג מקורי) |
| Ollama | graphit-flow | מלא (ממשק סטרימינג מקורי) |
| Cohere | graphit-flow | מלא (ממשק סטרימינג מקורי) |
| Mistral | graphit-flow | מלא (ממשק סטרימינג מקורי) |
| Azure OpenAI | graphit-flow | מלא (ממשק סטרימינג מקורי) |
| Google AI Studio | graphit-flow | מלא (ממשק סטרימינג מקורי) |
| VertexAI | graphit-vertexai | מלא (שלב 2) |
| Bedrock | graphit-bedrock | מלא (ממשק סטרימינג מקורי) |
| LM Studio | graphit-flow | מלא (תואם ל-OpenAI) |
| LlamaFile | graphit-flow | מלא (תואם ל-OpenAI) |
| vLLM | graphit-flow | מלא (תואם ל-OpenAI) |
| TGI | graphit-flow | מצומצם |
| Azure | graphit-flow | מצומצם |
...
סכימת מודל שפה גדול (LLM) נוכחית: `graphit-base/graphit/schema/services/llm.py`
סכימת הנחיה נוכחית: `graphit-base/graphit/schema/services/prompt.py`
סכימת סוכן נוכחית: `graphit-base/graphit/schema/services/agent.py`
שירות LLM בסיסי: `graphit-base/graphit/base/llm_service.py`
ספק VertexAI: `graphit-vertexai/graphit/model/text_completion/vertexai/llm.py`
ממשק API של שער: `graphit-base/graphit/api/`
כלי שורת פקודה: `graphit-cli/graphit/cli/`
