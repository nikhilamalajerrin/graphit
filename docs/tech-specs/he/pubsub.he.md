...
מסמך זה מפרט את כל החיבורים בין בסיס הקוד של GraphIt לתשתית ה-pub/sub. כיום, המערכת מקודדת בצורה קבועה לשימוש ב-Apache Pulsar. ניתוח זה מזהה את כל נקודות האינטגרציה כדי ליידע שינויים עתידיים לכיוון הפשטה של pub/sub הניתנת לתצורה.

## מצב נוכחי: נקודות אינטגרציה של Pulsar

### 1. שימוש ישיר בלקוח Pulsar

**מיקום:** `graphit-flow/graphit/gateway/service.py`
...
**מיקום:** `graphit-base/graphit/base/async_processor.py`
...
**מיקום:** `graphit-base/graphit/base/consumer.py`
...
**קובץ מפרט:** `graphit-base/graphit/base/consumer_spec.py`
**שורה 22:** מפנה ל-`processor.pulsar_client`

### 4. הפשטת מפרסם

**מיקום:** `graphit-base/graphit/base/producer.py`
...
**קובץ מפרט:** `graphit-base/graphit/base/producer_spec.py`
**שורה 18:** מפנה ל-`processor.pulsar_client`

### 5. הפשטת מפרסם

**מיקום:** `graphit-base/graphit/base/publisher.py`
...
**מיקום:** `graphit-base/graphit/base/subscriber.py`
...
**קובץ מפרט:** `graphit-base/graphit/base/subscriber_spec.py`
**שורה 19:** מפנה ל-`processor.pulsar_client`

### 7. מערכת סכימות (Heart of Darkness)

**מיקום:** `graphit-base/graphit/schema/`
...
ליצור **מערכת סכימות GraphIt** שאינה תלויה ב-pub/sub ספציפי (תוך שימוש ב-dataclasses או Pydantic), ולאחר מכן ליצור סכימות Pulsar/Kafka/וכו מתוך זה. זה דורש כתיבה מחדש של כל קובץ סכימה ועלול לגרום לשינויים משמעותיים.
...
1. **צור גרסאות של מחלקות נתונים** עבור כל הסכימות ב-`graphit/schema/`
