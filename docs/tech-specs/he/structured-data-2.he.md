...
מפרט זה עוסק בנושאים ופערים שזוהו במהלך יישום ראשוני של שילוב נתונים מובנים של GraphIt, כפי שמתואר ב-`structured-data.md`.
...
**שלב 1: חישוב הטמעה** (`graphit-flow/graphit/embeddings/row_embeddings/`) - צורך `ExtractedObject`, מחשב הטמעות באמצעות שירות ההטמעות, מוציא `RowEmbeddings`
**שלב 2: אחסון הטמעה** (`graphit-flow/graphit/storage/row_embeddings/qdrant/`) - צורך `RowEmbeddings`, כותב וקטורים ל-Qdrant

כותב השורות של Cassandra הוא צרכן מקבילי נפרד:

**כותב שורות Cassandra** (`graphit-flow/graphit/storage/rows/cassandra`) - צורך `ExtractedObject`, כותב שורות ל-Cassandra
...
|-------|--------|-------------|
| שלב 1 | `graphit-flow/graphit/embeddings/row_embeddings/` | `row-embeddings` |
| שלב 2 | `graphit-flow/graphit/storage/row_embeddings/qdrant/` | `row-embeddings-write-qdrant` |
...
מודול: `graphit-flow/graphit/query/row_embeddings/qdrant`
...
| רכיב | מיקום |
|-----------|----------|
| מפזר | `graphit-flow/graphit/gateway/dispatch/row_embeddings_query.py` |
...
פקודה: `gr-invoke-row-embeddings`
...
|-----------|----------|-------|-------------|
| שירות שאילתות | `graphit-flow/graphit/query/objects/cassandra/service.py` | ~740 | מונוליטי: יצירת סכימת GraphQL, ניתוח פילטרים, שאילתות Cassandra, טיפול בבקשות |
| כותב | `graphit-flow/graphit/storage/objects/cassandra/write.py` | ~540 | יצירת טבלאות לפי סכימה, אינדקסים משניים, הוספה/מחיקה |
...
**כותב** (`graphit-flow/graphit/storage/rows/cassandra/`):
...
**שירות שאילתות** (`graphit-flow/graphit/query/rows/cassandra/`):
...
| מודול | מטרה |
|--------|---------|
| `graphit-flow/graphit/query/graphql/` | כלי GraphQL משותפים |
| `graphit-flow/graphit/query/row_embeddings/qdrant/` | ממשק API לשאילתות הטמעות שורות |
| `graphit-flow/graphit/embeddings/row_embeddings/` | חישוב הטמעות שורות (שלב 1) |
| `graphit-flow/graphit/storage/row_embeddings/qdrant/` | אחסון הטמעות שורות (שלב 2) |
