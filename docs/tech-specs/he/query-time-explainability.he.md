...
כל ה-URIs משתמשים בשם מרחב שמות `urn:graphit:`, עם UUIDs:

| ישות | תבנית URI |
|---|---|
| סשן | `urn:graphit:session:{uuid}` |
| אחזור | `urn:graphit:prov:retrieval:{uuid}` |
| בחירה | `urn:graphit:prov:selection:{uuid}` |
| תשובה | `urn:graphit:prov:answer:{uuid}` |
| בחירת קצה | `urn:graphit:prov:edge:{uuid}:{index}` |
...
מוגדרים ב-`graphit-base/graphit/provenance/namespaces.py`:
