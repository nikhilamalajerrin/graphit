...
מסמך זה מתאר יכולות תצורה משופרות של שורת הפקודה עבור GraphIt, ומאפשר למשתמשים לנהל פריטי תצורה בודדים באמצעות פקודות שורת פקודה מפורטות. האינטגרציה תומכת ברשיות שימושיות עיקריות:
...
- **תבנית שורת פקודה עקבית**: לעקוב אחר התקנות והתבניות הסטנדרטיות של GraphIt
...
GraphIt מספקת כיום ניהול תצורה באמצעות ה-API של התצורה ופקודה בודדת בשם `gr-show-config` המציגה את כל התצורה. אמנם זה עובד לצורך הצגת התצורה, אך הוא חסר יכולות ניהול מפורטות.
...
מסמך זה מטפל בחוסרים אלה על ידי הוספת ארבע פקודות שורת פקודה חדשות המספקות ניהול תצורה מפורט. על ידי חשיפת פעולות API של התצורה הבודדות דרך פקודות שורת פקודה, GraphIt יכול:
...
1. **gr-list-config-items**
   ...
   מודול: `graphit.cli.list_config_items`
...
2. **gr-get-config-item**
   ...
   מודול: `graphit.cli.get_config_item`
...
3. **gr-put-config-item**
   ...
   מודול: `graphit.cli.put_config_item`
...
4. **gr-delete-config-item**
   ...
   מודול: `graphit.cli.delete_config_item`

### מודלים של נתונים

#### ConfigKey ו-ConfigValue

הפקודות משתמשות במבני נתונים הקיימים מ-`graphit.api.types`:
