...
אפשר תצורות של כלי MCP לציין טוקנים סטטיים אופציונליים לאימות עם שרתי MCP מוגנים. זה מאפשר ל-GraphIt להפעיל כלי MCP המאוחסנים בשרתים הדורשים אימות, מבלי לשנות את סוכן או ממשקי הפעלה של הכלי.
...
5. **אימות ברמת השירות:** מאמת את שירות GraphIt, ולא משתמשים בודדים.
6. **הקשר אבטחה משותף:** כל שימוש בכלי MCP משתמש באותו אישור.

### התאמת מקרי שימוש

**✅ מקרי שימוש מתאימים:**
פריסות GraphIt עם משתמש יחיד.
אימות בין שירותים (GraphIt → שרת MCP).
סביבות פיתוח ובדיקה.
כלי MCP פנימיים שאליהם ניגש מערכת GraphIt.
...
**תרחיש מתאים לדוגמה:**
פריסה של GraphIt עבור ארגון יחיד שבהם כל העובדים משתמשים באותו כלי MCP פנימי (לדוגמה, שאילתת מסד נתונים של החברה). שרת ה-MCP דורש אימות כדי למנוע גישה חיצונית, אך לכל המשתמשים הפנימיים יש את אותו רמת גישה.

**תרחיש לא מתאים לדוגמה:**
פלטפורמת SaaS מרובת משתמשים של GraphIt שבה כל אחד מ-Tenant A ו-Tenant B צריך לגשת לשרתי MCP מבודדים משלהם עם אישורים נפרדים. תכונה זו אינה תומכת בניהול טוקנים ברמת ה-Tenant.

### רכיבים קשורים
**graphit-flow/graphit/agent/mcp_tool/service.py**: שירות הפעלת כלי MCP.
**graphit-cli/graphit/cli/set_mcp_tool.py**: כלי שורת פקודה ליצירת/עדכון תצורות MCP.
**graphit-cli/graphit/cli/show_mcp_tools.py**: כלי שורת פקודה להצגת תצורות MCP.
...
3. **תמיכה בשורת הפקודה:** `gr-set-mcp-tool` חייב לקבל פרמטר אופציונלי `--auth-token`.
4. **הצגת טוקן:** `gr-show-mcp-tools` חייב לציין מתי טוקן אימות מוגדר (מוסתר מטעמי אבטחה).
...
1. **אחסון תצורה**: המשתמש מריץ את `gr-set-mcp-tool --id my-tool --tool-url http://server/api --auth-token xyz123`
...
**קובץ**: `graphit-flow/graphit/agent/mcp_tool/service.py`
...
**קובץ**: `graphit-cli/graphit/cli/set_mcp_tool.py`
...
`-u, --api-url` (אופציונלי): כתובת URL של ממשק API של GraphIt
...
**קובץ**: `graphit-cli/graphit/cli/show_mcp_tools.py`
...
#### רכיב 4: תיעוד
**קובץ**: `docs/cli/gr-set-mcp-tool.md`
...
### שלב 4: עדכון תיעוד
[ ] לתעד את הפרמטר `--auth-token` ב-`gr-set-mcp-tool.md`
...
| `gr-set-mcp-tool.md` | שינוי | שונות | תיעוד פרמטר חדש |
...
**ללא הקשר משתמש**: שרת ה-MCP אינו יכול להבחין בין משתמשי GraphIt שונים
...
הפריסה של GraphIt שלכם משרתת מספר ארגונים (מרובת לקוחות)
...
פריסת מופעי GraphIt נפרדים עבור כל לקוח
...
שרת ה-MCP אינו יכול להבחין בין משתמשי GraphIt שונים
...
### תיעוד למשתמש
[ ] עדכון `gr-set-mcp-tool.md` עם פרמטר `--auth-token`
...
[כלי שירות MCP נוכחי](../graphit-flow/graphit/agent/mcp_tool/service.py)
