...
- **חוויית משתמש סטנדרטית בסטרימינג**: לספק חוויית סטרימינג אחידה בכל שירותי GraphIt.
...
- חוויה לא עקבית בהשוואה לשירותי GraphIt אחרים.

מסמך זה מטפל בבעיות אלו על ידי הוספת תמיכה בסטרימינג ל-GraphRAG ו-DocumentRAG. על ידי אפשרות תגובות שוט אחר שוט, GraphIt יכול:
...
1. **סטרימינג של PromptClient** (כבר מיושם)
   ...
   - מודול: `graphit-base/graphit/base/prompt_client.py`
...
2. **שירות GraphRAG** (דורש העברת פרמטר סטרימינג)
   ...
   - מודולים:
     ...
     - `graphit-base/graphit/schema/graph_rag.py` (סכימת בקשה)
...
3. **שירות DocumentRAG** (דורש העברת פרמטר סטרימינג)
   ...
   - מודולים:
     ...
     - `graphit-base/graphit/schema/document_rag.py` (סכימת בקשה)
...
**בדיקות ידניות**:
- `gr-invoke-graph-rag -q "מהו למידת מכונה?"` (סטרימינג כברירת מחדל).
- `gr-invoke-document-rag -q "סכם את המסמכים על AI"` (סטרימינג כברירת מחדל).
- `gr-invoke-graph-rag --no-streaming -q "..."` (בדיקת מצב לא סטנדרטי).
...
- סטרימינג של PromptClient: `graphit-base/graphit/base/prompt_client.py`
- סטרימינג של סוכן: `graphit-flow/graphit/agent/react/agent_manager.py`
