...
מפרט זה עוסק בבעיות ביצועים ביישום בסיס הידע Cassandra של GraphIt ומציע אופטימיזציות לאחסון ושליפה של משולשות RDF.

## יישום נוכחי

### עיצוב הסכימה

היישום הנוכחי משתמש בעיצוב טבלה יחידה ב-`graphit-flow/graphit/direct/cassandra_kg.py`:
...
**קובץ: `graphit-flow/graphit/direct/cassandra_kg.py`**
...
**נקודות אינטגרציה:**
**נתיב כתיבה:** `graphit-flow/graphit/storage/triples/cassandra/write.py`
**נתיב שאילתה:** `graphit-flow/graphit/query/triples/cassandra/service.py`
**מאגר ידע:** `graphit-flow/graphit/tables/knowledge.py`
...
**`graphit-flow/graphit/direct/cassandra_kg.py`** - נדרש כתיבה מחדש מלאה.
