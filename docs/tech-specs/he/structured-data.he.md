...
מפרט זה מתאר את השילוב של GraphIt עם זרימות נתונים מובנים, המאפשר למערכת לעבוד עם נתונים שניתן לייצג אותם כשורות בטבלאות או כאובייקטים במאגרי אובייקטים. השילוב תומך בארבעה תרחישי שימוש עיקריים:
...
**גישה מאוחדת לנתונים**: מתן ממשק יחיד לגישה הן לנתונים מובנים והן לנתונים לא מובנים בתוך GraphIt.
**שילוב חלק**: אפשור תאימות הדוקה בין ייצוג הידע מבוסס הגרף של GraphIt לבין פורמטים מובנים מסורתיים.
...
**תאימות לאחור**: שמירה על פונקציונליות קיימת של GraphIt תוך הוספת יכולות נתונים מובנים.

## רקע

GraphIt מצטיינת כיום בעיבוד נתונים לא מובנים ובבניית גרפי ידע ממקורות מגוונים. עם זאת, תרחישי שימוש רבים בארגונים כוללים נתונים המובנים מטבעם - רשומות לקוחות, יומני עסקאות, מסדי נתונים של מלאי וערכות נתונים טבלאיים אחרים. ערכות נתונים מובנות אלו לעתים קרובות צריכות להיות מנותחות יחד עם תוכן לא מובנה כדי לספק תובנות מקיפות.
...
חוסר גשר בין שאילתות בסגנון SQL לשאילתות גרפיות של GraphIt.

מפרט זה מתייחס לפערים אלה על ידי הצגת שכבת נתונים מובנים המשלימה את היכולות הקיימות של GraphIt. על ידי תמיכה בנתונים מובנים באופן טבעי, GraphIt יכולה:
...
1. **שירות NLP ל-Structured-Query**
   ...
   משתלב עם יכולות NLP קיימות של GraphIt.
   
   מודול: graphit-flow/graphit/query/nlp_query/cassandra
...
3. **מודול חילוץ אובייקטים** ✅ **[הושלם]**
   ...
   רושם מטפל תצורה (לדוגמה: graphit-flow/graphit/prompt/template/service.py) כדי לקבל נתוני תצורה ולפענח מידע סכימה.
   מקבל אובייקטים ומפענח אותם לאובייקטים של ExtractedObject לצורך העברה בתורת ההודעות Pulsar.
   הערה: קיים קוד ב-`graphit-flow/graphit/extract/object/row/`. זו הייתה ניסיון קודם וצריך לשכתב אותו באופן משמעותי מכיוון שהוא אינו תואם ל-APIs הנוכחיים. השתמש בו אם זה שימושי, התחל מאפס אם לא.
   דורש ממשק שורת פקודה: `kg-extract-objects`

   מודול: graphit-flow/graphit/extract/kg/objects/
...
פלט חוזה (יש לעקוב בדיוק אחר הפורמט הבא).
   ...
   **הערות יישום**:
   קוד קיים ב-`graphit-flow/graphit/storage/objects/cassandra/` מיושן ואינו תואם ל-APIs הנוכחיים.
   יש להתייחס ל-`graphit-flow/graphit/storage/triples/cassandra` כדוגמה למעבד אחסון תקין.
   יש להעריך את הקוד הקיים כדי לזהות רכיבים שניתן לעשות בהם שימוש חוזר לפני שמחליטים לבצע שינוי מבני או כתיבה מחדש.

   מודול: graphit-flow/graphit/storage/objects/cassandra
...
5. **שירות שאילתות מובנה** ✅ **[הושלם]**
   ...
   מודול: graphit-flow/graphit/query/objects/cassandra
...
7. **שירות הכנסת נתונים מובנים**:
   ...
   מודול: graphit-flow/graphit/decoding/structured
...
8. **שירות הטמעת אובייקטים**:
   ...
   מודול: graphit-flow/graphit/embeddings/object_embeddings/qdrant

### מודלים של נתונים

#### מנגנון אחסון סכימות

סכימות מאוחסנות במערכת התצורה של GraphIt באמצעות המבנה הבא:
...
אינטגרציה עקבית עם ניהול התצורה הקיים של GraphIt
...
הכל נמצא בחבילות graphit-flow למעט פריטי סכימה
ב-graphit-base.
