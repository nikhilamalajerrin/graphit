...
מפרט זה מתאר שירות חדש הניתן לביצוע לאבחון וניתוח נתונים מובנים בתוך GraphIt. השירות שואב פונקציונליות מכלי שורת הפקודה הקיים `gr-load-structured-data` ומציג אותו כשירות בקשה/תגובה, ומאפשר גישה תכנותית ליכולות זיהוי סוג נתונים ויצירת תיאורים.
...
**יצירת תיאורים מדויקים**: יצירת תיאורי נתונים מובנים המתאימים בצורה מדויקת לנתוני המקור לתוך סכימות GraphIt
...
כיום, הפקודה `gr-load-structured-data` מספקת פונקציונליות מקיפה לניתוח נתונים מובנים ויצירת תיאורים. עם זאת, פונקציונליות זו קשורה הדוקות לממשק שורת הפקודה, מה שמגביל את יכולת השימוש החוזר בה.
...
מפרט זה מתייחס לפערים אלה על ידי יצירת שירות ייעודי לאבחון נתונים מובנים. על ידי חשיפת יכולות אלה כשירות, GraphIt יכולה:
...
1. **מעבד שירות אבחון**
   ...
   מודול: `graphit-flow/graphit/diagnosis/structured_data/service.py`
...
2. **מגַלֶּה סוג נתונים**
   ...
   מודול: `graphit-flow/graphit/diagnosis/structured_data/type_detector.py`

3. **יוצר תיאורים**
   משתמש בשירות פרומפטים כדי ליצור תיאורים
   מפעיל פרומפטים ספציפיים לפורמט (diagnose-csv, diagnose-json, diagnose-xml)
   ממפה שדות נתונים לשדות סכימת GraphIt באמצעות תגובות פרומפטים

   מודול: `graphit-flow/graphit/diagnosis/structured_data/descriptor_generator.py`
...
השירות יפעל בהתאם לקונבנציות של שירות GraphIt:
...
השירות ישולב עם ממשקי GraphIt קיימים:

רכיבים שעברו שינוי:
`gr-load-structured-data` CLI - שופץ לשימוש בשירות החדש לפעולות אבחון
...
בקרת גישה באמצעות אימות GraphIt הקיים
...
`gr-load-structured-data` יישום: `graphit-cli/graphit/cli/load_structured_data.py`
