...
**Location**: `graphit-flow/graphit/librarian/blob_store.py`
**Operations**:
...
### Key Files
1. `graphit-flow/graphit/librarian/blob_store.py` - יישום BlobStore
2. `graphit-flow/graphit/librarian/librarian.py` - אתחול BlobStore
3. `graphit-flow/graphit/librarian/service.py` - תצורת שירות
4. `graphit-flow/pyproject.toml` - תלויות (חבילת `minio`)
...
**עבור Ceph RGW:**
```bash
radosgw-admin user create --uid="graphit" --display-name="GraphIt Service"
...
**מיקום:** `graphit-flow/graphit/librarian/blob_store.py`
