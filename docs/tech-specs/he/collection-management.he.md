...
מפרט זה מתאר את יכולות ניהול האוספים עבור GraphIt, הדורש יצירת אוספים באופן מפורש ומספק שליטה ישירה על מחזור החיים של האוסף. יש ליצור אוספים באופן מפורש לפני השימוש, כדי להבטיח סנכרון תקין בין מטא-הנתונים של הספרן לבין כל אחסני הנתונים. הפיצ'ר תומך בארבעה תרחישי שימוש עיקריים:
...
בעבר, אוספים ב-GraphIt נוצרו באופן סמוי במהלך פעולות טעינת נתונים, מה שגרם לבעיות סנכרון שבהן אוספים יכלו להתקיים באחסני נתונים ללא מטא-נתונים תואמים בספרן. זה יצר אתגרים בניהול ואפשרות של נתונים יתומים.

מודל יצירת האוספים המפורש פותר בעיות אלה על ידי:
דרישה ליצירת אוספים לפני השימוש באמצעות `gr-set-collection`
...
מפרט זה מגדיר את מודל ניהול האוספים המפורש. על ידי דרישה ליצירת אוסף מפורשת, GraphIt מבטיחה:
...
מערכת ניהול האוספים תיושם בתוך התשתית הקיימת של GraphIt:
...
1. **יצירת אוסף** (שני מסלולים):
   ...
   **מסלול א': יצירה על ידי משתמש** באמצעות `gr-set-collection`:
...
4. **עדכוני מטא-נתונים**: משתמשים יכולים לעדכן מטא-נתונים של אוסף לאחר יצירתו:
   עדכון שם, תיאור ותגיות באמצעות `gr-set-collection`
   העדכונים חלים רק על מטא-נתונים של הספרן
   מערכות האחסון שומרות על האוסף, אך עדכוני מטא-נתונים אינם מועברים

5. **מחיקה מפורשת**: משתמשים מוחקים אוספים באמצעות `gr-delete-collection`:
...
פעולות נדרשות:
**יצירת אוסף**: פעולה של משתמש באמצעות `gr-set-collection` או אוטומטית בעת הגשת מסמך
...
אוספים קיימים על פני מספר מערכות אחסון ב-GraphIt:
...
**יצירת/עדכון איסוף**: יצירת איסוף חדש או עדכון מטא-נתונים קיימים באמצעות `gr-set-collection`
...
היישום יעקוב לדפוסי GraphIt קיימים לשילוב שירותים ומבנה פקודות שורת פקודה.
...
1. **שינוי מבנה מחלקה GraphIt** (`graphit/direct/cassandra.py`):
...
2. **עדכוני כותב אחסון** (`graphit/storage/triples/cassandra/write.py`):
   שמירה על חיבור GraphIt יחיד לכל משתמש במקום לכל (משתמש, אוסף)
   העברת האוסף לפעולות הכנסה
   שיפור ניצול משאבים עם פחות חיבורים

3. **עדכוני שירות שאילתות** (`graphit/query/triples/cassandra/service.py`):
   חיבור GraphIt יחיד לכל משתמש
...
פעולות ניהול איסוף דורשות הרשאות מתאימות כדי למנוע גישה או מחיקה לא מורשית של איסופים. בקרת הגישה תתאים למודלי האבטחה הקיימים של GraphIt.
...
1. **שירות ניהול איסופים של Librarian** (`graphit-flow/graphit/librarian/collection_manager.py`)
...
2. **סכימת מטא-נתונים של איסוף** (`graphit-base/graphit/schema/services/collection.py`)
...
3. **סכימת ניהול אחסון** (`graphit-base/graphit/schema/services/storage.py`)
...
4. **סכימת 4-טבלאות של Cassandra** (`graphit-flow/graphit/direct/cassandra_kg.py`)
