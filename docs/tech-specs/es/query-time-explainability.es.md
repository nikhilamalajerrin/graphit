...
Todos los URI utilizan el espacio de nombres `urn:graphit:` con UUIDs:

| Entidad | Patrón de URI |
|---|---|
| Sesión | `urn:graphit:session:{uuid}` |
| Recuperación | `urn:graphit:prov:retrieval:{uuid}` |
| Selección | `urn:graphit:prov:selection:{uuid}` |
| Respuesta | `urn:graphit:prov:answer:{uuid}` |
| Selección de borde | `urn:graphit:prov:edge:{uuid}:{index}` |
...
Definidas en `graphit-base/graphit/provenance/namespaces.py`:
