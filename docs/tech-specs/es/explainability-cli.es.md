...
Esta especificación describe las herramientas de CLI para depurar y explorar datos de explicabilidad en GraphIt. Estas herramientas permiten a los usuarios rastrear cómo se derivaron las respuestas y depurar la cadena de origen desde los bordes hasta los documentos fuente.

Tres herramientas de CLI:

1.  **`gr-show-document-hierarchy`** - Muestra la jerarquía documento → página → fragmento → borde
2.  **`gr-list-explain-traces`** - Lista todas las sesiones GraphRAG con preguntas
3.  **`gr-show-explain-trace`** - Muestra la trazabilidad completa de explicabilidad para una sesión
...
GraphIt tiene dos sistemas de origen:
...
### Herramienta 1: `gr-show-document-hierarchy`
...
| `-u/--api-url` | URL del gateway (por defecto: `$GRAPHIT_URL`) |
| `-t/--token` | Token de autenticación (por defecto: `$GRAPHIT_TOKEN`) |
| `-U/--user` | ID de usuario (por defecto: `graphit`) |
...
Document: urn:graphit:doc:abc123
  ...
  └── Página 1: urn:graphit:doc:abc123/p1
      ├── Fragmento 0: urn:graphit:doc:abc123/p1/c0
          Contenido: "The quick brown fox..." [truncado]
      └── Fragmento 1: urn:graphit:doc:abc123/p1/c1
          Contenido: "Machine learning is..." [truncado]
```

### Herramienta 2: `gr-list-explain-traces`
...
----------------------------------------------|--------------------------------|---------------------
urn:graphit:question:abc123                | ¿Cuál fue la Guerra Fría?    | 2024-01-15 10:30:00
urn:graphit:question:def456                | ¿Quién fundó la NASA?            | 2024-01-15 09:15:00
```

### Herramienta 3: `gr-show-explain-trace`
...
**Ejemplo de Salida**:
```
=== Sesión GraphRAG: urn:graphit:question:abc123 ===
...
-   Ejemplo de CLI: `graphit-cli/graphit/cli/invoke_graph_rag.py`
