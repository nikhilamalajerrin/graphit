...
5. **Autenticación a nivel de servicio:** Autentica el servicio GraphIt, no a usuarios individuales.
...
Implementaciones de GraphIt de un solo inquilino.
Autenticación de servicio a servicio (GraphIt → Servidor MCP).
Entornos de desarrollo y pruebas.
Herramientas MCP internas a las que accede el sistema GraphIt.
...
Una implementación de GraphIt de una sola organización donde todos los empleados utilizan la misma herramienta MCP interna (por ejemplo, búsqueda de bases de datos de la empresa). El servidor MCP requiere autenticación para evitar el acceso externo, pero todos los usuarios internos tienen el mismo nivel de acceso.

**Ejemplo de Escenario Inapropiado:**
Una plataforma SaaS multiinquilino de GraphIt donde el inquilino A y el inquilino B necesitan acceder a sus propios servidores MCP aislados con credenciales separadas. Esta función NO admite la administración de tokens por inquilino.

### Componentes Relacionados
**graphit-flow/graphit/agent/mcp_tool/service.py**: Servicio de invocación de la herramienta MCP.
**graphit-cli/graphit/cli/set_mcp_tool.py**: Herramienta de línea de comandos para crear/actualizar configuraciones de MCP.
**graphit-cli/graphit/cli/show_mcp_tools.py**: Herramienta de línea de comandos para mostrar configuraciones de MCP.
...
3. **Soporte de la CLI:** `gr-set-mcp-tool` DEBE aceptar un parámetro opcional `--auth-token`.
4. **Visualización del Token:** `gr-show-mcp-tools` DEBE indicar cuándo se ha configurado un token de autenticación (enmascarado por motivos de seguridad).
...
1. **Almacenamiento de la configuración:** El usuario ejecuta `gr-set-mcp-tool --id my-tool --tool-url http://server/api --auth-token xyz123`.
...
**Archivo:** `graphit-flow/graphit/agent/mcp_tool/service.py`
...
**Archivo**: `graphit-cli/graphit/cli/set_mcp_tool.py`
...
`-u, --api-url` (opcional): URL de la API de GraphIt.
...
**Archivo**: `graphit-cli/graphit/cli/show_mcp_tools.py`
...
**Archivo**: `docs/cli/gr-set-mcp-tool.md`
...
[ ] Documentar el parámetro `--auth-token` en `gr-set-mcp-tool.md`
...
| `gr-set-mcp-tool.md` | Modificado | Varios | Documentar el nuevo parámetro |
...
**Sin contexto de usuario**: El servidor MCP no puede distinguir entre diferentes usuarios de GraphIt
...
Su implementación de GraphIt sirve a múltiples organizaciones (multiinquilino)
...
Implementar instancias de GraphIt separadas por tenant
...
El servidor MCP no puede distinguir entre diferentes usuarios de GraphIt
...
[ ] Actualizar `gr-set-mcp-tool.md` con el parámetro `--auth-token`
[ ] Agregar una sección de consideraciones de seguridad
...
[Servicio de la herramienta MCP actual](../graphit-flow/graphit/agent/mcp_tool/service.py)
