...
Esta especificación describe la integración de GraphIt con flujos de datos estructurados, lo que permite que el sistema trabaje con datos que se pueden representar como filas en tablas u objetos en almacenes de objetos. La integración admite cuatro casos de uso principales:
...
**Acceso Unificado a Datos**: Proporcionar una única interfaz para acceder tanto a datos estructurados como no estructurados dentro de GraphIt.
**Integración Fluida**: Permitir una interoperabilidad fluida entre la representación de conocimiento basada en gráficos de GraphIt y los formatos de datos estructurados tradicionales.
...
**Compatibilidad con Versiones Anteriores**: Preservar la funcionalidad existente de GraphIt al agregar capacidades de datos estructurados.

## Antecedentes

Actualmente, GraphIt destaca en el procesamiento de datos no estructurados y en la creación de gráficos de conocimiento a partir de diversas fuentes. Sin embargo, muchos casos de uso empresariales implican datos que son inherentemente estructurados: registros de clientes, registros de transacciones, bases de datos de inventario y otros conjuntos de datos tabulares. Estos conjuntos de datos estructurados a menudo deben analizarse junto con contenido no estructurado para proporcionar información integral.
...
Falta de un puente entre las consultas tipo SQL y las consultas de gráficos de GraphIt.

Esta especificación aborda estas deficiencias mediante la introducción de una capa de datos estructurados que complementa las capacidades existentes de GraphIt. Al admitir datos estructurados de forma nativa, GraphIt puede:
...
1. **Servicio de Conversión de Lenguaje Natural a Consulta Estructurada**
   ...
   Se integra con las capacidades existentes de NLP de GraphIt.
...
3. **Módulo de Extracción de Objetos** ✅ **[COMPLETO]**
   ...
   Registra un controlador de configuración (ejemplo: graphit-flow/graphit/prompt/template/service.py) para recibir datos de configuración y decodificar información del esquema.
   Recibe objetos y los decodifica en objetos ExtractedObject para su entrega en la cola de Pulsar.
   NOTA: Existe código en `graphit-flow/graphit/extract/object/row/`. Este fue un intento anterior y deberá refactorizarse por completo, ya que no se ajusta a las API actuales. Úselo si es útil, comience desde cero si no.
...
4. **Módulo de Escritura de Almacén Estructurado** ✅ **[COMPLETO]**
   ...
   **Notas de Implementación**:
   El código existente en `graphit-flow/graphit/storage/objects/cassandra/` está obsoleto y no cumple con las API actuales.
   Debe referenciar `graphit-flow/graphit/storage/triples/cassandra` como un ejemplo de un procesador de almacenamiento que funciona.
...
Los esquemas se almacenan en el sistema de configuración de GraphIt utilizando la siguiente estructura:
...
Integración consistente con la gestión de configuración de GraphIt existente
...
Todo está en los paquetes de graphit-flow, excepto los elementos del esquema
en graphit-base.
