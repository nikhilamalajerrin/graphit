...
Esta especificación aborda los problemas y las deficiencias identificadas durante la implementación inicial de la integración de datos estructurados de GraphIt, como se describe en `structured-data.md`.
...
**Etapa 1: Cálculo de incrustados** (`graphit-flow/graphit/embeddings/row_embeddings/`) - Consume `ExtractedObject`, calcula incrustados a través del servicio de incrustados, produce `RowEmbeddings`
**Etapa 2: Almacenamiento de incrustados** (`graphit-flow/graphit/storage/row_embeddings/qdrant/`) - Consume `RowEmbeddings`, escribe vectores en Qdrant

El escritor de filas de Cassandra es un consumidor paralelo separado:

**Escritor de filas de Cassandra** (`graphit-flow/graphit/storage/rows/cassandra`) - Consume `ExtractedObject`, escribe filas en Cassandra
...
|-------|--------|-------------|
| Etapa 1 | `graphit-flow/graphit/embeddings/row_embeddings/` | `row-embeddings` |
| Etapa 2 | `graphit-flow/graphit/storage/row_embeddings/qdrant/` | `row-embeddings-write-qdrant` |
...
Módulo: `graphit-flow/graphit/query/row_embeddings/qdrant`
...
|-----------|----------|
| Despachador | `graphit-flow/graphit/gateway/dispatch/row_embeddings_query.py` |
...
Comando: `gr-invoke-row-embeddings`

```bash
# Query by text (computes embedding automatically)
gr-invoke-row-embeddings \
  --text "Chestnut Street" \
  --collection my_collection \
  --schema addresses \
  --index street_name \
...
# Output formats
gr-invoke-row-embeddings --text "..." --format json
gr-invoke-row-embeddings --text "..." --format table
```
...
|-----------|----------|-------|-------------|
| Servicio de Consulta | `graphit-flow/graphit/query/objects/cassandra/service.py` | ~740 | Monolítico: generación de esquema GraphQL, análisis de filtros, consultas de Cassandra, manejo de solicitudes |
| Escritor | `graphit-flow/graphit/storage/objects/cassandra/write.py` | ~540 | Creación de tablas por esquema, índices secundarios, inserción/eliminación |
...
```
graphit-flow/graphit/query/graphql/
├── __init__.py
...
**Escritor** (`graphit-flow/graphit/storage/rows/cassandra/`):
...
**Servicio de Consulta** (`graphit-flow/graphit/query/rows/cassandra/`):
...
| Módulo | Propósito |
|--------|---------|
| `graphit-flow/graphit/query/graphql/` | Utilidades GraphQL compartidas |
| `graphit-flow/graphit/query/row_embeddings/qdrant/` | API de consulta de incrustaciones de filas |
| `graphit-flow/graphit/embeddings/row_embeddings/` | Cálculo de incrustaciones de filas (Etapa 1) |
| `graphit-flow/graphit/storage/row_embeddings/qdrant/` | Almacenamiento de incrustaciones de filas (Etapa 2) |
