...
Esta especificación describe las capacidades de gestión de colecciones para GraphIt, que requieren la creación explícita de colecciones y proporcionan un control directo sobre el ciclo de vida de la colección. Las colecciones deben crearse explícitamente antes de su uso, lo que garantiza una sincronización adecuada entre los metadatos del bibliotecario y todos los backends de almacenamiento. La función admite cuatro casos de uso principales:
...
Anteriormente, las colecciones en GraphIt se creaban implícitamente durante las operaciones de carga de datos, lo que provocaba problemas de sincronización en los que las colecciones podían existir en los backends de almacenamiento sin metadatos correspondientes en el bibliotecario. Esto creaba desafíos de gestión y posibles datos huérfanos.

El modelo de creación explícita de colecciones aborda estos problemas mediante:
La necesidad de crear colecciones antes de su uso a través de `gr-set-collection`
...
Esta especificación define el modelo de gestión explícita de colecciones. Al requerir la creación explícita de colecciones, GraphIt garantiza:
...
El sistema de gestión de colecciones se implementará dentro de la infraestructura existente de GraphIt:
...
1. **Creación de la colección** (Dos caminos):
   ...
   **Camino A: Creación iniciada por el usuario** a través de `gr-set-collection`:
...
4. **Actualizaciones de metadatos**: Los usuarios pueden actualizar los metadatos de la colección después de la creación:
   Actualice el nombre, la descripción y las etiquetas a través de `gr-set-collection`
...
5. **Eliminación explícita**: Los usuarios eliminan las colecciones a través de `gr-delete-collection`:
...
**Crear colección**: Operación del usuario a través de `gr-set-collection` O automática al enviar un documento
...
Las colecciones existen en varios backends de almacenamiento en GraphIt:
...
**Crear/Actualizar colección**: Crear una nueva colección o actualizar los metadatos existentes a través de `gr-set-collection`
...
La implementación seguirá los patrones existentes de GraphIt para la integración de servicios y la estructura de comandos de la CLI.
...
1. **Refactorización de la Clase GraphIt** (`graphit/direct/cassandra.py`):
...
2. **Actualizaciones del Escritor de Almacenamiento** (`graphit/storage/triples/cassandra/write.py`):
   Mantener una única conexión GraphIt por usuario en lugar de por (usuario, colección).
   Pasar la colección a las operaciones de inserción.
   Mejor utilización de recursos con menos conexiones.

3. **Actualizaciones del Servicio de Consulta** (`graphit/query/triples/cassandra/service.py`):
   Una única conexión GraphIt por usuario.
...
Las operaciones de administración de colecciones requieren la autorización adecuada para evitar el acceso no autorizado o la eliminación de colecciones. El control de acceso se alineará con los modelos de seguridad de GraphIt existentes.
...
1. **Servicio de administración de colecciones Librarian** (`graphit-flow/graphit/librarian/collection_manager.py`)
...
2. **Esquema de metadatos de colecciones** (`graphit-base/graphit/schema/services/collection.py`)
...
3. **Esquema de administración de almacenamiento** (`graphit-base/graphit/schema/services/storage.py`)
...
4. **Esquema de tabla Cassandra de 4 tablas** (`graphit-flow/graphit/direct/cassandra_kg.py`)
...
✅ `graphit-flow/graphit/storage/triples/cassandra/write.py`
✅ `graphit-flow/graphit/storage/triples/neo4j/write.py`
✅ `graphit-flow/graphit/storage/triples/memgraph/write.py`
✅ `graphit-flow/graphit/storage/triples/falkordb/write.py`
✅ `graphit-flow/graphit/storage/doc_embeddings/qdrant/write.py`
✅ `graphit-flow/graphit/storage/graph_embeddings/qdrant/write.py`
✅ `graphit-flow/graphit/storage/doc_embeddings/milvus/write.py`
✅ `graphit-flow/graphit/storage/graph_embeddings/milvus/write.py`
✅ `graphit-flow/graphit/storage/doc_embeddings/pinecone/write.py`
✅ `graphit-flow/graphit/storage/graph_embeddings/pinecone/write.py`
✅ `graphit-flow/graphit/storage/objects/cassandra/write.py`
