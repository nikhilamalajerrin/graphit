...
本规范解决了 GraphIt Cassandra 知识库实现中的性能问题，并提出了针对 RDF 三元组存储和查询的优化方案。

## 当前实现

### 模式设计

当前实现使用单表设计，位于 `graphit-flow/graphit/direct/cassandra_kg.py`：
...
**文件: `graphit-flow/graphit/direct/cassandra_kg.py`**
...
**集成点:**
**写入路径:** `graphit-flow/graphit/storage/triples/cassandra/write.py`
**查询路径:** `graphit-flow/graphit/query/triples/cassandra/service.py`
**知识存储:** `graphit-flow/graphit/tables/knowledge.py`
...
**`graphit-flow/graphit/direct/cassandra_kg.py`** - 需要完全重写。
