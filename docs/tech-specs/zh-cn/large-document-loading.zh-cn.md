...
本规范解决了在将大型文档加载到 GraphIt 时出现的可扩展性和用户体验问题。 当前架构将文档上传视为单个原子操作，这会在管道的多个阶段导致内存压力，并且不为用户提供任何反馈或恢复选项。
...
关键文件：
REST/WebSocket 入口：`graphit-flow/graphit/gateway/service.py`
Librarian 核心：`graphit-flow/graphit/librarian/librarian.py`
Blob 存储：`graphit-flow/graphit/librarian/blob_store.py`
Cassandra 表：`graphit-flow/graphit/tables/library.py`
API 模式：`graphit-base/graphit/schema/services/library.py`
...
**文件：** `graphit-flow/graphit/librarian/blob_store.py`
...
Python SDK (`graphit-base/graphit/api/library.py`) 应该能够透明地处理分块上传。公共接口保持不变：
分块上传。公共接口保持不变：
...
`gr-load-pdf` - 已过时，请使用 `gr-add-library-document`
`gr-load-text` - 已过时，请使用 `gr-add-library-document`
