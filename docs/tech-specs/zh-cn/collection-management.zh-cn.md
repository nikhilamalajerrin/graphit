...
本规范描述了 GraphIt 的集合管理功能，要求显式创建集合，并提供对集合生命周期的直接控制。在开始使用之前，必须显式创建集合，以确保 librarian 元数据与所有存储后端之间的正确同步。该功能支持四个主要用例：
...
以前，GraphIt 中的集合是在数据加载操作期间隐式创建的，这会导致同步问题，即集合可能存在于存储后端中，但 librarian 中没有相应的元数据。这带来了管理挑战和潜在的数据孤立问题。

显式集合创建模型通过以下方式解决了这些问题：
通过 `gr-set-collection` 要求在开始使用之前创建集合。
将集合创建广播到所有存储后端。
维护 librarian 元数据和存储之间的同步状态。
阻止写入不存在的集合。
提供清晰的集合生命周期管理。

本规范定义了显式集合管理模型。通过要求显式创建集合，GraphIt 确保：
...
1. **集合创建** (两种路径):
   ...
   **路径 A: 用户发起创建** 通过 `gr-set-collection`:
   用户提供集合 ID、名称、描述和标签
...
4. **元数据更新**: 用户可以在创建后更新集合元数据:
   通过 `gr-set-collection` 更新名称、描述和标签
   更新仅应用于 librarian 元数据
   存储后端保留集合，但元数据更新不会传播

5. **显式删除**: 用户通过 `gr-delete-collection` 删除集合:
...
需要的操作:
**创建集合**: 通过 `gr-set-collection` 进行用户操作，或在文档提交时自动进行
...
1. **GraphIt 类重构** (`graphit/direct/cassandra.py`):
...
2. **存储写入器更新** (`graphit/storage/triples/cassandra/write.py`):
   维护每个用户单个 GraphIt 连接，而不是每个 (用户, 集合)
   将集合传递给插入操作
   通过减少连接数提高资源利用率

3. **查询服务更新** (`graphit/query/triples/cassandra/service.py`):
   每个用户的单个 GraphIt 连接
   将集合传递给所有查询操作
   保持相同的查询逻辑，并带有集合参数
...
集合管理操作需要适当的授权，以防止未经授权的访问或删除集合。 访问控制将与现有的 GraphIt 安全模型保持一致。
...
1. **Librarian 集合管理服务** (`graphit-flow/graphit/librarian/collection_manager.py`)
   集合元数据 CRUD 操作（列表、更新、删除）
   通过 `LibraryTableStore` 与 Cassandra 集合元数据表集成
   集合删除在所有存储类型上的级联协调
   具有适当错误管理的异步请求/响应处理

2. **集合元数据模式** (`graphit-base/graphit/schema/services/collection.py`)
   `CollectionManagementRequest` 和 `CollectionManagementResponse` 模式
   `CollectionMetadata` 模式用于集合记录
   集合请求/响应队列主题定义

3. **存储管理模式** (`graphit-base/graphit/schema/services/storage.py`)
   `StorageManagementRequest` 和 `StorageManagementResponse` 模式
   定义了存储管理队列主题
   存储级别集合操作的消息格式

4. **Cassandra 4-表模式** (`graphit-flow/graphit/direct/cassandra_kg.py`)
   用于查询性能的复合分区键
