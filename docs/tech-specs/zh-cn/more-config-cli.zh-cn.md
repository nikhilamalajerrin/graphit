...
该规范描述了 GraphIt 的增强型命令行配置功能，允许用户通过精细的 CLI 命令来管理单个配置项。 该集成支持四个主要用例：
...
-   **一致的 CLI 模式**: 遵循 GraphIt 的标准 CLI 约定和模式
-   **错误处理**: 提供无效操作的清晰错误消息
...
目前，GraphIt 通过 Config API 和一个名为 `gr-show-config` 的单一 CLI 命令来管理配置。 该命令显示整个配置。 虽然这对于查看配置有效，但缺乏精细的管理功能。
...
该规范通过添加四个新的 CLI 命令来解决这些问题，从而提供精细的配置管理。 通过将单个 Config API 操作暴露到 CLI 命令中，GraphIt 可以：
-   启用脚本化的配置管理
-   允许用户按类型探索配置结构
-   支持有针对性的配置更新
-   提供精细的配置控制
...
1.  **gr-list-config-items**
    ...
    模块：`graphit.cli.list_config_items`
...
2.  **gr-get-config-item**
    ...
    模块：`graphit.cli.get_config_item`
...
3.  **gr-put-config-item**
    ...
    模块：`graphit.cli.put_config_item`
...
4.  **gr-delete-config-item**
    ...
    模块：`graphit.cli.delete_config_item`

### 数据模型

#### ConfigKey 和 ConfigValue

这些命令使用来自 `graphit.api.types` 的现有数据结构：
...
所有命令都遵循 GraphIt 的标准 CLI 模式：
-   使用 `argparse` 进行命令行参数解析
-   导入和使用 `graphit.api.Api` 进行后端通信
...
**文本格式 (默认)**:
-   `gr-list-config-items`: 键按新行分隔，纯文本
-   `gr-get-config-item`: 原始字符串值，无引号或编码

**JSON 格式**:
-   `gr-list-config-items`: 字符串数组 `["key1", "key2", "key3"]`
-   `gr-get-config-item`: JSON 编码的字符串值 `"实际字符串值"`

#### 输入处理

**gr-put-config-item** 支持两种互斥的输入方法：
