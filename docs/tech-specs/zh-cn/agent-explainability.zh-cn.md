...
GraphRAG 和 Agent 都使用 PROV-O 作为基础本体，并具有 GraphIt 特定的子类型：
...
| 子类型 | URI 模式 | 机制 |
|---------|-------------|-----------|
| `tg:GraphRagQuestion` | `urn:graphit:question:{uuid}` | 知识图谱 RAG |
| `tg:DocRagQuestion` | `urn:graphit:docrag:{uuid}` | 文档/块 RAG |
| `tg:AgentQuestion` | `urn:graphit:agent:{uuid}` | ReAct 代理 |
...
**文件:** `graphit-base/graphit/schema/services/agent.py`
...
**文件:** `graphit-base/graphit/messaging/translators/agent.py`

更新翻译器，使其能够处理 `session_id` 和 `collection`，并在 `to_pulsar()` 和 `from_pulsar()` 中均能正确处理。

### 2. 向 Agent Service 添加可解释性生产者

**文件:** `graphit-flow/graphit/agent/react/service.py`
...
**文件:** `graphit-base/graphit/provenance/agent.py`
...
**文件:** `graphit-base/graphit/provenance/namespaces.py`
...
| 文件 | 更改 |
|------|--------|
| `graphit-base/graphit/schema/services/agent.py` | 向 AgentRequest 添加 session_id 和 collection |
| `graphit-base/graphit/messaging/translators/agent.py` | 更新翻译器以适应新字段 |
| `graphit-base/graphit/provenance/namespaces.py` | 添加实体类型、agent谓词和 Document RAG 谓词 |
| `graphit-base/graphit/provenance/triples.py` | 向 GraphRAG 三元组构建器添加 TG 类型，添加 Document RAG 三元组构建器 |
| `graphit-base/graphit/provenance/uris.py` | 添加 Document RAG URI 生成器 |
| `graphit-base/graphit/provenance/__init__.py` | 导出新类型、谓词和 Document RAG 函数 |
| `graphit-base/graphit/schema/services/retrieval.py` | 向 DocumentRagResponse 添加 explain_id 和 explain_graph |
| `graphit-base/graphit/messaging/translators/retrieval.py` | 更新 DocumentRagResponseTranslator 以适应可解释性字段 |
| `graphit-flow/graphit/agent/react/service.py` | 添加可解释性生产者 + 记录逻辑 |
| `graphit-flow/graphit/retrieval/document_rag/document_rag.py` | 添加可解释性回调并发出溯源三元组 |
| `graphit-flow/graphit/retrieval/document_rag/rag.py` | 添加可解释性生产者并连接回调 |
| `graphit-cli/graphit/cli/show_explain_trace.py` | 处理 agent 跟踪类型 |
| `graphit-cli/graphit/cli/list_explain_traces.py` | 在 GraphRAG 旁边列出 agent 会话 |

## 创建的文件

| 文件 | 目的 |
|------|---------|
| `graphit-base/graphit/provenance/agent.py` | Agent 相关的三元组生成器 |

## CLI 更新

**检测：** 无论是 GraphRAG 还是 Agent 问题，都具有 `tg:Question` 类型。通过以下方式区分：
1. URI 模式：`urn:graphit:agent:` vs `urn:graphit:question:`
