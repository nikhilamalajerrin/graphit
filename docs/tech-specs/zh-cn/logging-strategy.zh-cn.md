...
GraphIt 使用 Python 内置的 `logging` 模块进行所有日志操作，并具有集中配置以及可选的 Loki 集成，用于日志聚合。 这提供了一种标准且灵活的方式，可在系统的所有组件中进行日志记录。
...
所有日志配置都由 `graphit.base.logging` 模块管理，该模块提供：
...
已经在 `graphit-base/pyproject.toml` 和 `requirements.txt` 中包含。
