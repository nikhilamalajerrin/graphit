...
根据 `STRUCTURED_DATA.md` 规范，本文件提出必要的 Pulsar 模式添加和修改，以支持 GraphIt 中的结构化数据功能。
...
这些模式将由新的流模块使用：
- `graphit-flow/graphit/decoding/structured` - 使用 StructuredDataSubmission
- `graphit-flow/graphit/query/nlp_query/cassandra` - 使用 NLP 查询模式
- `graphit-flow/graphit/query/objects/cassandra` - 使用结构化查询模式
- `graphit-flow/graphit/extract/object/row/` - 消耗 Chunk，产生 ExtractedObject
- `graphit-flow/graphit/storage/objects/cassandra` - 使用 Rows 模式
- `graphit-flow/graphit/embeddings/object_embeddings/qdrant` - 使用对象嵌入模式
