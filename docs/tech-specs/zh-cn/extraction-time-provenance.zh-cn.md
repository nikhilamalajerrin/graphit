...
`graphit-flow/graphit/librarian/librarian.py` - 核心 librarian 操作。
`graphit-flow/graphit/librarian/service.py` - 服务处理器，文档加载。
`graphit-flow/graphit/tables/library.py` - Cassandra 表存储。
`graphit-base/graphit/schema/services/library.py` - 模式定义。
