...
本文档描述了 GraphIt 如何管理跨不同后端实现（Qdrant、Pinecone、Milvus）的向量存储集合。该设计解决了在不硬编码维度值的情况下，支持具有不同维度的嵌入向量的挑战。
...
存储模式：`graphit-base/graphit/schema/services/storage.py`
Librarian 服务：`graphit-flow/graphit/librarian/service.py`
