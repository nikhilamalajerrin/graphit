...
本规范描述了 GraphIt 系统中本体的结构和格式。 本体提供正式的知识模型，定义类、属性、以及它们之间的关系，从而支持推理和推断功能。 该系统采用基于 OWL 的配置格式，广泛地表示 OWL/RDFS 概念，同时针对 GraphIt 的特定需求进行了优化。
...
GraphIt 将本体存储为配置项，采用灵活的键值系统。 尽管该格式受到 OWL (Web Ontology Language) 的启发，但它针对 GraphIt 的特定用例进行了优化，并且并非完全符合所有 OWL 规范。

在 GraphIt 中，本体能够实现以下功能：
