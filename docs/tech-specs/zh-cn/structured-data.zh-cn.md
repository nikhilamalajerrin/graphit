...
本规范描述了 GraphIt 与结构化数据流的集成，使系统能够处理可以表示为表格中的行或对象存储中的对象的结构化数据。该集成支持四个主要用例：
...
**统一数据访问**: 提供一个用于访问 GraphIt 中结构化和非结构化数据的单一接口。
**无缝集成**: 实现 GraphIt 的基于图的知识表示与传统结构化数据格式之间的平滑互操作性。
...
**向后兼容性**: 在添加结构化数据功能的同时，保留现有的 GraphIt 功能。

## 背景

GraphIt 目前擅长处理非结构化数据并从各种来源构建知识图。然而，许多企业用例涉及本质上是结构化的数据，例如客户记录、事务日志、库存数据库和其他表格数据集。这些结构化数据集通常需要与非结构化内容一起进行分析，以提供全面的见解。

当前的局限性包括：
不支持导入预结构化数据格式（CSV、JSON 数组、数据库导出）。
无法在从文档中提取表格数据时保留其固有的结构。
缺乏用于结构化数据模式的有效查询机制。
SQL 样式的查询与 GraphIt 的图查询之间的桥梁缺失。

本规范通过引入补充 GraphIt 现有功能的结构化数据层来解决这些差距。通过原生支持结构化数据，GraphIt 可以：
作为结构化和非结构化数据分析的统一平台。
实现跨图关系和表格数据的混合查询。
...
1. **NLP-to-Structured-Query 服务**
   ...
   与现有的 GraphIt NLP 功能集成。
   
   模块: graphit-flow/graphit/query/nlp_query/cassandra
...
3. **对象提取模块** ✅ **[完成]**
   ...
   从非结构化源识别和提取结构化对象。
   维护溯源信息和置信度分数。
   注册一个配置处理程序（例如：graphit-flow/graphit/prompt/template/service.py），以接收配置数据并解码模式信息。
   接收对象并将其解码为 ExtractedObject 对象，并通过 Pulsar 队列进行传递。
   注意：存在于 `graphit-flow/graphit/extract/object/row/` 的现有代码。这是一个之前的尝试，需要进行重大重构，因为它不符合当前的 API。如果有用，可以使用它，否则从头开始。
   需要一个命令行界面：`kg-extract-objects`

   模块: graphit-flow/graphit/extract/kg/objects/
...
4. **结构化存储写入模块** ✅ **[完成]**
   ...
   **实现说明**:
   位于 `graphit-flow/graphit/storage/objects/cassandra/` 的现有代码已过时，不符合当前的 API。
   应该参考 `graphit-flow/graphit/storage/triples/cassandra` 作为工作存储处理器的示例。
   在决定重构或重写之前，需要评估现有代码以查找任何可重用组件。

   模块：graphit-flow/graphit/storage/objects/cassandra
...
5. **结构化查询服务** ✅ **[完成]**
   ...
   模块：graphit-flow/graphit/query/objects/cassandra
...
7. **结构化数据摄取服务**
   ...
   模块：graphit-flow/graphit/decoding/structured
...
8. **对象嵌入服务**
   ...
   模块：graphit-flow/graphit/embeddings/object_embeddings/qdrant

### 数据模型

#### 模式存储机制

模式存储在 GraphIt 的配置系统中使用以下结构：
...
遵循现有约定 - 这些只是新的处理模块。
除了 graphit-base 中的模式项之外，所有内容都位于 graphit-flow 包中。
