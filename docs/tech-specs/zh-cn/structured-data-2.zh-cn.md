...
本规范解决了在 GraphIt 结构化数据集成初始实施过程中发现的问题和差距，如 `structured-data.md` 中所述。
...
**第一阶段：嵌入计算** (`graphit-flow/graphit/embeddings/row_embeddings/`) - 消耗 `ExtractedObject`，通过嵌入服务计算嵌入，输出 `RowEmbeddings`
**第二阶段：嵌入存储** (`graphit-flow/graphit/storage/row_embeddings/qdrant/`) - 消耗 `RowEmbeddings`，将向量写入 Qdrant

Cassandra 行写入器是一个独立的并行消费者：

**Cassandra 行写入器** (`graphit-flow/graphit/storage/rows/cassandra`) - 消耗 `ExtractedObject`，将行写入 Cassandra
...
| 阶段 | 模块 | 入口点 |
|-------|--------|-------------|
| 阶段 1 | `graphit-flow/graphit/embeddings/row_embeddings/` | `row-embeddings` |
| 阶段 2 | `graphit-flow/graphit/storage/row_embeddings/qdrant/` | `row-embeddings-write-qdrant` |
...
模块：`graphit-flow/graphit/query/row_embeddings/qdrant`
...
| 组件 | 位置 |
|-----------|----------|
| 调度器 | `graphit-flow/graphit/gateway/dispatch/row_embeddings_query.py` |
...
| 组件 | 位置 | 行数 | 描述 |
|-----------|----------|-------|-------------|
| 查询服务 | `graphit-flow/graphit/query/objects/cassandra/service.py` | ~740 | 整体式：GraphQL 模式生成、过滤器解析、Cassandra 查询、请求处理 |
| 写入器 | `graphit-flow/graphit/storage/objects/cassandra/write.py` | ~540 | 每个模式的表创建、二级索引、插入/删除 |
...
**写入器 (Writer)** (`graphit-flow/graphit/storage/rows/cassandra/`):
使用单个 `rows` 表，而不是每个模式的表
写入 N 个副本到每行（每个索引一个）
注册到 `row_partitions` 表
更简单的表创建（一次性设置）

**查询服务 (Query Service)** (`graphit-flow/graphit/query/rows/cassandra/`):
查询统一的 `rows` 表
...
|--------|---------|
| `graphit-flow/graphit/query/graphql/` | 共享 GraphQL 工具
| `graphit-flow/graphit/query/row_embeddings/qdrant/` | 行嵌入查询 API
| `graphit-flow/graphit/embeddings/row_embeddings/` | 行嵌入计算（第一阶段）
| `graphit-flow/graphit/storage/row_embeddings/qdrant/` | 行嵌入存储（第二阶段）
