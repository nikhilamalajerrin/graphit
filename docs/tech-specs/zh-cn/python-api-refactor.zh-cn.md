...
本规范描述了 GraphIt Python API 客户端库的全面重构，旨在实现与 API 网关的功能对齐，并添加对现代实时通信模式的支持。
...
Python API (`graphit-base/graphit/api/`) 是一个仅支持 REST 的客户端库，包含以下模块：
...
模块: `graphit-base/graphit/api/api.py`
...
模块：`graphit-base/graphit/api/socket_client.py` (新)
...
模块：`graphit-base/graphit/api/async_socket_client.py` (新)
...
模块：`graphit-base/graphit/api/bulk_client.py` (新)
...
模块：`graphit-base/graphit/api/async_bulk_client.py` (新)
...
模块：`graphit-base/graphit/api/flow.py`
...
模块：`graphit-base/graphit/api/async_flow.py` (新)
...
模块：`graphit-base/graphit/api/metrics.py` (新)
...
模块：`graphit-base/graphit/api/types.py` (已修改)
...
### 实现文件
`graphit-base/graphit/api/` - Python API 源代码
`graphit-flow/graphit/gateway/` - 网关源代码
`graphit-flow/graphit/gateway/dispatch/mux.py` - WebSocket 多路复用参考实现
