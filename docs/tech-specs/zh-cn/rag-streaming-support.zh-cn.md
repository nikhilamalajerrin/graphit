...
- **一致的实时体验**: 在所有 GraphIt 服务中提供相同的时间体验
...
- 与其他 GraphIt 服务相比，体验不一致

本规范通过为 GraphRAG 和 DocumentRAG 添加实时支持来解决这些问题。 通过实现分块响应，GraphIt 可以：
- 在所有查询类型中提供一致的实时体验
- 减少 RAG 查询的感知延迟
- 允许长时间运行查询的更好进度反馈
- 客户端应用程序支持实时显示
...
1. **PromptClient 实时** (已实现)
   ...
   模块： `graphit-base/graphit/base/prompt_client.py`
...
2. **GraphRAG 服务** (需要传递实时参数)
   ...
   - `graphit-flow/graphit/retrieval/graph_rag/rag.py` (处理器)
   - `graphit-base/graphit/schema/graph_rag.py` (Request 模式)
   - `graphit-flow/graphit/gateway/dispatch/graph_rag.py` (网关)
...
3. **DocumentRAG 服务** (需要传递实时参数)
   ...
   - `graphit-flow/graphit/retrieval/document_rag/rag.py` (处理器)
   - `graphit-base/graphit/schema/document_rag.py` (Request 模式)
   - `graphit-flow/graphit/gateway/dispatch/document_rag.py` (网关)
...
- Agent 实时： `graphit-flow/graphit/agent/react/agent_manager.py`
- PromptClient 实时： `graphit-base/graphit/base/prompt_client.py`
