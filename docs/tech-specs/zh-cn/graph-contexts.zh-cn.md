...
本规范描述了 GraphIt 核心图结构体的变更，以符合 RDF 1.2 标准并支持完整的 RDF 数据集语义。 这对 2.x 版本系列是一个破坏性变更。
...
`graphit-base/graphit/schema/core/primitives.py` 中的当前 `Value` 类可以表示：
- URI 节点 (`is_uri=True`)
- 原始值 (`is_uri=False`)
...
- **模式原始数据类型** (`graphit-base/graphit/schema/core/primitives.py`)
  - `Value` → `Term` 重命名。
  - 具有类型区分器的新的 `Term` 结构。
  - `Triple` 获得 `g` 字段以进行图上下文。

- **消息翻译器** (`graphit-base/graphit/messaging/translators/`)
