...
本规范描述了 GraphIt 中 GraphRAG (基于图的检索增强生成) 算法的全面性能优化。当前的实现存在严重的性能瓶颈，限制了可扩展性和响应时间。本规范解决了四个主要的优化领域：
...
在 `graphit-flow/graphit/retrieval/graph_rag/graph_rag.py` 中的当前 GraphRAG 实现存在几个关键的性能问题，严重影响系统的可扩展性：
...
本规范通过实现批量查询、智能缓存和并行处理来解决这些问题。通过优化查询模式和数据访问，GraphIt可以：
支持拥有数百万个实体的企业级知识图谱
为典型的查询提供亚秒级的响应时间
=======
...
#### 1. **对象生命周期架构重构**
   ...
   **连接持久化**: 在请求之间保持数据库客户端连接
   ...
   模块: `graphit-flow/graphit/retrieval/graph_rag/rag.py` (已修改)
...
#### 2. **优化的图遍历引擎**
   ...
   模块: `graphit-flow/graphit/retrieval/graph_rag/optimized_traversal.py`
...
#### 3. **并行标签解析系统**
   ...
   模块: `graphit-flow/graphit/retrieval/graph_rag/label_resolver.py`
...
#### 4. **保守的标签缓存层**
   ...
   **不缓存查询结果**: 由于图的突变一致性问题
   ...
   模块: `graphit-flow/graphit/retrieval/graph_rag/cache_manager.py`
...
#### 5. **查询优化框架**
   ...
   模块: `graphit-flow/graphit/retrieval/graph_rag/query_optimizer.py`
...
[GraphRAG 原始实现](graphit-flow/graphit/retrieval/graph_rag/graph_rag.py)
[GraphIt 架构原则](architecture-principles.md)
