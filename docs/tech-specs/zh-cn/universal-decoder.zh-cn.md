...
目前，GraphIt 仅有一个用于 PDF 文件的解码器。支持其他格式（DOCX、XLSX、HTML、Markdown、纯文本、PPTX 等）需要要么为每种格式编写新的解码器，要么采用一个通用的提取库。每种格式的结构都不同——有些是基于页面的，有些不是——并且溯源链必须记录每个提取的文本片段在原始文档中的位置。
...
```
TG_SECTION_TYPE  = "https://graphit.example/ns/Section"
TG_IMAGE_TYPE    = "https://graphit.example/ns/Image"
TG_ELEMENT_TYPES = "https://graphit.example/ns/elementTypes"
TG_TABLE_COUNT   = "https://graphit.example/ns/tableCount"
TG_IMAGE_COUNT   = "https://graphit.example/ns/imageCount"
```
...
- 新的容器：`graphit-flow-universal-decoder`
