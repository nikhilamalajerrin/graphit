...
允许 MCP 工具配置指定可选的承载令牌，用于对受保护的 MCP 服务器进行身份验证。 这允许 GraphIt 安全地调用托管在需要身份验证的服务器上的 MCP 工具，而无需修改代理或工具调用接口。
...
5. **服务级别身份验证：** 仅对 GraphIt 服务进行身份验证，而不是对单个用户进行身份验证。
...
**✅ 适用场景：**
单租户 GraphIt 部署。
服务到服务身份验证（GraphIt → MCP 服务器）。
开发和测试环境。
GraphIt 系统访问的内部 MCP 工具。
...
**示例适用场景：**
单个组织的 GraphIt 部署，其中所有员工都使用相同的内部 MCP 工具（例如，公司数据库查询）。MCP 服务器需要身份验证以防止外部访问，但所有内部用户都具有相同的访问级别。

**示例不适用场景：**
一个多租户 GraphIt SaaS 平台，其中租户 A 和租户 B 需要访问各自隔离的 MCP 服务器，并使用不同的凭据。此功能不支持按租户管理令牌。

### 相关组件
**graphit-flow/graphit/agent/mcp_tool/service.py**: MCP 工具调用服务。
**graphit-cli/graphit/cli/set_mcp_tool.py**: 用于创建/更新 MCP 配置的 CLI 工具。
**graphit-cli/graphit/cli/show_mcp_tools.py**: 用于显示 MCP 配置的 CLI 工具。
...
3. **CLI 支持：** `gr-set-mcp-tool` 必须接受一个可选的 `--auth-token` 参数。
4. **令牌显示：** `gr-show-mcp-tools` 必须指示是否配置了 auth-token（出于安全考虑，已屏蔽）。
...
1. **配置存储**: 用户运行 `gr-set-mcp-tool --id my-tool --tool-url http://server/api --auth-token xyz123`
...
**文件**: `graphit-flow/graphit/agent/mcp_tool/service.py`
...
**文件**: `graphit-cli/graphit/cli/set_mcp_tool.py`
...
**文件**: `graphit-cli/graphit/cli/show_mcp_tools.py`
...
#### 组件 4：文档
**文件**: `docs/cli/gr-set-mcp-tool.md`
...
### 第四阶段：更新文档
[ ] 在 `gr-set-mcp-tool.md` 中记录 `--auth-token` 参数
[ ] 添加安全注意事项部分
[ ] 提供示例用法
...
| `gr-set-mcp-tool.md` | 修改 | 各种 | 文档新的参数 |
...
### 用户文档
[ ] 使用 `--auth-token` 参数更新 `gr-set-mcp-tool.md`
[ ] 添加安全注意事项部分
[ ] 提供使用 bearer 令牌的示例
[ ] 记录令牌存储的影响
