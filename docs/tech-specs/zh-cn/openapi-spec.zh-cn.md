...
创建一个全面的、模块化的 OpenAPI 3.1 规范，用于 GraphIt REST API 网关，该规范应：
记录所有 REST 接口
...
API 由以下内容定义：
**消息转换器**: `graphit-base/graphit/messaging/translators/*.py`
**分派器管理器**: `graphit-flow/graphit/gateway/dispatch/manager.py`
**端点管理器**: `graphit-flow/graphit/gateway/endpoint/manager.py`
...
对于 `graphit-base/graphit/messaging/translators/` 中的每个翻译器：
...
翻译人员：`graphit-base/graphit/messaging/translators/`
调度映射：`graphit-flow/graphit/gateway/dispatch/manager.py`
终端路由：`graphit-flow/graphit/gateway/endpoint/manager.py`
