...
GraphIt 系统使用**基于流的架构**进行动态队列解析，该架构本质上支持多租户：
...
**文件：** `graphit-base/graphit/base/async_processor.py`
**行：** 260-264
...
**文件：** `graphit-flow/graphit/config/service/service.py`
**行：** 276-279
...
**文件：** `graphit-base/graphit/base/cassandra_config.py`
...
**文件：** `graphit-flow/graphit/cores/service.py`
...
**文件：** `graphit-flow/graphit/librarian/service.py`
...
**文件：** `graphit-flow/graphit/librarian/collection_manager.py`
...
**文件:** `graphit-flow/graphit/tables/library.py`
...
#### 变更 11：更新集合模式 - 移除时间戳
**文件:** `graphit-base/graphit/schema/services/collection.py`
...
1. **三元组查询服务** (`graphit-flow/graphit/query/triples/cassandra/service.py:65`)
   使用 `keyspace=query.user`
2. **对象查询服务** (`graphit-flow/graphit/query/objects/cassandra/service.py:479`)
   使用 `keyspace=self.sanitize_name(user)`
3. **知识图谱直接访问** (`graphit-flow/graphit/direct/cassandra_kg.py:18`)
   默认参数 `keyspace="graphit"`
