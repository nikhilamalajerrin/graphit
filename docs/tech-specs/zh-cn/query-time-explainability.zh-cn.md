...
所有 URI 使用 `urn:graphit:` 命名空间和 UUID：

| 实体 | URI 模式 |
|---|---|
| 会话 | `urn:graphit:session:{uuid}` |
| 检索 | `urn:graphit:prov:retrieval:{uuid}` |
| 选择 | `urn:graphit:prov:selection:{uuid}` |
| 答案 | `urn:graphit:prov:answer:{uuid}` |
| 边选择 | `urn:graphit:prov:edge:{uuid}:{index}` |
...
定义在 `graphit-base/graphit/provenance/namespaces.py`：
