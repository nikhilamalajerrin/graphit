...
Bu özellik, GraphIt'ta açıklanabilirlik verilerini hata ayıklamak ve incelemek için kullanılan CLI araçlarını tanımlar. Bu araçlar, kullanıcıların cevapların nasıl elde edildiğini izlemesini ve kenarlardan kaynak belgelere kadar olan köken zincirini hata ayıklamasını sağlar.

Üç CLI aracı:

1. **`gr-show-document-hierarchy`** - Belge → sayfa → parça → kenar hiyerarşisini gösterir
2. **`gr-list-explain-traces`** - Tüm GraphRAG oturumlarını sorularla birlikte listeler
3. **`gr-show-explain-trace`** - Bir oturum için tam açıklanabilirlik izini gösterir
...
GraphIt'un iki köken sistemi vardır:
...
### Araç 1: gr-show-document-hierarchy
...
| `-u/--api-url` | Ağ geçidi URL'si (varsayılan: `$GRAPHIT_URL`) |
| `-t/--token` | Kimlik doğrulama belirteci (varsayılan: `$GRAPHIT_TOKEN`) |
| `-U/--user` | Kullanıcı Kimliği (varsayılan: `graphit`) |
...
| Dosya | Amaç |
|------|---------|
| `graphit-cli/graphit/cli/show_document_hierarchy.py` | Araç 1 |
| `graphit-cli/graphit/cli/list_explain_traces.py` | Araç 2 |
| `graphit-cli/graphit/cli/show_explain_trace.py` | Araç 3 |

## Değiştirilecek Dosyalar

| Dosya | Değişiklik |
|------|--------|
| `graphit-cli/setup.py` | console_scripts girişlerini ekle |
...
`--token` veya `$GRAPHIT_TOKEN` aracılığıyla token kimlik doğrulaması desteklenir.
...
Mevcut komut satırı örneği: `graphit-cli/graphit/cli/invoke_graph_rag.py`
