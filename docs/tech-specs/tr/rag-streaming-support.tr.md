...
**Tutarlı akış kullanıcı deneyimi**: Tüm GraphIt hizmetlerinde aynı akış deneyimini sağlayın.
...
Diğer GraphIt hizmetlerine kıyasla tutarsız deneyim.

Bu özellik, GraphRAG ve DocumentRAG'a akış desteği ekleyerek bu boşlukları giderir. Token bazlı yanıtları etkinleştirerek, GraphIt şunları yapabilir:
...
1. **PromptClient Akışı** (Zaten uygulandı)
   ...
   Modül: `graphit-base/graphit/base/prompt_client.py`
...
2. **GraphRAG Hizmeti** (Akış parametresi geçişi gereklidir)
   ...
   `graphit-flow/graphit/retrieval/graph_rag/rag.py` (İşlemci)
   `graphit-base/graphit/schema/graph_rag.py` (İstek şeması)
...
3. **DocumentRAG Hizmeti** (Akış parametresi geçişi gereklidir)
   ...
   `graphit-flow/graphit/retrieval/document_rag/rag.py` (İşlemci)
   `graphit-base/graphit/schema/document_rag.py` (İstek şeması)
...
5. `--no-streaming` bayraklarını `gr-invoke-graph-rag` ve `gr-invoke-document-rag`'ye ekleyin (akış varsayılan olarak etkindir, ajan CLI kalıbını takip eder)
...
**Manuel testler**:
`gr-invoke-graph-rag -q "What is machine learning?"` (akış varsayılan olarak etkindir)
`gr-invoke-document-rag -q "Summarize the documents about AI"` (akış varsayılan olarak etkindir)
`gr-invoke-graph-rag --no-streaming -q "..."` (akış dışı modu test edin)
...
Ajan akışı: `graphit-flow/graphit/agent/react/agent_manager.py`
PromptClient akışı: `graphit-base/graphit/base/prompt_client.py`
