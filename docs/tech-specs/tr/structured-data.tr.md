...
Bu özellik, GraphIt'ın yapılandırılmış veri akışlarıyla entegrasyonunu tanımlar ve sistemin, satırlar halinde tablolarda veya nesne depolarında temsil edilebilen verilerle çalışmasını sağlar. Bu entegrasyon, dört birincil kullanım senaryosunu destekler:
...
**Birleşik Veri Erişimi**: GraphIt içinde hem yapılandırılmış hem de yapılandırılmamış verilere erişmek için tek bir arayüz sağlayın.
**Sorunsuz Entegrasyon**: GraphIt'ın grafik tabanlı bilgi gösterimi ile geleneksel yapılandırılmış veri formatları arasındaki sorunsuz uyumluluğu sağlayın.
...
SQL benzeri sorgular ile GraphIt'ın grafik sorguları arasındaki köprünün olmaması.

Bu özellik, GraphIt'ın mevcut yeteneklerini tamamlayan bir yapılandırılmış veri katmanı tanıtarak bu boşlukları giderir. Yapılandırılmış verilere yerel olarak destek sağlayarak, GraphIt şunları yapabilir:
...
1. **NLP'den Yapılandırılmış Sorgu Hizmetine**
   ...
   Mevcut GraphIt NLP yetenekleriyle entegre olur.
   
   Modül: graphit-flow/graphit/query/nlp_query/cassandra
...
3. **Nesne Çıkarma Modülü** ✅ **[TAMAMLANDI]**
   ...
   Yapılandırma verilerini almak ve şema bilgilerini çözmek için bir yapılandırma işleyici (örneğin: graphit-flow/graphit/prompt/template/service.py) kaydeder.
   Nesneleri alır ve bunları Pulsar kuyruğuna teslim etmek üzere ExtractedObject nesnelerine dönüştürür.
   NOT: `graphit-flow/graphit/extract/object/row/` adresinde mevcut kod bulunmaktadır. Bu, önceki bir girişimdir ve mevcut API'lere uymadığından büyük ölçüde yeniden düzenlenmesi gerekecektir. Kullanışlıysa kullanın, değilse sıfırdan başlayın.
   Bir komut satırı arayüzü gerektirir: `kg-extract-objects`

   Modül: graphit-flow/graphit/extract/kg/objects/
...
4. **Yapılandırılmış Depo Yazma Modülü** ✅ **[TAMAMLANDI]**
   ...
   **Uygulama Notları**:
   `graphit-flow/graphit/storage/objects/cassandra/` adresindeki mevcut kod güncel değildir ve mevcut API'lere uygun değildir.
   Çalışan bir depolama işlemcisinin örneği olarak `graphit-flow/graphit/storage/triples/cassandra`'a başvurulmalıdır.
   ...
   Modül: graphit-flow/graphit/storage/objects/cassandra
...
5. **Yapılandırılmış Sorgu Hizmeti** ✅ **[TAMAMLANDI]**
   ...
   Modül: graphit-flow/graphit/query/objects/cassandra
...
7. **Yapılandırılmış Veri Alma Hizmeti**
   ...
   Modül: graphit-flow/graphit/decoding/structured
...
8. **Nesne Gömme Hizmeti**
   ...
   Modül: graphit-flow/graphit/embeddings/object_embeddings/qdrant

### Veri Modelleri

#### Şema Depolama Mekanizması

Şemalar, aşağıdaki yapı kullanılarak GraphIt'ın yapılandırma sisteminde saklanır:
...
Mevcut GraphIt yapılandırma yönetimi ile tutarlı entegrasyon
...
Her şey graphit-flow paketlerinde yer almaktadır, şema öğeleri ise graphit-base'de bulunmaktadır.
