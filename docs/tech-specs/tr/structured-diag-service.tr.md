...
Bu özellik, GraphIt içinde yapılandırılmış verileri teşhis etmek ve analiz etmek için yeni bir kullanılabilir hizmeti tanımlar. Bu hizmet, mevcut `gr-load-structured-data` komut satırı aracından işlevleri ayırır ve bunları istek/yanıt hizmeti olarak sunar, böylece veri türü algılama ve tanımlayıcı oluşturma yeteneklerine programlı erişim sağlar.
...
2. **Tanımlayıcı Oluşturma**: Verilen bir veri örneği ve tür için bir GraphIt yapılandırılmış veri tanımlayıcısı oluşturun.
...
**Doğru Tanımlayıcılar Oluşturma**: Kaynak verileri GraphIt şemalarına doğru bir şekilde eşleyen yapılandırılmış veri tanımlayıcıları oluşturma.
...
Şu anda, `gr-load-structured-data` komutu, yapılandırılmış verileri analiz etmek ve tanımlayıcılar oluşturmak için kapsamlı işlevsellik sağlar. Ancak, bu işlevsellik CLI arayüzüne sıkı bir şekilde bağlıdır ve yeniden kullanılabilirliğini sınırlar.
...
Bu özellik, yapılandırılmış veri teşhisi için özel bir hizmet oluşturarak bu boşlukları giderir. Bu yetenekleri bir hizmet olarak sunarak, GraphIt şunları yapabilir:
...
1. **Teşhis Hizmeti İşlemcisi**
   ...
   Modül: `graphit-flow/graphit/diagnosis/structured_data/service.py`
...
2. **Veri Türü Algılayıcı**
   ...
   Modül: `graphit-flow/graphit/diagnosis/structured_data/type_detector.py`
...
3. **Tanımlayıcı Oluşturucu**
   ...
   Veri alanlarını, istem yanıtları aracılığıyla GraphIt şema alanlarına eşler.

   Modül: `graphit-flow/graphit/diagnosis/structured_data/descriptor_generator.py`
...
Hizmet, GraphIt hizmeti geleneklerini takip edecektir:
...
Hizmet, mevcut GraphIt API'leriyle entegre olacaktır:

Değiştirilen Bileşenler:
`gr-load-structured-data` CLI - Tanılama işlemleri için yeni hizmeti kullanmak üzere yeniden düzenlendi
...
Mevcut GraphIt kimlik doğrulama aracılığıyla erişim kontrolü
...
`gr-load-structured-data` uygulaması: `graphit-cli/graphit/cli/load_structured_data.py`
