...
**Konum**: `graphit-flow/graphit/librarian/blob_store.py`
**İşlemler**:
...
### Önemli Dosyalar
1. `graphit-flow/graphit/librarian/blob_store.py` - BlobStore uygulaması
2. `graphit-flow/graphit/librarian/librarian.py` - BlobStore başlatma
3. `graphit-flow/graphit/librarian/service.py` - Hizmet yapılandırması
4. `graphit-flow/pyproject.toml` - Bağımlılıklar (`minio` paketi)
