...
Bu özellik, GraphIt'taki akış şemaları için yapılandırılabilir parametrelerin uygulanmasını açıklamaktadır. Parametreler, kullanıcıların akış başlatma zamanında işlemci parametrelerini, akış şeması tanımındaki parametre yer tutucularını değiştiren değerler sağlayarak özelleştirmesini sağlar.
...
GraphIt'taki akış şemaları artık, sabit değerler veya parametre yer tutucuları içerebilen işlemci parametrelerini desteklemektedir. Bu, çalışma zamanı özelleştirmesi için bir fırsat yaratır.
...
Parametreli işlemci parametrelerini kullanarak, GraphIt şunları yapabilir:
...
1. **Parametre Şema Tanımı**
   ...
   Modül: graphit-flow/graphit/flow/definition.py
...
>>>>>>> 82edf2d (New md files from RunPod)
   ...
   Modül: graphit-flow/graphit/flow/parameter_resolver.py
...
>>>>>>> 82edf2d (New md files from RunPod)
   ...
   Modül: graphit-flow/graphit/flow/parameter_store.py
...
>>>>>>> 82edf2d (New md files from RunPod)
   ...
   Modül: graphit-flow/graphit/flow/launcher.py
...
>>>>>>> 82edf2d (New md files from RunPod)
   ...
   Modül: graphit-ui/components/flow-parameters/
...
Akış yapılandırma hizmeti (`graphit-flow/graphit/config/service/flow.py`), aşağıdaki geliştirmeleri gerektirmektedir:
