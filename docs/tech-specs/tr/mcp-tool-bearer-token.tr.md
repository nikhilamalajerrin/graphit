...
MCP araçlarının, korumalı MCP sunucularıyla kimlik doğrulaması yapmak için isteğe bağlı taşıyıcı (bearer) token'lar belirtmesine olanak tanıyın. Bu, GraphIt'ın, kimlik doğrulaması gerektiren sunucularda barındırılan MCP araçlarını, aracının veya araç çağrı arayüzlerinin değiştirilmesi olmadan güvenli bir şekilde çağırmasını sağlar.
...
5. **Hizmet seviyesi kimlik doğrulaması:** Bireysel kullanıcılar yerine GraphIt hizmetini doğrular.
6. **Paylaşılan güvenlik bağlamı:** Bir MCP aracının tüm çağrıları aynı kimlik bilgisini kullanır.

### Kullanım Alanı Uygunluğu

**✅ Uygun Kullanım Alanları:**
Tek kiracılı GraphIt dağıtımları
Hizmetten hizmete kimlik doğrulama (GraphIt → MCP Sunucusu)
Geliştirme ve test ortamları
GraphIt sistemi tarafından erişilen dahili MCP araçları
...
**Örnek Uygun Senaryo:**
Tüm çalışanların aynı dahili MCP aracını (örneğin, şirket veritabanı sorgusu) kullandığı tek organizasyonlu bir GraphIt dağıtımı. MCP sunucusunun harici erişimi önlemek için kimlik doğrulama gerektirmesi, ancak tüm dahili kullanıcıların aynı erişim düzeyine sahip olması.

**Örnek Uygun Olmayan Senaryo:**
Kiracı A ve Kiracı B'nin her birinin ayrı kimlik bilgileriyle kendi izole MCP sunucularına erişmesi gereken çok kiracılı bir GraphIt SaaS platformu. Bu özellik, kiracı başına belirteç yönetimi DEĞİLDİR.

### İlgili Bileşenler
**graphit-flow/graphit/agent/mcp_tool/service.py**: MCP aracı çağrı hizmeti
**graphit-cli/graphit/cli/set_mcp_tool.py**: MCP yapılandırmalarını oluşturmak/güncellemek için CLI aracı
**graphit-cli/graphit/cli/show_mcp_tools.py**: MCP yapılandırmalarını görüntülemek için CLI aracı
...
3. **CLI Desteği**: `gr-set-mcp-tool`, isteğe bağlı bir `--auth-token` parametresi KABUL ETMELİDİR.
4. **Belirteç Görüntüleme**: `gr-show-mcp-tools`, kimlik doğrulama belirtecinin yapılandırıldığını GÖSTERMELİDİR (güvenlik için gizlenmiş).
...
1. **Yapılandırma Depolama**: Kullanıcı `gr-set-mcp-tool --id my-tool --tool-url http://server/api --auth-token xyz123`'ı çalıştırır
...
**Dosya**: `graphit-flow/graphit/agent/mcp_tool/service.py`
...
**Dosya**: `graphit-cli/graphit/cli/set_mcp_tool.py`
...
`-u, --api-url` (isteğe bağlı): GraphIt API URL'si
...
**Dosya**: `graphit-cli/graphit/cli/show_mcp_tools.py`
...
#### Bileşen 4: Belgeler
**Dosya**: `docs/cli/gr-set-mcp-tool.md`
...
[ ] `gr-set-mcp-tool.md` içindeki `--auth-token` parametresini belgeleyin
...
| `gr-set-mcp-tool.md` | Değiştirildi | Çeşitli | Yeni parametreyi belgeleyin |
...
**Kullanıcı bağlamı yok**: MCP sunucusu, farklı GraphIt kullanıcıları arasında ayrım yapamaz
...
GraphIt dağıtımınız birden fazla kuruluşu (çok kiracılı) hizmet veriyorsa
...
Her kiracı için ayrı GraphIt örnekleri dağıtın
...
MCP sunucusu, farklı GraphIt kullanıcıları arasında ayrım yapamaz
...
### Kullanıcı Belgeleri
[ ] `gr-set-mcp-tool.md`'ı `--auth-token` parametresiyle güncelleyin
...
[Mevcut MCP Aracı Hizmeti](../graphit-flow/graphit/agent/mcp_tool/service.py)
