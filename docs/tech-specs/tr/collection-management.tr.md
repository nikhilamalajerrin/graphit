...
Bu özellik, açık koleksiyon yönetimi modelini tanımlar. Açık koleksiyon oluşturmayı gerektirerek, GraphIt şunları sağlar:
...
Koleksiyon yönetimi sistemi, mevcut GraphIt altyapısının içinde uygulanacaktır:
...
1. **Koleksiyon Oluşturma** (İki Yol):
   ...
   **Yol A: Kullanıcı Tarafından Başlatılan Oluşturma** `gr-set-collection` aracılığıyla:
...
4. **Meta Veri Güncellemeleri**: Kullanıcılar, koleksiyon oluşturulduktan sonra koleksiyon meta verilerini güncelleyebilir:
   Adı, açıklamasını ve etiketleri `gr-set-collection` aracılığıyla güncelleyin
...
5. **Açık Silme**: Kullanıcılar, koleksiyonları `gr-delete-collection` aracılığıyla siler:
...
Gerekli işlemler:
**Koleksiyon Oluşturma**: `gr-set-collection` aracılığıyla kullanıcı işlemi VEYA belge gönderimi üzerine otomatik
...
Koleksiyonlar, GraphIt'ta birden çok depolama arka uçunda bulunur:
...
**Koleksiyon Oluşturma/Güncelleme**: Yeni bir koleksiyon oluşturma veya mevcut meta verileri `gr-set-collection` aracılığıyla güncelleme
...
Uygulama, hizmet entegrasyonu ve CLI komut yapısı için mevcut GraphIt desenlerini takip edecektir.
...
1. **GraphIt Sınıfı Yeniden Düzenlemesi** (`graphit/direct/cassandra.py`):
...
2. **Depolama Yazarı Güncellemeleri** (`graphit/storage/triples/cassandra/write.py`):
   (kullanıcı, koleksiyon) başına bir bağlantı yerine, kullanıcı başına tek bir GraphIt bağlantısı sürdürün
   Ekleme işlemlerine koleksiyonu iletin
   Daha az bağlantıyla daha verimli kaynak kullanımı

3. **Sorgu Hizmeti Güncellemeleri** (`graphit/query/triples/cassandra/service.py`):
   Kullanıcı başına tek bir GraphIt bağlantısı
...
Koleksiyon yönetimi işlemleri, yetkisiz erişimi veya koleksiyonların silinmesini önlemek için uygun yetkilendirme gerektirir. Erişim kontrolü, mevcut GraphIt güvenlik modelleriyle uyumlu olacaktır.
...
1. **Librarian Koleksiyon Yönetimi Hizmeti** (`graphit-flow/graphit/librarian/collection_manager.py`)
...
2. **Koleksiyon Meta Veri Şeması** (`graphit-base/graphit/schema/services/collection.py`)
...
3. **Depolama Yönetimi Şeması** (`graphit-base/graphit/schema/services/storage.py`)
...
4. **Cassandra 4-Tablo Şeması** (`graphit-flow/graphit/direct/cassandra_kg.py`)
