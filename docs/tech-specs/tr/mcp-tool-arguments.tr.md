...
### İlgili Bileşenler
**graphit-flow/agent/react/service.py**: Araç yapılandırması yükleme ve AgentManager oluşturma.
**graphit-flow/agent/react/tools.py**: McpToolImpl uygulaması.
**graphit-flow/agent/react/agent_manager.py**: Araç argümanlarıyla birlikte istem oluşturma.
**graphit-cli**: MCP araç yönetimi için komut satırı araçları.
...
5. **Komut Satırı Desteği**: Mevcut `gr-invoke-mcp-tool` komut satırı araçları, argümanları destekler (zaten uygulanmıştır).
...
**CLI Entegrasyonu**: `gr-invoke-mcp-tool`'un yeni argüman yapılandırmalı MCP araçlarıyla çalıştığını test edin.
...
[İstem Şablonu Araç Uygulaması](./graphit-flow/graphit/agent/react/service.py#L114-129)
[Mevcut MCP Araç Uygulaması](./graphit-flow/graphit/agent/react/tools.py#L58-86)
