...
STRUCTURED_DATA.md spesifikasyonuna dayanarak, bu belge, GraphIt'ta yapılandırılmış veri yeteneklerini desteklemek için gerekli olan Pulsar şema eklemelerini ve değişikliklerini önermektedir.
...
`graphit-flow/graphit/decoding/structured` - StructuredDataSubmission'ı kullanır
`graphit-flow/graphit/query/nlp_query/cassandra` - NLP sorgu şemalarını kullanır
`graphit-flow/graphit/query/objects/cassandra` - Yapılandırılmış sorgu şemalarını kullanır
`graphit-flow/graphit/extract/object/row/` - Chunk'ı tüketir, ExtractedObject üretir
`graphit-flow/graphit/storage/objects/cassandra` - Rows şemasını kullanır
`graphit-flow/graphit/embeddings/object_embeddings/qdrant` - Nesne gömme şemalarını kullanır
