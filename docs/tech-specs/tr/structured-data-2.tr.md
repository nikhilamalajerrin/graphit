...
Bu özellik, GraphIt'ın yapılandırılmış veri entegrasyonunun ilk uygulamasının ilk aşamalarında tespit edilen sorunları ve eksiklikleri ele almaktadır, bu da `structured-data.md`'da açıklanmıştır.
...
**1. Aşama: Gömü hesaplama** (`graphit-flow/graphit/embeddings/row_embeddings/`) - `ExtractedObject`'i tüketir, gömü hizmeti aracılığıyla gömüleri hesaplar, `RowEmbeddings` çıktısını üretir.
**2. Aşama: Gömü depolama** (`graphit-flow/graphit/storage/row_embeddings/qdrant/`) - `RowEmbeddings`'i tüketir, vektörleri Qdrant'a yazar.

Cassandra satır yazarı, ayrı bir paralel tüketici işlemidir:

**Cassandra satır yazarı** (`graphit-flow/graphit/storage/rows/cassandra`) - `ExtractedObject`'i tüketir, satırları Cassandra'ya yazar.
...
|-------|--------|-------------|
| Aşama 1 | `graphit-flow/graphit/embeddings/row_embeddings/` | `row-embeddings` |
| Aşama 2 | `graphit-flow/graphit/storage/row_embeddings/qdrant/` | `row-embeddings-write-qdrant` |
...
Modül: `graphit-flow/graphit/query/row_embeddings/qdrant`
...
| Bileşen | Konum |
|-----------|----------|
| Yönlendirici | `graphit-flow/graphit/gateway/dispatch/row_embeddings_query.py` |
...
Komut: `gr-invoke-row-embeddings`
...
|-----------|----------|-------|-------------|
| Sorgu Servisi | `graphit-flow/graphit/query/objects/cassandra/service.py` | ~740 | Tek bir blok: GraphQL şema oluşturma, filtre ayrıştırma, Cassandra sorguları, istek işleme |
| Yazıcı | `graphit-flow/graphit/storage/objects/cassandra/write.py` | ~540 | Şema başına tablo oluşturma, ikincil indeksler, ekleme/silme |
...
**Yazıcı** (`graphit-flow/graphit/storage/rows/cassandra/`):
...
**Sorgu Hizmeti** (`graphit-flow/graphit/query/rows/cassandra/`):
...
| Modül | Amaç |
|--------|---------|
| `graphit-flow/graphit/query/graphql/` | Paylaşılan GraphQL yardımcı programları |
| `graphit-flow/graphit/query/row_embeddings/qdrant/` | Satır gömme sorgu API'si |
| `graphit-flow/graphit/embeddings/row_embeddings/` | Satır gömme hesaplama (1. Aşama) |
| `graphit-flow/graphit/storage/row_embeddings/qdrant/` | Satır gömme depolama (2. Aşama) |
