...
Bu özellik, GraphIt'ta LLM yanıtları için akış desteğinin uygulanmasını tanımlar. Akış, üretilen token'ların LLM tarafından üretildikleri anda gerçek zamanlı olarak iletilmesini sağlar, böylece tamamlanmış bir yanıtın oluşturulmasını beklemez.
...
Mevcut şema (`graphit-base/graphit/schema/services/llm.py`):
...
##### LLM Şeması (`graphit-base/graphit/schema/services/llm.py`)
...
##### İstek Şeması (`graphit-base/graphit/schema/services/prompt.py`)
...
**LlmClient Güncellemeleri** (`graphit-base/graphit/clients/llm_client.py`):
...
**PromptClient Güncellemeleri** (`graphit-base/graphit/base/prompt_client.py`):
...
**Modül:** `graphit-vertexai/graphit/model/text_completion/vertexai/llm.py`
...
|----------|---------|-------------------|
| OpenAI | graphit-flow | Tam (yerel akış API'si) |
| Claude/Anthropic | graphit-flow | Tam (yerel akış API'si) |
| Ollama | graphit-flow | Tam (yerel akış API'si) |
| Cohere | graphit-flow | Tam (yerel akış API'si) |
| Mistral | graphit-flow | Tam (yerel akış API'si) |
| Azure OpenAI | graphit-flow | Tam (yerel akış API'si) |
| Google AI Studio | graphit-flow | Tam (yerel akış API'si) |
| VertexAI | graphit-vertexai | Tam (2. Aşama) |
| Bedrock | graphit-bedrock | Tam (yerel akış API'si) |
| LM Studio | graphit-flow | Tam (OpenAI ile uyumlu) |
| LlamaFile | graphit-flow | Tam (OpenAI ile uyumlu) |
| vLLM | graphit-flow | Tam (OpenAI ile uyumlu) |
| TGI | graphit-flow | Belirlenecek |
| Azure | graphit-flow | Belirlenecek |
...
Mevcut LLM şeması: `graphit-base/graphit/schema/services/llm.py`
Mevcut istem şeması: `graphit-base/graphit/schema/services/prompt.py`
Mevcut ajan şeması: `graphit-base/graphit/schema/services/agent.py`
LLM hizmeti tabanı: `graphit-base/graphit/base/llm_service.py`
VertexAI sağlayıcısı: `graphit-vertexai/graphit/model/text_completion/vertexai/llm.py`
Ağ Geçidi API'si: `graphit-base/graphit/api/`
CLI araçları: `graphit-cli/graphit/cli/`
