...
Hem GraphRAG hem de Agent, GraphIt'e özgü alt türlere sahip olan PROV-O'yu temel ontoloji olarak kullanır:
...
|---------|-------------|-----------|
| `tg:GraphRagQuestion` | `urn:graphit:question:{uuid}` | Bilgi grafiği RAG |
| `tg:DocRagQuestion` | `urn:graphit:docrag:{uuid}` | Belge/parça RAG |
| `tg:AgentQuestion` | `urn:graphit:agent:{uuid}` | ReAct ajanı |
...
**Dosya:** `graphit-base/graphit/schema/services/agent.py`
...
**Dosya:** `graphit-base/graphit/messaging/translators/agent.py`
...
**Dosya:** `graphit-flow/graphit/agent/react/service.py`
...
**Dosya:** `graphit-base/graphit/provenance/agent.py`
...
**Dosya:** `graphit-base/graphit/provenance/namespaces.py`
...
| Dosya | Değişiklik |
|------|--------|
| `graphit-base/graphit/schema/services/agent.py` | AgentRequest'e session_id ve collection eklendi |
| `graphit-base/graphit/messaging/translators/agent.py` | Yeni alanlar için çevirici güncellendi |
| `graphit-base/graphit/provenance/namespaces.py` | Varlık türleri, ajan önişlemleri ve Document RAG önişlemleri eklendi |
| `graphit-base/graphit/provenance/triples.py` | GraphRAG üçlü oluşturucularına TG türleri eklendi, Document RAG üçlü oluşturucuları eklendi |
| `graphit-base/graphit/provenance/uris.py` | Document RAG URI oluşturucuları eklendi |
| `graphit-base/graphit/provenance/__init__.py` | Yeni türler, önişlemler ve Document RAG fonksiyonları dışa aktarıldı |
| `graphit-base/graphit/schema/services/retrieval.py` | DocumentRagResponse'a explain_id ve explain_graph eklendi |
| `graphit-base/graphit/messaging/translators/retrieval.py` | Açıklanabilirlik alanları için DocumentRagResponseTranslator güncellendi |
| `graphit-flow/graphit/agent/react/service.py` | Açıklanabilirlik üretici + kayıt mantığı eklendi |
| `graphit-flow/graphit/retrieval/document_rag/document_rag.py` | Açıklanabilirlik geri çağırması eklendi ve kaynak üçlüleri yayıldı |
| `graphit-flow/graphit/retrieval/document_rag/rag.py` | Açıklanabilirlik üretici eklendi ve geri çağırma ile bağlandı |
| `graphit-cli/graphit/cli/show_explain_trace.py` | Ajan izleme türleri işlendi |
| `graphit-cli/graphit/cli/list_explain_traces.py` | Ajan oturumları, GraphRAG ile birlikte listelendi |

## Oluşturulan Dosyalar

| Dosya | Amaç |
|------|---------|
| `graphit-base/graphit/provenance/agent.py` | Ajan özel üçlü oluşturucuları |

## CLI Güncellemeleri

**Algılama:** Hem GraphRAG hem de Ajan Soruları `tg:Question` türündedir. Aşağıdakilerle ayırt edilir:
1. URI kalıbı: `urn:graphit:agent:` vs `urn:graphit:question:`
