...
Tüm URI'ler, UUID'lerle birlikte `urn:graphit:` ad alanını kullanır:

| Varlık | URI Kalıbı |
|--------|-------------|
| Oturum | `urn:graphit:session:{uuid}` |
| Alma | `urn:graphit:prov:retrieval:{uuid}` |
| Seçim | `urn:graphit:prov:selection:{uuid}` |
| Cevap | `urn:graphit:prov:answer:{uuid}` |
| Kenar Seçimi | `urn:graphit:prov:edge:{uuid}:{index}` |
...
`graphit-base/graphit/provenance/namespaces.py` içinde tanımlanmıştır:
...
| Dosya | Amaç |
|------|---------|
| `graphit-base/graphit/provenance/uris.py` | URI oluşturucular |
| `graphit-base/graphit/provenance/namespaces.py` | RDF ad alanı sabitleri |
| `graphit-base/graphit/provenance/triples.py` | Üçlü oluşturucular |
| `graphit-base/graphit/schema/services/retrieval.py` | GraphRagResponse şeması |
| `graphit-flow/graphit/retrieval/graph_rag/graph_rag.py` | URI korumasıyla temel GraphRAG |
| `graphit-flow/graphit/retrieval/graph_rag/rag.py` | Kütüphaneci entegrasyonlu hizmet |
| `graphit-flow/graphit/query/triples/cassandra/service.py` | Tırnaklı üçlü sorgu desteği |
| `graphit-cli/graphit/cli/invoke_graph_rag.py` | Açıklanabilirlik gösterimiyle CLI |
