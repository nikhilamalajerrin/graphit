...
Bu teknik özellik, GraphIt'e büyük belgelerin yüklenmesi sırasında ortaya çıkan ölçeklenebilirlik ve kullanıcı deneyimi sorunlarını ele almaktadır. Mevcut mimari, belge yüklemeyi tek bir atomik işlem olarak ele almaktadır, bu da boru hattının çeşitli noktalarında bellek yüklenmesine neden olmakta ve kullanıcılara herhangi bir geri bildirim veya kurtarma seçeneği sunmamaktadır.
...
Önemli dosyalar:
REST/WebSocket girişi: `graphit-flow/graphit/gateway/service.py`
Librarian çekirdeği: `graphit-flow/graphit/librarian/librarian.py`
Blob depolama: `graphit-flow/graphit/librarian/blob_store.py`
Cassandra tabloları: `graphit-flow/graphit/tables/library.py`
API şeması: `graphit-base/graphit/schema/services/library.py`
...
**Dosya:** `graphit-flow/graphit/librarian/blob_store.py`
...
Python SDK'sı (`graphit-base/graphit/api/library.py`), parçalı yüklemeleri şeffaf bir şekilde işlemelidir. Kamu arayüzü değişmeden kalır:
...
**`gr-add-library-document`** değişmeden çalışmaya devam ediyor:
...
`gr-load-pdf` - kullanımdan kaldırıldı, `gr-add-library-document`'i kullanın.
`gr-load-text` - kullanımdan kaldırıldı, `gr-add-library-document`'i kullanın.
