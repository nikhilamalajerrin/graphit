---
layout: default
title: "GraphIt Logging Strategy"
parent: "Tech Specs"
---

# GraphIt Logging Strategy

## Overview

GraphIt uses Python's built-in `logging` module for all logging operations, with centralized configuration and optional Loki integration for log aggregation. This provides a standardized, flexible approach to logging across all components of the system.
...
All logging configuration is managed by `graphit.base.logging` module, which provides:
...
- `logger`: Module name (e.g., `graphit.gateway.service`, `graphit.agent.react.service`)
...
Already included in `graphit-base/pyproject.toml` and `requirements.txt`.
