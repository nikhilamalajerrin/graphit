...
This specification describes changes to GraphIt's core graph primitives to
...
The current `Value` class in `graphit-base/graphit/schema/core/primitives.py`
can represent:
...
#### This Repository (graphit)

- **Schema primitives** (`graphit-base/graphit/schema/core/primitives.py`)
  - Value → Term rename
  - New Term structure with type discriminator
  - Triple gains `g` field for graph context

- **Message translators** (`graphit-base/graphit/messaging/translators/`)
