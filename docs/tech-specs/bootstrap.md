...
against a GraphIt deployment — replacing the dedicated
`gr-init-graphit` container with a long-running processor that
...
ship in the `graphit-flow` package; enterprise and third-party
initialisers can be loaded by dotted path without any core code
change.

## Motivation

The existing `gr-init-graphit` is a one-shot CLI run in its own
...
The following initialisers ship in `graphit.bootstrap.initialisers`
...
- Running the bootstrapper as a processor-group entry replaces the
  previous `gr-init-graphit` container. The bootstrapper is also
