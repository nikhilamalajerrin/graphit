...
This specification describes a new invokable service for diagnosing and analyzing structured data within GraphIt. The service extracts functionality from the existing `gr-load-structured-data` command-line tool and exposes it as a request/response service, enabling programmatic access to data type detection and descriptor generation capabilities.
...
2. **Descriptor Generation**: Generate a GraphIt structured data descriptor for a given data sample and type
...
- **Generate Accurate Descriptors**: Produce structured data descriptors that accurately map source data to GraphIt schemas
...
Currently, the `gr-load-structured-data` command provides comprehensive functionality for analyzing structured data and generating descriptors. However, this functionality is tightly coupled to the CLI interface, limiting its reusability.
...
This specification addresses these gaps by creating a dedicated service for structured data diagnosis. By exposing these capabilities as a service, GraphIt can:
...
1. **Diagnostic Service Processor**
   ...
   Module: `graphit-flow/graphit/diagnosis/structured_data/service.py`
...
2. **Data Type Detector**
   ...
   Module: `graphit-flow/graphit/diagnosis/structured_data/type_detector.py`
...
3. **Descriptor Generator**
   ...
   - Maps data fields to GraphIt schema fields through prompt responses

   Module: `graphit-flow/graphit/diagnosis/structured_data/descriptor_generator.py`
...
The service will follow GraphIt service conventions:
...
The service will integrate with existing GraphIt APIs:

Modified Components:
- `gr-load-structured-data` CLI - Refactored to use the new service for diagnosis operations
...
- Access control through existing GraphIt authentication
...
- `gr-load-structured-data` implementation: `graphit-cli/graphit/cli/load_structured_data.py`
