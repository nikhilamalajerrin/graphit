...
Ruhusu usanidi wa zana za MCP kubainisha simu za hiari za bearer kwa uthibitishaji na seva za MCP zilizolindwa. Hii inaruhusu GraphIt kuita zana za MCP zilizohifadhiwa kwenye seva ambazo zinahitaji uthibitishaji, bila kubadilisha wakala au interfaces za kutumia zana.
...
5. **Uthibitishaji wa huduma**: Inathibitisha huduma ya GraphIt, sio watumiaji binafsi.
6. **Muktadha wa usalama unaoshirikiwa**: Matumizi yote ya zana ya MCP hutumia anwani sawa.

### Ufaa wa Matumizi

**✅ Matumizi Yanayofaa:**
Uwekaji wa GraphIt kwa watumiaji mmoja.
Uthibitishaji kutoka kwa huduma hadi huduma (GraphIt → Seva ya MCP).
Mazingira ya maendeleo na majaribio.
Zana za ndani za MCP zinazopatikana na mfumo wa GraphIt.
...
Uwekaji wa GraphIt wa shirika moja ambamo wafanyakazi wote hutumia zana sawa ya ndani ya MCP (k.m., utafutaji wa hifadhi ya kampuni). Seva ya MCP inahitaji uthibitishaji ili kuzuia ufikiaji wa nje, lakini watumiaji wote wa ndani wana kiwango sawa cha ufikiaji.

**Mfano wa Matumizi Yasiyofaa:**
Jukwaa la SaaS la GraphIt la watumiaji wengi ambamo Mhakiki A na Mhakiki B kila mmoja anahitaji kufikia seva zao zilizotenganishwa za MCP na anwani tofauti. Kipengele hiki hakitumii usimamizi wa anwani wa kila mhakiki.

### Vipengele Vinavyohusiana
**graphit-flow/graphit/agent/mcp_tool/service.py**: Huduma ya utekelezaji wa zana ya MCP.
**graphit-cli/graphit/cli/set_mcp_tool.py**: Zana ya CLI ya kuunda/kusasisha mipangilio ya MCP.
**graphit-cli/graphit/cli/show_mcp_tools.py**: Zana ya CLI ya kuonyesha mipangilio ya MCP.
...
3. **Usaidizi wa CLI**: `gr-set-mcp-tool` INAWEZA kukubali parameter ya `--auth-token`.
4. **Uonyesho wa Ishara**: `gr-show-mcp-tools` INAWEZA kuonyesha wakati ishara ya uthibitishaji imewekwa (imeficha kwa usalama).
...
1. **Hifadhi ya Usanidi:** Mtumiaji huanzisha `gr-set-mcp-tool --id my-tool --tool-url http://server/api --auth-token xyz123`
...
**Faili:** `graphit-flow/graphit/agent/mcp_tool/service.py`
...
**Faili**: `graphit-cli/graphit/cli/set_mcp_tool.py`
...
`-u, --api-url` (ya hiari): URL ya API ya GraphIt
...
**Faili**: `graphit-cli/graphit/cli/show_mcp_tools.py`
...
#### Sehemu ya 4: Nyaraka
**Faili**: `docs/cli/gr-set-mcp-tool.md`
...
[ ] Andika `--auth-token` katika `gr-set-mcp-tool.md`
...
| `gr-set-mcp-tool.md` | Imebadilishwa | Mbalimbali | Andika parameter mpya |
...
**Hakuna muktadha wa mtumiaji**: Seva ya MCP haiwezi kutofautisha kati ya watumiaji tofauti wa GraphIt
...
Weka mifumo tofauti ya GraphIt kwa kila mtoa huduma
...
Seva ya MCP haiwezi kutofautisha kati ya watumiaji tofauti wa GraphIt
...
### Nyaraka za Mtumiaji
[ ] Sasisha `gr-set-mcp-tool.md` na parameter ya `--auth-token`
...
[Huduma ya Zana ya MCP ya Sasa](../graphit-flow/graphit/agent/mcp_tool/service.py)
