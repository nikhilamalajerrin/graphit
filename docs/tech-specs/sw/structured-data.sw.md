...
Maelekezo haya yanaelezea jinsi GraphIt inavyounganishwa na mtiririko wa data iliyoainishwa, na kuwezesha mfumo kufanya kazi na data ambayo inaweza kuwakilishwa kama mistari katika meza au vitu katika maduka ya vitu. Uunganisho huu unaunga mkono matumizi manne makuu:
...
**Ufikiaji Umoja wa Data**: Toa kiungo kimoja cha kufikia data zote, iliyoainishwa na isiyoainishwa, ndani ya GraphIt.
**Uunganisho Kamili**: Uwezesha utendaji wa pamoja kati ya uwakilishi wa maarifa wa GraphIt unaotegemea chati na umbizo wa jadi wa data iliyoainishwa.
...
**Ulinganifu na Mifumo ya Zamani**: Dumishe utendaji wa sasa wa GraphIt huku uongezwa uwezekano wa data iliyoainishwa.

## Asili

Hivi sasa, GraphIt inafaa katika kuchakata data isiyoainishwa na kuunda chati za maarifa kutoka kwa vyanzo tofauti. Hata hivyo, matumizi mengi ya kampuni yanahusisha data ambayo ina muundo - rekodi za wateja, magogo ya miamala, hifadhi za bidhaa, na mengineyo ya seti za data za meza. Data hii iliyoainishwa mara nyingi inahitaji kuchanganuliwa pamoja na maudhui isiyoainishwa ili kutoa ufahamu kamili.
...
Upungufu wa daraja kati ya maswali kama ya SQL na maswali ya chati ya GraphIt.

Maelekezo haya yanaashiria pengo hizi kwa kuleta safu ya data iliyoainishwa ambayo inakamilisha uwezekano wa sasa wa GraphIt. Kwa kusaidia data iliyoainishwa kwa asili, GraphIt inaweza:
...
1. **Huduma ya NLP-kwa-Uulizaje-Imeinishwa**
   ...
   Inaunganishwa na uwezekano wa sasa wa NLP ya GraphIt.
   
   Moduli: graphit-flow/graphit/query/nlp_query/cassandra
...
3. **Moduli ya Utoaji wa Vitu** ✅ **[IMEKAMILIKA]**
   ...
   Inasajili kiungo cha usanidi (mfano: graphit-flow/graphit/prompt/template/service.py) ili kupokea data ya usanidi na kuondoa maelezo ya mfumo.
   Inapokea vitu na kuyaondoa kuwa vitu vya ExtractedObject ili kuwasilisha kwenye folyo ya Pulsar.
   NOTE: Kuna msimbo uliopo kwenye `graphit-flow/graphit/extract/object/row/`. Hii ilikuwa jaribio la awali na itahitaji marekebisho makubwa kwani haikubaliana na API za sasa. Tumia ikiwa ni muhimu, anza kutoka mwanzo ikiwa sio.
   Inahitaji kiungo cha mstari wa amri: `kg-extract-objects`

   Moduli: graphit-flow/graphit/extract/kg/objects/
...
4. **Moduli ya Kuandika ya Duka la Imeinishwa** ✅ **[IMEKAMILIKA]**
   ...
   **Maelezo ya Utendaji**:
   Msimbo uliopo katika `graphit-flow/graphit/storage/objects/cassandra/` ni wa zamani na haukidhi vipimo vya sasa vya API
   Inapaswa kurejelea `graphit-flow/graphit/storage/triples/cassandra` kama mfano wa mchakato wa hifadhi unaofanya kazi
   ...
   Moduli: graphit-flow/graphit/storage/objects/cassandra
...
5. **Huduma ya Maswali (Structured Query Service)** ✅ **[IMEKAMILIKA]**
   ...
   Moduli: graphit-flow/graphit/query/objects/cassandra
...
7. **Huduma ya Uingizaji wa Data Iliyopangwa (Structured Data Ingestion Service)**
   ...
   Moduli: graphit-flow/graphit/decoding/structured
...
8. **Huduma ya Uwekaji wa Data (Object Embedding Service)**
   ...
   Moduli: graphit-flow/graphit/embeddings/object_embeddings/qdrant

### Mifano ya Data

#### Utaratibu wa Uhifadhi wa Schema

Schemas zinawekwa katika mfumo wa usanidi wa GraphIt kwa kutumia muundo ufuatao:
...
Uunganishaji thabiti na usimamizi wa usanidi wa GraphIt uliopo
...
Kila kitu kiko katika vifurushi vya graphit-flow isipokuwa vipengele vya muundo (schema)
katika graphit-base.
