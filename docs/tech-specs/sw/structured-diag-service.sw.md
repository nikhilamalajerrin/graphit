...
Vigezo hivi vinaelezea huduma mpya inayoweza kutumika kwa utambuzi na uchambuzi wa data iliyobuniwa ndani ya GraphIt. Huduma hii hutoa utendakazi kutoka kwa zana ya `gr-load-structured-data` iliyopo ya mstari wa amri na kuifanya kuwa huduma ya ombi/jibu, na hivyo kuwezesha ufikiaji wa programu kwa uwezo wa utambuzi wa aina ya data na uundaji wa maelezo.
...
2. **Uundaji wa Maelezo**: Unda maelezo ya GraphIt ya data iliyobuniwa kwa sampuli fulani ya data na aina
...
**Kuzalisha Maelezo Sahihi**: Toa maelezo ya data iliyobuniwa ambayo yanaelea data ya chanzo kwa schemas za GraphIt
...
Kwa sasa, amri ya `gr-load-structured-data` hutoa utendakazi kamili kwa uchambuzi wa data iliyobuniwa na uundaji wa maelezo. Hata hivyo, utendakazi huu umeunganishwa sana na kiolesura cha CLI, na hivyo kupunguza uwezekano wake wa kutumika tena.
...
Vigezo hivi vinashughulikia pengo hizi kwa kuunda huduma maalum ya utambuzi wa data iliyobuniwa. Kwa kuonyesha uwezo huu kama huduma, GraphIt inaweza:
...
1. **Mchakato wa Huduma ya Utambuzi**
   ...
   Moduli: `graphit-flow/graphit/diagnosis/structured_data/service.py`
...
2. **Kigunduzi cha Aina ya Data**
   ...
   Moduli: `graphit-flow/graphit/diagnosis/structured_data/type_detector.py`

3. **Mundua wa Maelezo**
   Hutumia huduma ya ombi ili kuzalisha maelezo
   Huita ombi maalum ya muundo (diagnose-csv, diagnose-json, diagnose-xml)
   Inoelekeza nafasi za data kwa nafasi za schema za GraphIt kupitia majibu ya ombi

   Moduli: `graphit-flow/graphit/diagnosis/structured_data/descriptor_generator.py`
...
Huduma itafuata miongozo ya huduma ya GraphIt:
...
Huduma itounganisha na API za GraphIt zilizopo:

Vipengele Vilivyobadilishwa:
`gr-load-structured-data` CLI - Imepangwa upya ili kutumia huduma mpya kwa operesheni za uchambuzi
...
Udhibiti wa ufikiaji kupitia uthibitishaji wa GraphIt uliopo
...
`gr-load-structured-data` utekelezaji: `graphit-cli/graphit/cli/load_structured_data.py`
