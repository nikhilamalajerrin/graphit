...
**Uzoefu sawa wa utiririshaji**: Kutoa uzoefu sawa wa utiririshaji katika huduma zote za GraphIt.
...
Uzoefu usio sawa na huduma zingine za GraphIt.

Vigezo hivi vinashughulikia pengo hizi kwa kuongeza usaidizi wa utiririshaji kwa GraphRAG na DocumentRAG. Kwa kuruhusu majibu ya token kwa token, GraphIt inaweza:
...
1. **Utiririshaji wa PromptClient** (Tayari umetumiwa)
   ...
   Moduli: `graphit-base/graphit/base/prompt_client.py`
...
2. **Huduma ya GraphRAG** (Inahitaji kupitisha parameter ya utiririshaji)
   ...
   `graphit-flow/graphit/retrieval/graph_rag/rag.py` (Mchakato)
   `graphit-base/graphit/schema/graph_rag.py` (Schema ya ombi)
   `graphit-flow/graphit/gateway/dispatch/graph_rag.py` (Lango)
...
3. **Huduma ya DocumentRAG** (Inahitaji kupitisha parameter ya utiririshaji)
   ...
   `graphit-flow/graphit/retrieval/document_rag/rag.py` (Mchakato)
   `graphit-base/graphit/schema/document_rag.py` (Schema ya ombi)
   `graphit-flow/graphit/gateway/dispatch/document_rag.py` (Lango)
...
5. Ongeza `--no-streaming` bendera kwenye `gr-invoke-graph-rag` na `gr-invoke-document-rag` (utiririshaji umeanzishwa kwa chaguizi, kufuatia mtindo wa CLI ya wakala)
...
**Upimaji wa mwongozo**:
`gr-invoke-graph-rag -q "What is machine learning?"` (utiririshaji kwa chaguizi)
`gr-invoke-document-rag -q "Summarize the documents about AI"` (utiririshaji kwa chaguizi)
`gr-invoke-graph-rag --no-streaming -q "..."` (pima hali ya utiririshaji usio na utiririshaji)
...
Utiririshaji wa Wakala: `graphit-flow/graphit/agent/react/agent_manager.py`
PromptClient utiririshaji: `graphit-base/graphit/base/prompt_client.py`
