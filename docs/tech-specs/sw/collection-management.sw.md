...
Maelekezo haya yanafafanua mfumo wa usimamizi wa mkusanyiko unaoonekana. Kwa kuhitaji uundaji wa mkusanyiko unaoonekana, GraphIt huhakikisha:
...
Mfumo wa usimamizi wa mkusanyiko utatekelezwa ndani ya miundombinu iliyopo ya GraphIt:
...
1. **Uundaji wa Mkusanyiko** (Njia Mbili):
   ...
   **Njia A: Uundaji unaoanzishwa na Mtumiaji** kupitia `gr-set-collection`:
...
4. **Sasisho za Metadata**: Watumiaji wanaweza kusasisha metadata ya mkusanyiko baada ya uundaji:
   Sasisha jina, maelezo, na lebo kupitia `gr-set-collection`
...
5. **Ufutaji Wazi**: Watumiaji huondoa mkusanyiko kupitia `gr-delete-collection`:
...
**Unda Mkusanyiko**: Shughuli ya mtumiaji kupitia `gr-set-collection` AU otomatiki wakati wa uwasilishaji wa hati
...
Mkusanyiko huwepo katika mifumo tofauti ya kuhifadhi katika GraphIt:
...
**Unda/Boresha Mkusanyiko**: Unda mkusanyiko mpya au boresha metadata iliyopo kupitia `gr-set-collection`.
...
Utendaji utafuata mifumo iliyopo ya GraphIt kwa ujumuishaji wa huduma na muundo wa amri ya CLI.
...
1. **Urekebishaji wa Darasa la GraphIt** (`graphit/direct/cassandra.py`):
...
2. **Sasisho za Mwandishi wa Hifadhi ya Data** (`graphit/storage/triples/cassandra/write.py`):
   Dumishe muunganisho mmoja wa GraphIt kwa kila mtumiaji badala ya kwa kila (mtumiaji, mkusanyiko)
   Pasa mkusanyiko kwenye operesheni za kuingiza
   Matumizi bora ya rasilimali kwa muunganisho mdogo

3. **Sasisho za Huduma ya Swali** (`graphit/query/triples/cassandra/service.py`):
   Muunganisho mmoja wa GraphIt kwa kila mtumiaji
...
Operesheni za usimamizi wa ukusanyaji zinahitaji idhini inayofaa ili kuzuia ufikiaji usioidhinishwa au kufutwa kwa ukusanyaji. Udhibiti wa ufikiaji utalingana na modeli za usalama za GraphIt zilizopo.
...
1. **Huduma ya Usimamizi wa Ukusanyaji ya Librarian** (`graphit-flow/graphit/librarian/collection_manager.py`)
...
2. **Mfumo wa Metadata wa Ukusanyaji** (`graphit-base/graphit/schema/services/collection.py`)
...
3. **Mfumo wa Usimamizi wa Uhifadhi** (`graphit-base/graphit/schema/services/storage.py`)
...
4. **Mfumo wa Jedwali la Cassandra 4** (`graphit-flow/graphit/direct/cassandra_kg.py`)
