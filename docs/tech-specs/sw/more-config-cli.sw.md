...
Haya yanaeleza uwezo wa ziada wa usanidi wa amri ya utekelezaji (CLI) kwa GraphIt, ambayo inaruhusu watumiaji kusimamia vipengele vya usanidi kila kimoja kupitia amri mahususi za CLI. Uunganisho huu unaunga mkono matumizi manne makuu:
...
- **Mifumo ya Utekelezaji ya Umoja**: Kufuata misingi na mifumo iliyopo ya CLI ya GraphIt.
...
Hivi sasa, GraphIt hutoa usimamizi wa usanidi kupitia API ya Usanidi na amri moja ya CLI, `gr-show-config`, ambayo inaonyesha usanidi wote. Ingawa hii inafaa kwa kuona usanidi, haitoi uwezo wa usimamizi wa kina.
...
Haya yanaashiria pengo hili kwa kuongeza amri nne mpya za CLI ambazo hutoa usimamizi wa kina wa usanidi. Kwa kufichua shughuli za API ya Usanidi kupitia amri za CLI, GraphIt inaweza:
...
1. **gr-list-config-items**
   ...
   Moduli: `graphit.cli.list_config_items`
...
2. **gr-get-config-item**
   ...
   Moduli: `graphit.cli.get_config_item`
...
3. **gr-put-config-item**
   ...
   Moduli: `graphit.cli.put_config_item`
...
4. **gr-delete-config-item**
   ...
   Moduli: `graphit.cli.delete_config_item`

### Mifano ya Data

#### ConfigKey na ConfigValue

Amri hizi hutumia miundo ya data iliyopo kutoka `graphit.api.types`:
...
#### gr-put-config-item
```bash
gr-put-config-item --type <type> --key <key> --value <value>
```
...
```bash
gr-delete-config-item --type <type> --key <key>
```
...
- API ya Usanidi iliyopo: `graphit/api/config.py`
- Mfumo wa CLI: `graphit-cli/graphit/cli/show_config.py`
- Data: `graphit/api/types.py`
