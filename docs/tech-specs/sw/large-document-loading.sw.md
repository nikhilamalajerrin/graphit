...
hati kubwa katika GraphIt. Muundo wa sasa hutumia kupakia hati kama operesheni moja, na kusababisha
...
hati kubwa katika GraphIt. Muundo wa sasa hutibu kupakia hati kama operesheni moja na kamili,
...
Faili muhimu:
Ingizo la REST/WebSocket: `graphit-flow/graphit/gateway/service.py`
Msingi wa Librarian: `graphit-flow/graphit/librarian/librarian.py`
Uhifadhi wa blob: `graphit-flow/graphit/librarian/blob_store.py`
Jedwali la Cassandra: `graphit-flow/graphit/tables/library.py`
Mpango wa API: `graphit-base/graphit/schema/services/library.py`
...
**Faili:** `graphit-flow/graphit/librarian/blob_store.py`
...
SDK ya Python (`graphit-base/graphit/api/library.py`) inapaswa kushughulikia
<<<<<<< HEAD
...
**`gr-add-library-document`** inaendelea kufanya kazi bila kubadilika:

```bash
# Works transparently for any size - SDK handles chunking internally
gr-add-library-document --file large-report.pdf --title "Large Report"
```

Onyesho la maendeleo la hiari linaweza kuongezwa:

```bash
gr-add-library-document --file large-report.pdf --title "Large Report" --progress
# Output:
...
<<<<<<< HEAD
`gr-load-pdf` - imepitwa na wakati, tumia `gr-add-library-document`
`gr-load-text` - imepitwa na wakati, tumia `gr-add-library-document`
=======
`gr-load-pdf` - yamefutwa, tumia `gr-add-library-document`
`gr-load-text` - yamefutwa, tumia `gr-add-library-document`
>>>>>>> 82edf2d (New md files from RunPod)
...
gr-add-library-document --list-pending

# Resume specific upload (recovery scenario)
gr-add-library-document --resume upload-abc-123 --file large-report.pdf
```
