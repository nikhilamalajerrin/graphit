...
Com base na especificação STRUCTURED_DATA.md, este documento propõe as adições e modificações necessárias no esquema Pulsar para suportar as capacidades de dados estruturados no GraphIt.
...
`graphit-flow/graphit/decoding/structured` - Usa StructuredDataSubmission
`graphit-flow/graphit/query/nlp_query/cassandra` - Usa esquemas de consulta NLP
`graphit-flow/graphit/query/objects/cassandra` - Usa esquemas de consulta estruturados
`graphit-flow/graphit/extract/object/row/` - Consome Chunk, produz ExtractedObject
`graphit-flow/graphit/storage/objects/cassandra` - Usa o esquema Rows
`graphit-flow/graphit/embeddings/object_embeddings/qdrant` - Usa esquemas de incorporação de objetos
