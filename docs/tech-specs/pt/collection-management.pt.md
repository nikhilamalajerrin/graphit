...
Anteriormente, as coleções no GraphIt eram criadas implicitamente durante as operações de carregamento de dados, levando a problemas de sincronização em que as coleções poderiam existir em backends de armazenamento sem metadados correspondentes no bibliotecário. Isso criava desafios de gerenciamento e dados órfãos.

O modelo de criação explícita de coleções aborda esses problemas:
Exigindo que as coleções sejam criadas antes de serem usadas via `gr-set-collection`
...
Esta especificação define o modelo de gerenciamento explícito de coleções. Ao exigir a criação explícita de coleções, o GraphIt garante:
...
O sistema de gerenciamento de coleções será implementado dentro da infraestrutura existente do GraphIt:
...
1. **Criação da Coleção** (Dois Caminhos):
   ...
   **Caminho A: Criação Iniciada pelo Usuário** via `gr-set-collection`:
...
4. **Atualizações de Metadados**: Os usuários podem atualizar os metadados da coleção após a criação:
   Atualize o nome, a descrição e as tags via `gr-set-collection`
...
5. **Exclusão Explícita**: Os usuários excluem coleções via `gr-delete-collection`:
...
**Criar Coleção**: Operação do usuário via `gr-set-collection` OU automática na submissão de documento
...
As coleções existem em vários backends de armazenamento no GraphIt:
...
**Criar/Atualizar Coleção**: Criar uma nova coleção ou atualizar metadados existentes via `gr-set-collection`
...
A implementação seguirá os padrões existentes do GraphIt para integração de serviços e estrutura de comandos de linha de comando.
...
1. **Refatoração da Classe GraphIt** (`graphit/direct/cassandra.py`):
...
2. **Atualizações do Gravador de Armazenamento** (`graphit/storage/triples/cassandra/write.py`):
   Manter uma única conexão GraphIt por usuário em vez de por (usuário, coleção)
   Passar a coleção para as operações de inserção
   Melhor utilização de recursos com menos conexões

3. **Atualizações do Serviço de Consulta** (`graphit/query/triples/cassandra/service.py`):
   Uma única conexão GraphIt por usuário
...
As operações de gerenciamento de coleção requerem autorização apropriada para evitar acesso ou exclusão não autorizados de coleções. O controle de acesso estará alinhado com os modelos de segurança GraphIt existentes.
...
1. **Serviço de Gerenciamento de Coleção Librarian** (`graphit-flow/graphit/librarian/collection_manager.py`)
...
2. **Esquema de Metadados de Coleção** (`graphit-base/graphit/schema/services/collection.py`)
...
3. **Esquema de Gerenciamento de Armazenamento** (`graphit-base/graphit/schema/services/storage.py`)
...
4. **Esquema de 4 Tabelas do Cassandra** (`graphit-flow/graphit/direct/cassandra_kg.py`)
