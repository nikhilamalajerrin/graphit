...
Permite que as configurações de ferramentas MCP especifiquem tokens bearer opcionais para autenticar com servidores MCP protegidos. Isso permite que o GraphIt invoque ferramentas MCP hospedadas em servidores que exigem autenticação, sem modificar as interfaces de invocação de agentes ou ferramentas.
...
5. **Autenticação em nível de serviço**: Autentica o serviço GraphIt, não usuários individuais.
...
Implantações GraphIt de um único tenant.
Autenticação de serviço para serviço (GraphIt → Servidor MCP).
Ambientes de desenvolvimento e teste.
Ferramentas MCP internas acessadas pelo sistema GraphIt.
...
Uma implantação GraphIt de uma única organização em que todos os funcionários usam a mesma ferramenta MCP interna (por exemplo, consulta de banco de dados da empresa). O servidor MCP requer autenticação para evitar acesso externo, mas todos os usuários internos têm o mesmo nível de acesso.

**Exemplo de Cenário Inadequado:**
Uma plataforma SaaS GraphIt multi-tenant em que o Tenant A e o Tenant B precisam acessar seus próprios servidores MCP isolados com credenciais separadas. Este recurso NÃO suporta o gerenciamento de tokens por tenant.

### Componentes Relacionados
**graphit-flow/graphit/agent/mcp_tool/service.py**: Serviço de invocação da ferramenta MCP.
**graphit-cli/graphit/cli/set_mcp_tool.py**: Ferramenta de linha de comando para criar/atualizar configurações de MCP.
**graphit-cli/graphit/cli/show_mcp_tools.py**: Ferramenta de linha de comando para exibir configurações de MCP.
...
3. **Suporte da CLI**: `gr-set-mcp-tool` DEVE aceitar um parâmetro opcional `--auth-token`.
4. **Exibição do Token**: `gr-show-mcp-tools` DEVE indicar quando um token de autenticação está configurado (mascarado por segurança).
...
1. **Armazenamento de Configuração**: O usuário executa `gr-set-mcp-tool --id my-tool --tool-url http://server/api --auth-token xyz123`.
...
**Arquivo**: `graphit-flow/graphit/agent/mcp_tool/service.py`
...
**Arquivo**: `graphit-cli/graphit/cli/set_mcp_tool.py`
...
`-u, --api-url` (opcional): URL da API GraphIt
...
**Arquivo**: `graphit-cli/graphit/cli/show_mcp_tools.py`
...
**Arquivo**: `docs/cli/gr-set-mcp-tool.md`
...
[ ] Documentar o parâmetro `--auth-token` em `gr-set-mcp-tool.md`
...
| `gr-set-mcp-tool.md` | Modificado | Várias | Documentar novo parâmetro |
...
**Sem contexto de usuário**: O servidor MCP não pode distinguir entre diferentes usuários do GraphIt
...
Seu deployment do GraphIt atende a várias organizações (multi-inquilino)
...
Implantar instâncias separadas do GraphIt por locatário
...
O servidor MCP não consegue distinguir entre diferentes usuários do GraphIt
...
[ ] Atualizar `gr-set-mcp-tool.md` com o parâmetro `--auth-token`
...
[Serviço da Ferramenta MCP Atual](../graphit-flow/graphit/agent/mcp_tool/service.py)
