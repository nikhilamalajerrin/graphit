...
Esta especificação descreve ferramentas de linha de comando para depurar e explorar dados de explicabilidade no GraphIt. Essas ferramentas permitem que os usuários rastreiem como as respostas foram derivadas e depurem a cadeia de origem desde as arestas até os documentos de origem.

Três ferramentas de linha de comando:

1. **`gr-show-document-hierarchy`** - Mostrar a hierarquia de documento → página → trecho → aresta
2. **`gr-list-explain-traces`** - Listar todas as sessões GraphRAG com perguntas
3. **`gr-show-explain-trace`** - Mostrar o rastreamento completo de explicabilidade para uma sessão
...
O GraphIt possui dois sistemas de rastreabilidade:
...
### Ferramenta 1: gr-show-document-hierarchy
...
| `-u/--api-url` | URL do gateway (padrão: `$GRAPHIT_URL`) |
| `-t/--token` | Token de autenticação (padrão: `$GRAPHIT_TOKEN`) |
| `-U/--user` | ID do usuário (padrão: `graphit`) |
...
### Ferramenta 2: gr-list-explain-traces
...
### Ferramenta 3: gr-show-explain-trace
...
| Arquivo | Propósito |
|------|---------|
| `graphit-cli/graphit/cli/show_document_hierarchy.py` | Ferramenta 1 |
| `graphit-cli/graphit/cli/list_explain_traces.py` | Ferramenta 2 |
| `graphit-cli/graphit/cli/show_explain_trace.py` | Ferramenta 3 |

## Arquivos a Modificar

| Arquivo | Alteração |
|------|--------|
| `graphit-cli/setup.py` | Adicionar entradas de console_scripts |
...
Autenticação por token suportada via `--token` ou `$GRAPHIT_TOKEN`
...
Exemplo de CLI existente: `graphit-cli/graphit/cli/invoke_graph_rag.py`
