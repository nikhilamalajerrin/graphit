...
Esta especificação descreve um novo serviço invocável para diagnosticar e analisar dados estruturados dentro do GraphIt. O serviço extrai a funcionalidade da ferramenta de linha de comando existente `gr-load-structured-data` e a expõe como um serviço de solicitação/resposta, permitindo o acesso programático às capacidades de detecção de tipo de dados e geração de descritores.
...
2. **Geração de Descritor**: Gera um descritor de dados estruturados do GraphIt para uma determinada amostra de dados e tipo
...
**Gerar Descritores Precisos**: Produzir descritores de dados estruturados que mapeiem com precisão os dados de origem para os esquemas do GraphIt
...
Atualmente, o comando `gr-load-structured-data` fornece funcionalidade abrangente para analisar dados estruturados e gerar descritores. No entanto, essa funcionalidade está fortemente acoplada à interface da CLI, limitando sua reutilização.
...
Esta especificação aborda essas lacunas, criando um serviço dedicado para o diagnóstico de dados estruturados. Ao expor essas capacidades como um serviço, o GraphIt pode:
...
3. **Gerador de Descritores**
   ...
   Mapeia campos de dados para campos de esquema do GraphIt por meio de respostas de prompt
...
O serviço seguirá as convenções do serviço GraphIt:
...
O serviço será integrado com as APIs GraphIt existentes:

Componentes Modificados:
`gr-load-structured-data` CLI - Refatorado para usar o novo serviço para operações de diagnóstico
...
Controle de acesso através da autenticação GraphIt existente
...
`gr-load-structured-data` implementação: `graphit-cli/graphit/cli/load_structured_data.py`
