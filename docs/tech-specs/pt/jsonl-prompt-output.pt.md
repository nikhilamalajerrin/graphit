...
<<<<<<< HEAD
para respostas de prompt no GraphIt. JSONL permite a extração de dados estruturados
...
=======
para respostas de prompt no GraphIt. O JSONL permite a extração de dados estruturados
...
Implementação atual em `graphit-flow/graphit/template/prompt_manager.py`:
...
Implementação atual: `graphit-flow/graphit/template/prompt_manager.py`
