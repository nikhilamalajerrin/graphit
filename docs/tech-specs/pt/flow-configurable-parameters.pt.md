...
Esta especificação descreve a implementação de parâmetros configuráveis para flow blueprints no GraphIt. Os parâmetros permitem que os usuários personalizem os parâmetros do processador no momento da execução do fluxo, fornecendo valores que substituem os espaços reservados de parâmetros na definição do flow blueprint.
...
Flow blueprints no GraphIt agora suportam parâmetros de processador que podem conter valores fixos ou espaços reservados de parâmetros. Isso cria uma oportunidade para personalização em tempo de execução.
...
Ao aproveitar os parâmetros de processador parametrizados, o GraphIt pode:
...
O serviço de configuração de fluxo (`graphit-flow/graphit/config/service/flow.py`) requer as seguintes melhorias:
