...
Esta especificação descreve a estrutura e o formato das ontologias dentro do sistema GraphIt. As ontologias fornecem modelos de conhecimento formais que definem classes, propriedades e relacionamentos, suportando capacidades de raciocínio e inferência. O sistema usa um formato de configuração inspirado em OWL que representa amplamente os conceitos OWL/RDFS, ao mesmo tempo que é otimizado para os requisitos do GraphIt.
...
O GraphIt armazena ontologias como itens de configuração em um sistema flexível de chave-valor. Embora o formato seja inspirado em OWL (Web Ontology Language), ele é otimizado para casos de uso específicos do GraphIt e não adere estritamente a todas as especificações do OWL.

As ontologias no GraphIt permitem:
