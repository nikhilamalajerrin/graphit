...
**Experiência de streaming consistente**: Fornecer a mesma experiência de streaming em todos os serviços GraphIt.
...
Experiência inconsistente em comparação com outros serviços GraphIt.

Esta especificação aborda essas lacunas adicionando suporte a streaming ao GraphRAG e ao DocumentRAG. Ao permitir respostas token por token, o GraphIt pode:
...
1. **Streaming do PromptClient** (Já implementado)
   ...
   Módulo: `graphit-base/graphit/base/prompt_client.py`
...
2. **Serviço GraphRAG** (Precisa de passagem de parâmetro de streaming)
   ...
   Módulos:
   `graphit-flow/graphit/retrieval/graph_rag/graph_rag.py`
   `graphit-flow/graphit/retrieval/graph_rag/rag.py` (Processador)
   `graphit-base/graphit/schema/graph_rag.py` (Esquema de solicitação)
   `graphit-flow/graphit/gateway/dispatch/graph_rag.py` (Gateway)
...
3. **Serviço DocumentRAG** (Precisa de passagem de parâmetro de streaming)
   ...
   Módulos:
   `graphit-flow/graphit/retrieval/document_rag/document_rag.py`
   `graphit-flow/graphit/retrieval/document_rag/rag.py` (Processador)
   `graphit-base/graphit/schema/document_rag.py` (Esquema de solicitação)
   `graphit-flow/graphit/gateway/dispatch/document_rag.py` (Gateway)
...
5. Adicionar `--no-streaming` flags a `gr-invoke-graph-rag` e `gr-invoke-document-rag` (streaming habilitado por padrão, seguindo o padrão da CLI do agente)
...
**Testes manuais**:
`gr-invoke-graph-rag -q "What is machine learning?"` (streaming por padrão)
`gr-invoke-document-rag -q "Summarize the documents about AI"` (streaming por padrão)
`gr-invoke-graph-rag --no-streaming -q "..."` (testar o modo não-streaming)
...
Streaming do Agente: `graphit-flow/graphit/agent/react/agent_manager.py`
Streaming do PromptClient: `graphit-base/graphit/base/prompt_client.py`
