...
Esta especificação descreve a integração do GraphIt com fluxos de dados estruturados, permitindo que o sistema trabalhe com dados que podem ser representados como linhas em tabelas ou objetos em armazenamentos de objetos. A integração suporta quatro casos de uso primários:
...
**Acesso Unificado a Dados**: Fornecer uma única interface para acessar dados estruturados e não estruturados dentro do GraphIt.
**Integração Perfeita**: Permitir interoperabilidade perfeita entre a representação de conhecimento baseada em grafo do GraphIt e formatos de dados estruturados tradicionais.
...
**Compatibilidade com Versões Anteriores**: Preservar a funcionalidade existente do GraphIt, adicionando recursos de dados estruturados.

## Contexto

Atualmente, o GraphIt se destaca no processamento de dados não estruturados e na construção de grafos de conhecimento a partir de diversas fontes. No entanto, muitos casos de uso empresariais envolvem dados que são inerentemente estruturados - registros de clientes, logs de transações, bancos de dados de inventário e outros conjuntos de dados tabulares. Esses conjuntos de dados estruturados geralmente precisam ser analisados ​​juntamente com conteúdo não estruturado para fornecer insights abrangentes.
...
Falta de uma ponte entre consultas semelhantes a SQL e as consultas de grafo do GraphIt.

Esta especificação aborda essas lacunas, introduzindo uma camada de dados estruturados que complementa as capacidades existentes do GraphIt. Ao suportar dados estruturados nativamente, o GraphIt pode:
...
1. **Serviço de Conversão de Linguagem Natural para Consulta Estruturada**
   ...
   Integra-se com as capacidades existentes de NLP do GraphIt.
   
   Módulo: graphit-flow/graphit/query/nlp_query/cassandra
...
3. **Módulo de Extração de Objetos** ✅ **[COMPLETO]**
   ...
   Registra um manipulador de configuração (exemplo: graphit-flow/graphit/prompt/template/service.py) para receber dados de configuração e decodificar informações de esquema.
   Recebe objetos e os decodifica em objetos ExtractedObject para entrega na fila Pulsar.
   OBS: Existe código existente em `graphit-flow/graphit/extract/object/row/`. Esta foi uma tentativa anterior e precisará ser refatorada significativamente, pois não está em conformidade com as APIs atuais. Use-o se for útil, comece do zero se não for.
   Requer uma interface de linha de comando: `kg-extract-objects`

   Módulo: graphit-flow/graphit/extract/kg/objects/
...
4. **Módulo de Escrita de Armazenamento Estruturado** ✅ **[COMPLETO]**
   ...
   **Observações de Implementação**:
   O código existente em `graphit-flow/graphit/storage/objects/cassandra/` está desatualizado e não está em conformidade com as APIs atuais.
   Deve referenciar `graphit-flow/graphit/storage/triples/cassandra` como um exemplo de um processador de armazenamento funcional.
   ...
   Módulo: graphit-flow/graphit/storage/objects/cassandra
...
5. **Serviço de Consulta Estruturada** ✅ **[COMPLETO]**
   ...
   Módulo: graphit-flow/graphit/query/objects/cassandra
...
7. **Serviço de Ingestão de Dados Estruturados**
   ...
   Módulo: graphit-flow/graphit/decoding/structured
...
8. **Serviço de Incorporação de Objetos**
   ...
   Módulo: graphit-flow/graphit/embeddings/object_embeddings/qdrant

### Modelos de Dados

#### Mecanismo de Armazenamento de Esquemas

Os esquemas são armazenados no sistema de configuração do GraphIt usando a seguinte estrutura:
...
Integração consistente com o gerenciamento de configuração existente do GraphIt
...
Tudo está nos pacotes graphit-flow, exceto os itens de esquema
em graphit-base.
