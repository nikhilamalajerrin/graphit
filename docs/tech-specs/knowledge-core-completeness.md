...
They can be downloaded as files, transferred between GraphIt instances, and
loaded back into vector and graph stores.

Recent additions to GraphIt — explainability/provenance and named graphs —
...
(`graphit-base/graphit/provenance/triples.py`) and published to graph
...
|-----------|------|---------|
| Core Cassandra schema | `graphit-flow/graphit/tables/knowledge.py` | Table definitions, read/write |
| Core manager | `graphit-flow/graphit/cores/knowledge.py` | API operations, load-to-store |
| Core store processor | `graphit-flow/graphit/storage/knowledge/store.py` | Extraction → Cassandra |
| CLI download | `graphit-cli/graphit/cli/get_kg_core.py` | Core → msgpack file |
| CLI upload | `graphit-cli/graphit/cli/put_kg_core.py` | Msgpack file → core |
| CLI load | `graphit-cli/graphit/cli/load_kg_core.py` | Core → graph/vector stores |
| API client | `graphit-base/graphit/api/knowledge.py` | Client-side knowledge API |
| Triple schema | `graphit-base/graphit/schema/core/primitives.py` | Triple dataclass with `g` field |
| Provenance generation | `graphit-base/graphit/provenance/triples.py` | PROV-O triple creation |
| Librarian | `graphit-flow/graphit/librarian/librarian.py` | Document storage service |
| Library tables | `graphit-flow/graphit/tables/library.py` | Document metadata in Cassandra |
| Blob store | `graphit-flow/graphit/librarian/blob_store.py` | S3/MinIO object storage |
...
**`gr-get-kg-core`**: Add handling for `"lm"` and `"lb"` record types in
the file writer.

**`gr-put-kg-core`**: Add handling for `"lm"` and `"lb"` record types in
