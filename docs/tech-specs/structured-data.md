...
This specification describes the integration of GraphIt with structured data flows, enabling the system to work with data that can be represented as rows in tables or objects in object stores. The integration supports four primary use cases:
...
- **Unified Data Access**: Provide a single interface for accessing both structured and unstructured data within GraphIt
- **Seamless Integration**: Enable smooth interoperability between GraphIt's graph-based knowledge representation and traditional structured data formats
...
- Missing bridge between SQL-like queries and GraphIt's graph queries

This specification addresses these gaps by introducing a structured data layer that complements GraphIt's existing capabilities. By supporting structured data natively, GraphIt can:
...
1. **NLP-to-Structured-Query Service**
   ...
   - Integrates with existing GraphIt NLP capabilities
   
   Module: graphit-flow/graphit/query/nlp_query/cassandra
...
3. **Object Extraction Module** ✅ **[COMPLETE]**
   ...
   - Registers a config handler (example: graphit-flow/graphit/prompt/template/service.py) to receive config data and decode schema information
   - Receives objects and decodes them to ExtractedObject objects for delivery on the Pulsar queue
   - NOTE: There's existing code at `graphit-flow/graphit/extract/object/row/`. This was a previous attempt and will need to be majorly refactored as it doesn't conform to current APIs. Use it if it's useful, start from scratch if not.
   - Requires a command-line interface: `kg-extract-objects`

   Module: graphit-flow/graphit/extract/kg/objects/
...
4. **Structured Store Writer Module** ✅ **[COMPLETE]**
   ...
   **Implementation Notes**:
   - Existing code at `graphit-flow/graphit/storage/objects/cassandra/` is outdated and doesn't comply with current APIs
   - Should reference `graphit-flow/graphit/storage/triples/cassandra` as an example of a working storage processor
   - Needs evaluation of existing code for any reusable components before deciding to refactor or rewrite

   Module: graphit-flow/graphit/storage/objects/cassandra
...
5. **Structured Query Service** ✅ **[COMPLETE]**
   ...
   Module: graphit-flow/graphit/query/objects/cassandra
...
7. **Structured Data Ingestion Service**
   ...
   Module: graphit-flow/graphit/decoding/structured
...
8. **Object Embedding Service**
   ...
   Module: graphit-flow/graphit/embeddings/object_embeddings/qdrant

### Data Models

#### Schema Storage Mechanism

Schemas are stored in GraphIt's configuration system using the following structure:
...
- Consistent integration with existing GraphIt configuration management
...
Everything is in the graphit-flow packages except for schema items
in graphit-base.
