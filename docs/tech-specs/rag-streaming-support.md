...
- **Consistent streaming UX**: Provide the same streaming experience across all GraphIt services
...
- Inconsistent experience compared to other GraphIt services

This specification addresses these gaps by adding streaming support to GraphRAG and DocumentRAG. By enabling token-by-token responses, GraphIt can:
...
1. **PromptClient Streaming** (Already implemented)
   ...
   Module: `graphit-base/graphit/base/prompt_client.py`
...
2. **GraphRAG Service** (Needs streaming parameter pass-through)
   ...
   - `graphit-flow/graphit/retrieval/graph_rag/rag.py` (Processor)
   - `graphit-base/graphit/schema/graph_rag.py` (Request schema)
...
3. **DocumentRAG Service** (Needs streaming parameter pass-through)
   ...
   - `graphit-flow/graphit/retrieval/document_rag/rag.py` (Processor)
   - `graphit-base/graphit/schema/document_rag.py` (Request schema)
...
5. Add `--no-streaming` flags to `gr-invoke-graph-rag` and `gr-invoke-document-rag` (streaming enabled by default, following agent CLI pattern)
...
**Manual testing**:
- `gr-invoke-graph-rag -q "What is machine learning?"` (streaming by default)
- `gr-invoke-document-rag -q "Summarize the documents about AI"` (streaming by default)
- `gr-invoke-graph-rag --no-streaming -q "..."` (test non-streaming mode)
...
- Agent streaming: `graphit-flow/graphit/agent/react/agent_manager.py`
- PromptClient streaming: `graphit-base/graphit/base/prompt_client.py`
