---
layout: default
title: "GraphIt Agent Orchestration — Technical Specification"
parent: "Tech Specs"
---

# GraphIt Agent Orchestration — Technical Specification

## Overview

This specification describes the extension of GraphIt's agent architecture
...
A central principle of GraphIt's explainability architecture is that
...
The aggregator is event-driven, consistent with GraphIt's Pulsar-based
...
Extending the existing `urn:graphit:agent:{session_id}` pattern:

| Entity | URI Pattern |
|---|---|
| Session (existing) | `urn:graphit:agent:{session_id}` |
| Iteration (existing) | `urn:graphit:agent:{session_id}/i{n}` |
| Conclusion (existing) | `urn:graphit:agent:{session_id}/answer` |
| Routing decision | `urn:graphit:agent:{session_id}/routing` |
| Fan-out record | `urn:graphit:agent:{session_id}/fanout/{correlation_id}` |
| Subagent completion | `urn:graphit:agent:{session_id}/completion` |
