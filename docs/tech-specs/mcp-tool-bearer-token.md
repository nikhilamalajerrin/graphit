...
Enable MCP tool configurations to specify optional bearer tokens for authenticating with protected MCP servers. This allows GraphIt to securely invoke MCP tools hosted on servers that require authentication, without modifying the agent or tool invocation interfaces.
...
5. **Service-level authentication**: Authenticates the GraphIt service, not individual users
6. **Shared security context**: All invocations of an MCP tool use the same credential

### Use Case Applicability

**✅ Appropriate Use Cases:**
- Single-tenant GraphIt deployments
- Service-to-service authentication (GraphIt → MCP Server)
- Development and testing environments
- Internal MCP tools accessed by the GraphIt system
...
A single-organization GraphIt deployment where all employees use the same internal MCP tool (e.g., company database lookup). The MCP server requires authentication to prevent external access, but all internal users have the same access level.

**Example Inappropriate Scenario:**
A multi-tenant GraphIt SaaS platform where Tenant A and Tenant B each need to access their own isolated MCP servers with separate credentials. This feature does NOT support per-tenant token management.

### Related Components
- **graphit-flow/graphit/agent/mcp_tool/service.py**: MCP tool invocation service
- **graphit-cli/graphit/cli/set_mcp_tool.py**: CLI tool for creating/updating MCP configurations
- **graphit-cli/graphit/cli/show_mcp_tools.py**: CLI tool for displaying MCP configurations
...
3. **CLI Support**: `gr-set-mcp-tool` MUST accept optional `--auth-token` parameter
4. **Token Display**: `gr-show-mcp-tools` MUST indicate when auth-token is configured (masked for security)
...
1. **Configuration Storage**: User runs `gr-set-mcp-tool --id my-tool --tool-url http://server/api --auth-token xyz123`
...
**File**: `graphit-flow/graphit/agent/mcp_tool/service.py`
...
**File**: `graphit-cli/graphit/cli/set_mcp_tool.py`
...
- `-u, --api-url` (optional): GraphIt API URL
...
**File**: `graphit-cli/graphit/cli/show_mcp_tools.py`
...
**File**: `docs/cli/gr-set-mcp-tool.md`
...
- [ ] Document `--auth-token` parameter in `gr-set-mcp-tool.md`
...
| `gr-set-mcp-tool.md` | Modified | Various | Document new parameter |
...
- **No user context**: The MCP server cannot distinguish between different GraphIt users
...
- Your GraphIt deployment serves multiple organizations (multi-tenant)
...
- Deploy separate GraphIt instances per tenant
...
- MCP server cannot distinguish between different GraphIt users
...
### User Documentation
- [ ] Update `gr-set-mcp-tool.md` with `--auth-token` parameter
- [ ] Add security considerations section
...
- [Current MCP Tool Service](../graphit-flow/graphit/agent/mcp_tool/service.py)
