...
This specification addresses issues and gaps identified during the initial implementation of GraphIt's structured data integration, as described in `structured-data.md`.
...
- **Stage 1: Embedding computation** (`graphit-flow/graphit/embeddings/row_embeddings/`) - Consumes `ExtractedObject`, computes embeddings via the embeddings service, outputs `RowEmbeddings`
- **Stage 2: Embedding storage** (`graphit-flow/graphit/storage/row_embeddings/qdrant/`) - Consumes `RowEmbeddings`, writes vectors to Qdrant

The Cassandra row writer is a separate parallel consumer:

- **Cassandra row writer** (`graphit-flow/graphit/storage/rows/cassandra`) - Consumes `ExtractedObject`, writes rows to Cassandra
...
|-------|--------|-------------|
| Stage 1 | `graphit-flow/graphit/embeddings/row_embeddings/` | `row-embeddings` |
| Stage 2 | `graphit-flow/graphit/storage/row_embeddings/qdrant/` | `row-embeddings-write-qdrant` |
...
Module: `graphit-flow/graphit/query/row_embeddings/qdrant`
...
Command: `gr-invoke-row-embeddings`
...
|-----------|----------|-------|-------------|
| Query Service | `graphit-flow/graphit/query/objects/cassandra/service.py` | ~740 | Monolithic: GraphQL schema generation, filter parsing, Cassandra queries, request handling |
| Writer | `graphit-flow/graphit/storage/objects/cassandra/write.py` | ~540 | Per-schema table creation, secondary indexes, insert/delete |
...
**Writer** (`graphit-flow/graphit/storage/rows/cassandra/`):
...
**Query Service** (`graphit-flow/graphit/query/rows/cassandra/`):
...
| Module | Purpose |
|--------|---------|
| `graphit-flow/graphit/query/graphql/` | Shared GraphQL utilities |
| `graphit-flow/graphit/query/row_embeddings/qdrant/` | Row embeddings query API |
| `graphit-flow/graphit/embeddings/row_embeddings/` | Row embeddings computation (Stage 1) |
| `graphit-flow/graphit/storage/row_embeddings/qdrant/` | Row embeddings storage (Stage 2) |
