...
La documentación de la API Python se genera a partir de los docstrings utilizando un script de Python personalizado que introspecciona el paquete `graphit.api`.

### Requisitos previos

El paquete `graphit` debe ser importable. Si estás trabajando en un entorno de desarrollo:
...
La documentación generada muestra la API pública exactamente como los usuarios la importan desde `graphit.api`, sin exponer la estructura interna del módulo.
