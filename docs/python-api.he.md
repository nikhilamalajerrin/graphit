---
layout: default
title: "מדריך הפניות ל-API של GraphIt בפייתון"
parent: "Hebrew (Beta)"
---

# מדריך הפניות ל-API של GraphIt בפייתון
...
כל המחלקות והטיפוסים מיובאים מהחבילה `graphit.api`:
...
לקוח API הראשי של GraphIt לפעולות סינכרוניות ואסינכרוניות.

מחלקה זו מספקת גישה לכל שירותי GraphIt, כולל ניהול זרימות,
...
אתחול לקוח ה-API של GraphIt.

**ארגומנטים:**

`url`: כתובת הבסיס עבור ה-API של GraphIt (ברירת מחדל: "http://localhost:8088/"")
...
זרימות הן יחידות הביצוע העיקריות ב-GraphIt, המספקות גישה ל
...
שולף מדדים בפורמט Prometheus מהשירות GraphIt
לצורך ניטור וניתוח.
...
מחלקה זו מספקת גישה לכל שירותי GraphIt, כולל:
השלמת טקסט והטמעות
...
`user`: מזהה משתמש (ברירת מחדל: "graphit")
...
`user`: מזהה משתמש/מרחב מפתחות (ברירת מחדל: "graphit")
...
`user`: מזהה משתמש/אזור מפתחות (ברירת מחדל: "graphit")
`collection`: מזהה אוסף (ברירת מחדל: "default")
...
`user`: מזהה משתמש/מרחב (ברירת מחדל: "graphit")
`collection`: מזהה אוסף (ברירת מחדל: "default")
...
`user`: מזהה מרחב מפתחות Cassandra (ברירת מחדל: "graphit")
...
`url`: כתובת URL בסיסית עבור ממשק ה-API של GraphIt
...
שיטה פנימית לביצוע בקשות מאומתות ל-API של GraphIt.
...
מספק ממשק סינכרוני לשירותי GraphIt המבוססים על WebSocket,
...
`url`: כתובת URL בסיסית עבור ממשק ה-API של GraphIt (HTTP/HTTPS יומרו ל-WS/WSS)
...
`user`: מזהה משתמש/מרחב מפתחות
`collection`: מזהה אוסף
`api`: מופע GraphIt Api לגישה לספרן (אופציונלי)
...
`api`: מופע GraphIt Api לגישה לספרן
...
`user`: מזהה משתמש/מרחב מפתחות
`collection`: מזהה אוסף
`api`: מופע של GraphIt Api לגישה לספרן (אופציונלי)
`max_content`: אורך תוכן מקסימלי לסינתזה
...
מחלקה בסיסית עבור כל שגיאות השירות של GraphIt.
