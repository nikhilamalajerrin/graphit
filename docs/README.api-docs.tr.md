...
Python API dokümantasyonu, `graphit.api` paketini inceleyen özel bir Python betiği kullanılarak, dokümantasyon dizelerinden (docstrings) oluşturulur.

### Ön Koşullar

graphit paketi içe aktarılabilir olmalıdır. Geliştirme ortamında çalışıyorsanız:
...
Oluşturulan dokümantasyon, kullanıcıların `graphit.api`'dan içe aktardığı şekilde, tam olarak kamu API'sini gösterir ve dahili modül yapısını ortaya çıkarmaz.
