---
layout: default
title: "GraphIt Python API Referansı"
parent: "Turkish (Beta)"
---

# GraphIt Python API Referansı
...
Tüm sınıflar ve tipler, `graphit.api` paketinden içe aktarılmıştır:

```python
from graphit.api import Api, Triple, ConfigKey
...
[GraphItException](#graphitexception)
...
Senkron ve asenkron işlemler için ana GraphIt API istemcisi.

Bu sınıf, akış yönetimi,
bilgi grafiği işlemleri, belge işleme, RAG sorguları ve daha fazlası dahil olmak üzere tüm GraphIt hizmetlerine erişim sağlar. Hem REST tabanlı hem de WebSocket tabanlı iletişim modellerini destekler.
...
GraphIt API istemciyi başlatın.

**Argümanlar:**

`url`: GraphIt API'si için temel URL (varsayılan: "http://localhost:8088/"")
...
# List collections
colls = collection.list_collections(user="graphit")
...
Akışlar, GraphIt'ta birincil yürütme birimleridir ve ajanlar, RAG sorguları, gömme ve belge işleme gibi
...
cores = knowledge.list_kg_cores(user="graphit")

# Load a KG core
knowledge.load_kg_core(id="core-123", user="graphit")
```
...
# List documents
docs = library.get_documents(user="graphit")
```
...
İzleme ve gözlemlenebilirlik için GraphIt hizmetinden Prometheus formatında ölçümleri alır.
...
```python
from graphit.api import FlowInstance
```

Belirli bir akış üzerinde hizmetleri çalıştırmak için kullanılan akış örneği istemcisi.

Bu sınıf, aşağıdaki GraphIt hizmetlerine erişim sağlar:
...
### `agent(self, question, user='graphit', state=None, group=None, history=None)`
...
`user`: Kullanıcı tanımlayıcısı (varsayılan: "graphit")
...
### `document_rag(self, query, user='graphit', collection='default', doc_limit=10)`
...
`user`: Kullanıcı/keyspace tanımlayıcısı (varsayılan: "graphit")
...
### `graph_rag(self, query, user='graphit', collection='default', entity_limit=50, triple_limit=30, max_subgraph_size=150, max_path_length=2)`
...
`query`: Doğal dil sorgusu
`user`: Kullanıcı/veri kümesi tanımlayıcısı (varsayılan: "graphit")
`collection`: Koleksiyon tanımlayıcısı (varsayılan: "default")
...
### `row_embeddings_query(self, text, schema_name, user='graphit', collection='default', index_name=None, limit=10)`
...
`user`: Kullanıcı/veri tabanı tanımlayıcısı (varsayılan: "graphit")
`collection`: Koleksiyon tanımlayıcısı (varsayılan: "default")
...
### `rows_query(self, query, user='graphit', collection='default', variables=None, operation_name=None)`
...
`user`: Kullanıcı/veri alanı tanımlayıcısı (varsayılan: "graphit")
`collection`: Koleksiyon tanımlayıcısı (varsayılan: "default")
...
### `structured_query(self, question, user='graphit', collection='default')`
...
`user`: Cassandra keyspace tanımlayıcısı (varsayılan: "graphit")
...
```python
from graphit.knowledge import Uri, Literal
...
```python
from graphit.api import AsyncFlow
```
...
`url`: GraphIt API'si için temel URL
...
GraphIt API'sine kimlik doğrulaması yapılmış istekler göndermek için kullanılan iç metot.
...
```python
from graphit.api import AsyncFlowInstance
```
...
### `__init__(self, flow: graphit.api.async_flow.AsyncFlow, flow_id: str)`
...
### `row_embeddings_query(self, text: str, schema_name: str, user: str = 'graphit', collection: str = 'default', index_name: str | None = None, limit: int = 10, **kwargs: Any)`
...
`user`: Kullanıcı kimliği (varsayılan: "graphit")
...
```python
from graphit.api import SocketClient
```

Akış işlemleri için senkron WebSocket istemcisi.

WebSocket tabanlı GraphIt hizmetlerine senkron bir arayüz sağlar,
...
`url`: GraphIt API'si için temel URL (HTTP/HTTPS, WS/WSS'ye dönüştürülecektir)
...
# Stream agent responses
for chunk in flow.agent(question="Hello", user="graphit", streaming=True):
    print(chunk.content, end='', flush=True)
```
...
```python
from graphit.api import SocketFlowInstance
```
...
### `__init__(self, client: graphit.api.socket_client.SocketClient, flow_id: str) -> None`
...
### `agent(self, question: str, user: str, state: Dict[str, Any] | None = None, group: str | None = None, history: List[Dict[str, Any]] | None = None, streaming: bool = False, **kwargs: Any) -> Dict[str, Any] | Iterator[graphit.api.types.StreamingChunk]`
...
### `agent_explain(self, question: str, user: str, collection: str, state: Dict[str, Any] | None = None, group: str | None = None, history: List[Dict[str, Any]] | None = None, **kwargs: Any) -> Iterator[graphit.api.types.StreamingChunk | graphit.api.types.ProvenanceEvent]`
...
```python
from graphit.api import Api, ExplainabilityClient, ProvenanceEvent
from graphit.api import AgentThought, AgentObservation, AgentAnswer
...
### `document_rag_explain(self, query: str, user: str, collection: str, doc_limit: int = 10, **kwargs: Any) -> Iterator[graphit.api.types.RAGChunk | graphit.api.types.ProvenanceEvent]`
...
```python
from graphit.api import Api, ExplainabilityClient, RAGChunk, ProvenanceEvent
...
### `graph_rag_explain(self, query: str, user: str, collection: str, max_subgraph_size: int = 1000, max_subgraph_count: int = 5, max_entity_distance: int = 3, **kwargs: Any) -> Iterator[graphit.api.types.RAGChunk | graphit.api.types.ProvenanceEvent]`
...
### `row_embeddings_query(self, text: str, schema_name: str, user: str = 'graphit', collection: str = 'default', index_name: str | None = None, limit: int = 10, **kwargs: Any) -> Dict[str, Any]`
...
```python
from graphit.api import AsyncSocketClient
```
...
```python
from graphit.api import AsyncSocketFlowInstance
```
...
### `__init__(self, client: graphit.api.async_socket_client.AsyncSocketClient, flow_id: str)`
...
### `row_embeddings_query(self, text: str, schema_name: str, user: str = 'graphit', collection: str = 'default', index_name: str | None = None, limit: int = 10, **kwargs)`
...
```python
from graphit.api import BulkClient
```
...
### `export_triples(self, flow: str, **kwargs: Any) -> Iterator[graphit.api.types.Triple]`
...
### `import_triples(self, flow: str, triples: Iterator[graphit.api.types.Triple], metadata: Dict[str, Any] | None = None, batch_size: int = 100, **kwargs: Any) -> None`
...
```python
from graphit.api import Triple
...
```python
from graphit.api import AsyncBulkClient
```
...
### `export_triples(self, flow: str, **kwargs: Any) -> AsyncIterator[graphit.api.types.Triple]`
...
### `import_triples(self, flow: str, triples: AsyncIterator[graphit.api.types.Triple], **kwargs: Any) -> None`
...
```python
from graphit.api import Metrics
```
...
```python
from graphit.api import AsyncMetrics
```
...
```python
from graphit.api import ExplainabilityClient
```
...
`user`: Kullanıcı/keyspace tanımlayıcısı.
`collection`: Koleksiyon tanımlayıcısı.
`api`: Kütüphaneci erişimi için GraphIt Api örneği (isteğe bağlı).
...
`api`: Kütüphaneci erişimi için GraphIt Api örneği.
...
### `fetch_edge_selection(self, uri: str, graph: str | None = None, user: str | None = None, collection: str | None = None) -> graphit.api.explainability.EdgeSelection | None`
...
### `fetch_entity(self, uri: str, graph: str | None = None, user: str | None = None, collection: str | None = None) -> graphit.api.explainability.ExplainEntity | None`
...
### `fetch_focus_with_edges(self, uri: str, graph: str | None = None, user: str | None = None, collection: str | None = None) -> graphit.api.explainability.Focus | None`
...
### `list_sessions(self, graph: str | None = None, user: str | None = None, collection: str | None = None, limit: int = 50) -> List[graphit.api.explainability.Question]`
...
```python
from graphit.api import ExplainEntity
```
...
```python
from graphit.api import Question
```
...
```python
from graphit.api import Exploration
```
...
`edge_selections`: typing.List[graphit.api.explainability.EdgeSelection]

### Yöntemler

### `__init__(self, uri: str, entity_type: str = '', selected_edge_uris: List[str] = <factory>, edge_selections: List[graphit.api.explainability.EdgeSelection] = <factory>) -> None`
...
```python
from graphit.api import Synthesis
```
...
```python
from graphit.api import Analysis
```
...
```python
from graphit.api import Conclusion
```
...
```python
from graphit.api import EdgeSelection
```
...
```python
from graphit.api import Literal
```
...
```python
from graphit.api import ConfigKey
```
...
```python
from graphit.api import ConfigValue
```
...
```python
from graphit.api import DocumentMetadata
```
...
`title`: <class 'str'>
`comments`: <class 'str'>
`metadata`: typing.List[graphit.api.types.Triple]
`user`: <class 'str'>
...
### `__init__(self, id: str, time: datetime.datetime, kind: str, title: str, comments: str, metadata: List[graphit.api.types.Triple], user: str, tags: List[str], parent_id: str = '', document_type: str = 'source') -> None`
...
```python
from graphit.api import ProcessingMetadata
```
...
```python
from graphit.api import CollectionMetadata
```
...
```python
from graphit.api import StreamingChunk
```
...
```python
from graphit.api import AgentThought
```
...
```python
from graphit.api import AgentObservation
```
...
```python
from graphit.api import AgentAnswer
```
...
```python
from graphit.api import RAGChunk
```
...
```python
from graphit.api import ProvenanceEvent
```
...
```python
from graphit.api import ProtocolException
```
...
```python
from graphit.api import GraphItException
```

Tüm GraphIt hizmeti hataları için temel sınıf.
...
```python
from graphit.api import AgentError
```
...
```python
from graphit.api import ConfigError
```
...
```python
from graphit.api import DocumentRagError
```
...
```python
from graphit.api import FlowError
```
...
```python
from graphit.api import GatewayError
```
...
```python
from graphit.api import GraphRagError
```
...
```python
from graphit.api import LLMError
```
...
```python
from graphit.api import LoadError
```
...
```python
from graphit.api import LookupError
```
...
```python
from graphit.api import NLPQueryError
```
...
```python
from graphit.api import RowsQueryError
```
...
```python
from graphit.api import RequestError
```
...
```python
from graphit.api import StructuredQueryError
```
...
```python
from graphit.api import UnexpectedError
```
...
```python
from graphit.api import ApplicationException
```
