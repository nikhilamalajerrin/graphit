...
This document provides instructions for running and maintaining the GraphIt test suite.

## Overview

The GraphIt test suite follows the testing strategy outlined in [TEST_STRATEGY.md](TEST_STRATEGY.md) and implements the test cases defined in [TEST_CASES.md](TEST_CASES.md). The tests are organized into unit tests, integration tests, and performance tests.
...
### Install GraphIt Packages

The tests require GraphIt packages to be installed. You can use the provided scripts:
...
```bash
# Run simple tests that don't require full GraphIt infrastructure
./run_simple_tests.sh
...
```bash
# Run all VertexAI tests (requires full GraphIt setup)
pytest tests/unit/test_text_completion/test_vertexai_processor.py -v

# Run with coverage
pytest tests/unit/test_text_completion/test_vertexai_processor.py --cov=graphit.model.text_completion.vertexai
...
**Symptom**: `ModuleNotFoundError: No module named 'graphit'` or similar import errors
...
echo $PYTHONPATH
export PYTHONPATH=/home/mark/work/graphit.ai/graphit:$PYTHONPATH

# Try running tests from project root
cd /home/mark/work/graphit.ai/graphit
pytest tests/unit/test_text_completion/test_vertexai_processor.py -v
```

**Common causes**:
- GraphIt packages not installed (`pip install -e .` in each package directory)
...
**Solution**:
```bash
# Try the simple tests first - they don't require full GraphIt infrastructure
./run_simple_tests.sh
...
**Why this happens**:
- The full GraphIt processors require async task groups and Pulsar infrastructure
...
# Run tests with coverage
pytest --cov=graphit --cov-report=xml --cov-fail-under=80
...
pytest --cov=graphit --cov-report=html --cov-report=xml --junitxml=test-results.xml
```
...
from graphit.new_service.processor import Processor
...
class TestNewProcessor(IsolatedAsyncioTestCase):
    ...
    @patch('graphit.new_service.processor.external_dependency')
...
This testing guide provides comprehensive instructions for running and maintaining the GraphIt test suite. Follow the patterns and guidelines to ensure consistent, reliable, and maintainable tests across all services.