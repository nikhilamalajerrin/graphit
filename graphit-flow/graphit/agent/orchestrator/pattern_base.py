...
from graphit.provenance import (
    agent_session_uri,
    agent_iteration_uri,
    agent_thought_uri,
...
class PatternBase:
    ...
    async def emit_iteration_triples(self, flow, session_id, iteration_num,
        ...
        if act.thought:
            thought_doc_id = (
                f"urn:graphit:agent:{session_id}/i{iteration_num}/thought"
            )
            try:
    ...
    async def emit_observation_triples(self, flow, session_id, iteration_num,
        ...
        if observation_text:
            observation_doc_id = (
                f"urn:graphit:agent:{session_id}/i{iteration_num}/observation"
            )
            try:
                await flow.librarian.save_document(
    ...
    async def emit_final_triples(self, flow, session_id, iteration_num,
        ...
        answer_doc_id = None
        if answer_text:
            answer_doc_id = f"urn:graphit:agent:{session_id}/answer"
            try:
                await flow.librarian.save_document(
    ...
    async def emit_finding_triples(
        ...
        doc_id = f"urn:graphit:agent:{session_id}/finding/{index}/doc"
        try:
            await flow.librarian.save_document(
                doc_id=doc_id,
                content=answer_text,
    ...
    async def emit_step_result_triples(
        ...
        doc_id = f"urn:graphit:agent:{session_id}/step/{index}/doc"
        try:
            await flow.librarian.save_document(
                doc_id=doc_id,
                content=answer_text,
    ...
    async def emit_synthesis_triples(
        ...
        doc_id = f"urn:graphit:agent:{session_id}/synthesis/doc"
        try:
            await flow.librarian.save_document(
                doc_id=doc_id,
                content=answer_text,
