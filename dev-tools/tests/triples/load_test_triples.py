#!/usr/bin/env python3
"""
Load test triples into the triple store for testing gr-query-graph.
...
# Configuration
API_URL = os.getenv("GRAPHIT_URL", "http://localhost:8088/")
TOKEN = os.getenv("GRAPHIT_TOKEN", None)
FLOW = "default"
USER = "graphit"
COLLECTION = "default"
...
def main():
    print("Loading test triples for gr-query-graph testing\n")
    asyncio.run(load_triples())
    print("\nDone! Now test with:")
    print("  gr-query-graph -s http://example.org/marie-curie")
    print("  gr-query-graph -p http://www.w3.org/2000/01/rdf-schema#label")
    print("  gr-query-graph -o 'Marie Curie' --object-language en")
    print("  gr-query-graph --format json | jq .")
