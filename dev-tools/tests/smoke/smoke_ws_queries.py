#!/usr/bin/env python3
"""
WebSocket smoke / load test that hammers a GraphIt gateway with a
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--url",
        default=os.getenv("GRAPHIT_URL", "http://localhost:8088/"),
        help="Gateway URL (http or ws). Default: %(default)s",
    )
    p.add_argument(
        "--token",
        default=os.getenv("GRAPHIT_TOKEN"),
        help="Auth token (or set GRAPHIT_TOKEN). Optional — if "
