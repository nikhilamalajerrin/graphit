...
"""
Client utility for browsing and loading documents from the GraphIt
...
from graphit.api.types import Uri, Literal, Triple

BUCKET_URL = "https://storage.googleapis.com/graphit-library"
...
def load_document(api, doc_entry):
    """Fetch metadata and content for a document, then load into GraphIt."""
    doc_id = doc_entry["id"]
    ...
    print(f"    loading into GraphIt ({len(content) // 1024}KB)...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Browse and load documents from the GraphIt public document library.",
    )
    ...
    sub.add_parser("load-all", help="Load all documents into GraphIt")
