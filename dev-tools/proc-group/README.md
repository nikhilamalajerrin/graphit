# proc-group — run GraphIt as a single process
...
Install the GraphIt packages into a venv:

```
pip install graphit-base graphit-flow graphit-unstructured
```

`graphit-base` provides the `processor-group` endpoint. The others
provide the processor classes that `group.yaml` imports at runtime.
`graphit-unstructured` is only needed if you want `document-decoder`
...
podman-compose up -d init-graphit
```

`init-graphit` is a one-shot that seeds config and the default flow
into cassandra/rabbitmq.  Don't leave too long a delay between starting
`init-graphit` and running the processor-group, because it needs to
